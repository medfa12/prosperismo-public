...
static vk::Device VulkanCreateDevice(vk::PhysicalDevice physical_device, const VulkanExtensions& r,
	...
	const auto provoking_vertex_ext_enabled =
	    HasExtension(device_extensions, VK_EXT_PROVOKING_VERTEX_EXTENSION_NAME);
	const auto conservative_rasterization_ext_enabled =
	    HasExtension(device_extensions, VK_EXT_CONSERVATIVE_RASTERIZATION_EXTENSION_NAME);

	vk::PhysicalDeviceConservativeRasterizationPropertiesEXT conservative_properties {};
	conservative_properties.sType =
	    vk::StructureType::ePhysicalDeviceConservativeRasterizationPropertiesEXT;
	vk::PhysicalDeviceProperties2 properties2 {};
	properties2.sType = vk::StructureType::ePhysicalDeviceProperties2;
	properties2.pNext = conservative_rasterization_ext_enabled
	                        ? static_cast<void*>(&conservative_properties)
	                        : nullptr;
	physical_device.getProperties2(&properties2);
	graphics.conservative_rasterization_enabled = conservative_rasterization_ext_enabled;
	graphics.conservative_rasterization_underestimation_enabled =
	    conservative_rasterization_ext_enabled &&
	    conservative_properties.primitiveUnderestimation == VK_TRUE;
	graphics.conservative_point_line_rasterization_enabled =
	    conservative_rasterization_ext_enabled &&
	    conservative_properties.conservativePointAndLineRasterization == VK_TRUE;
	...
	vk::PhysicalDeviceProvokingVertexFeaturesEXT supported_provoking_vertex {};
	supported_provoking_vertex.sType =
	    vk::StructureType::ePhysicalDeviceProvokingVertexFeaturesEXT;
	supported_provoking_vertex.pNext =
	    robustness2_ext_enabled ? static_cast<void*>(&supported_robustness2) : nullptr;
	if (provoking_vertex_ext_enabled) {
		supported_features12.pNext = &supported_provoking_vertex;
	...
	device_features.dualSrcBlend = supported_features2.features.dualSrcBlend;
	graphics.dual_source_blend_enabled = device_features.dualSrcBlend == VK_TRUE;
	...
	device_features.logicOp              = supported_features2.features.logicOp;
	graphics.logic_op_enabled            = device_features.logicOp == VK_TRUE;
	device_features.depthBiasClamp         = supported_features2.features.depthBiasClamp;
	graphics.depth_bias_clamp_enabled      = device_features.depthBiasClamp == VK_TRUE;
	graphics.provoking_vertex_last_enabled =
	    provoking_vertex_ext_enabled &&
	...
	provoking_vertex.sType = vk::StructureType::ePhysicalDeviceProvokingVertexFeaturesEXT;
	provoking_vertex.pNext =
	    robustness2_ext_enabled ? static_cast<void*>(&robustness2)
	                            : static_cast<void*>(&features12);
	provoking_vertex.provokingVertexLast =
	    graphics.provoking_vertex_last_enabled ? VK_TRUE : VK_FALSE;

	auto features13 = required_features13;
	features13.pNext = provoking_vertex_ext_enabled
	...
	LOGF("Vulkan provoking vertex: extension=%s last=%s\n",
	     provoking_vertex_ext_enabled ? "true" : "false",
	     graphics.provoking_vertex_last_enabled ? "true" : "false");
	LOGF("Vulkan conservative rasterization: extension=%s underestimate=%s point_line=%s\n",
	     graphics.conservative_rasterization_enabled ? "true" : "false",
	     graphics.conservative_rasterization_underestimation_enabled ? "true" : "false",
	     graphics.conservative_point_line_rasterization_enabled ? "true" : "false");
	...
	// Validation is an instance-layer contract. Vulkan 1.0 device layers were
	// deprecated before they ever became functional, and current Vulkan requires
	// device enabledLayerCount to be zero (VUID-VkDeviceCreateInfo-enabledLayerCount-12384).
...
void WindowContext::CreateVulkan() {
	...
	app_info.pApplicationName   = "Prosperismo";
	app_info.applicationVersion = 1;
	app_info.pEngineName        = "Prosperismo";
...

	{
		...
		if (HasExtension(available_extensions, VK_EXT_PROVOKING_VERTEX_EXTENSION_NAME)) {
