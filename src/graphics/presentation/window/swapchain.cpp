...
void Presenter::Present(Frame& frame, bool reuse) {
	...
	for (uint32_t attempt = 0; attempt < 2; attempt++) {
		...
		// The writer trace normally samples an image when the next writer arrives.  A
		// frame's final writer has no successor, so flush it at the completed present
		// boundary.  This is opt-in and only performs a readback when the trace target
		// environment variable is set.
		{
			Common::LockGuard render_lock(m_impl->renderer.GetMutex());
			m_impl->renderer.GetTextureCache().DebugFlushImageWriterTrace();
		}
