...
Opcode DecodeMimgOpcode(uint32_t opcode, const MimgSampleInfo* sample, const MimgGatherInfo* gather,
	...
	switch (opcode) {
		...
		// IMAGE_MSAA_LOAD is a distinct native MIMG opcode (0x80), but it has the
		// same result/dmask path as IMAGE_LOAD.  The multisample dimension below
		// makes the sample coordinate explicit to the existing SPIR-V fetch path.
		case 0x80u: return Opcode::ImageLoad;
...
uint32_t DecodeMimgAddressComponents(uint32_t opcode, ImageDimension dimension,
	...
	if (atomic != nullptr) {
		...
		case 0x80u: return ImageCoordComponents(dimension);
		default: return 0;
	}
}
...
bool DecodeMimg(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (opcode == 0x80u && dimension != ImageDimension::Dim2DMsaa &&
	    dimension != ImageDimension::Dim2DMsaaArray) {
		SetUnsupported(inst, Family::MIMG, opcode,
		               "image_msaa_load requires a multisampled image dimension");
	}
