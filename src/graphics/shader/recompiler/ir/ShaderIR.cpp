...
bool LowerScalarConditionalMove(const Decoder::Instruction& decoded, BasicBlock& block,
                                std::string* error) {
	Decoder::Operand scc;
	scc.kind = Decoder::OperandKind::Scc;
	...
	if (!LowerRegisterOperand(decoded.dst, inst.dst, error) ||
	    !LowerRegisterOperand(scc, inst.src[0], error) ||
	    !LowerSourceOperand(decoded.src0, inst.src[1], error) ||
	    !LowerRegisterOperand(decoded.dst, inst.src[2], error)) {
		return false;
	}
	block.instructions.push_back(inst);
	return true;
...
bool LowerScalarConditionalMove64(const Decoder::Instruction& decoded, BasicBlock& block,
                                  std::string* error) {
	Decoder::Operand scc;
	scc.kind = Decoder::OperandKind::Scc;
...
bool LowerScalarBitset0B64(const Decoder::Instruction& decoded, BasicBlock& block,
                           std::string* error) {
	Instruction inst;
	inst.pc        = decoded.pc;
	inst.op        = Opcode::BitClearU64;
	inst.src_count = 2;
	if (!LowerRegisterOperand(decoded.dst, inst.dst, error) ||
	    !LowerSourceOperand(decoded.dst, inst.src[0], error) ||
	    !LowerSourceOperand(decoded.src0, inst.src[1], error)) {
...
bool LowerScalarBitset1B64(const Decoder::Instruction& decoded, BasicBlock& block,
                           std::string* error) {
	Instruction inst;
	inst.pc        = decoded.pc;
	inst.op        = Opcode::BitSetU64;
	inst.src_count = 2;
	if (!LowerRegisterOperand(decoded.dst, inst.dst, error) ||
	    !LowerSourceOperand(decoded.dst, inst.src[0], error) ||
	    !LowerSourceOperand(decoded.src0, inst.src[1], error)) {
...
bool LowerVectorSwapB32(const Decoder::Instruction& decoded, BasicBlock& block,
                        std::string* error) {
	if (decoded.dst.kind != Decoder::OperandKind::Vgpr ||
	    decoded.src0.kind != Decoder::OperandKind::Vgpr || decoded.dst.sdwa_sel != 6u ||
	    decoded.src0.sdwa_sel != 6u || decoded.dst.dpp || decoded.src0.dpp || decoded.dst.clamp ||
	    decoded.src0.negate || decoded.src0.absolute || decoded.dst.omod != 0u ||
	    decoded.src0.omod != 0u) {
		SetError(error, "V_SWAP_B32 requires unmodified VGPR source and destination");
		return false;
	}
	Instruction inst;
	inst.pc        = decoded.pc;
	inst.op        = Opcode::SwapU32;
	inst.src_count = 1;
	if (!LowerRegisterOperand(decoded.dst, inst.dst, error) ||
	    !LowerSourceOperand(decoded.src0, inst.src[0], error)) {
		return false;
...
bool LowerVectorSwapRelB32(const Decoder::Instruction& decoded, BasicBlock& block,
                           std::string* error) {
	if (decoded.dst.kind != Decoder::OperandKind::Vgpr ||
	    decoded.src0.kind != Decoder::OperandKind::Vgpr) {
		SetError(error, "V_SWAPREL_B32 requires VGPR source and destination");
		return false;
	}
	if (decoded.dst.sdwa_sel != 6u || decoded.dst.omod != 0u || decoded.dst.clamp ||
		...
		SetError(error, "V_SWAPREL_B32 modifiers are not implemented");
		return false;
	}

	Instruction inst;
	inst.pc        = decoded.pc;
	inst.op        = Opcode::SwapRelU32;
	inst.src_count = 2;
...
bool LowerVectorSubrevBorrowCarry(const Decoder::Instruction& decoded, BasicBlock& block,
                                  std::string* error) {
	Instruction inst;
	inst.pc        = decoded.pc;
...
bool IsScalarSaveexecOpcode(Decoder::Opcode opcode) {
	switch (opcode) {
		...
		case Decoder::Opcode::SOrSaveexecB32:
		case Decoder::Opcode::SXorSaveexecB32:
		case Decoder::Opcode::SAndn2SaveexecB32:
		case Decoder::Opcode::SOrn2SaveexecB32:
		case Decoder::Opcode::SNandSaveexecB32:
		case Decoder::Opcode::SNorSaveexecB32:
		case Decoder::Opcode::SXnorSaveexecB32:
		case Decoder::Opcode::SOrn1SaveexecB32:
		case Decoder::Opcode::SAndSaveexecB64:
		case Decoder::Opcode::SOrSaveexecB64:
		case Decoder::Opcode::SXorSaveexecB64:
		case Decoder::Opcode::SAndn2SaveexecB64:
		case Decoder::Opcode::SNandSaveexecB64:
		case Decoder::Opcode::SNorSaveexecB64:
		case Decoder::Opcode::SXnorSaveexecB64:
		case Decoder::Opcode::SOrn1SaveexecB64:
...
SaveexecMode ScalarSaveexecMode(Decoder::Opcode opcode) {
	switch (opcode) {
		case Decoder::Opcode::SOrSaveexecB32:
		case Decoder::Opcode::SOrSaveexecB64: return SaveexecMode::Or;
		case Decoder::Opcode::SXorSaveexecB32:
		case Decoder::Opcode::SXorSaveexecB64: return SaveexecMode::Xor;
		case Decoder::Opcode::SAndn2SaveexecB32:
		case Decoder::Opcode::SAndn2SaveexecB64: return SaveexecMode::Andn2;
		case Decoder::Opcode::SNandSaveexecB32:
		case Decoder::Opcode::SNandSaveexecB64: return SaveexecMode::Nand;
		case Decoder::Opcode::SNorSaveexecB32:
		case Decoder::Opcode::SNorSaveexecB64: return SaveexecMode::Nor;
		case Decoder::Opcode::SXnorSaveexecB32:
		case Decoder::Opcode::SXnorSaveexecB64: return SaveexecMode::Xnor;
		case Decoder::Opcode::SOrn1SaveexecB32:
		case Decoder::Opcode::SOrn1SaveexecB64: return SaveexecMode::Orn1;
		case Decoder::Opcode::SOrn2SaveexecB32:
...
bool LowerScalarSaveexec(const Decoder::Instruction& decoded, BasicBlock& block,
	...
	inst.op            = (decoded.opcode == Decoder::Opcode::SAndSaveexecB32 ||
	                      ...
	                      decoded.opcode == Decoder::Opcode::SOrSaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SXorSaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SAndn2SaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SOrn2SaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SNandSaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SNorSaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SXnorSaveexecB32 ||
	                      decoded.opcode == Decoder::Opcode::SOrn1SaveexecB32)
	                         ? Opcode::SaveexecB32
	                         : Opcode::SaveexecB64;
...
bool LowerDecodedInstruction(const Decoder::Instruction& inst, BasicBlock& block,
	...
	switch (inst.opcode) {
		...
		case Decoder::Opcode::SCmovB32: return LowerScalarConditionalMove(inst, block, error);
		case Decoder::Opcode::SCmovB64: return LowerScalarConditionalMove64(inst, block, error);
		case Decoder::Opcode::SCmovkI32: return LowerScalarConditionalMove(inst, block, error);
		...
		case Decoder::Opcode::SBitset0B64: return LowerScalarBitset0B64(inst, block, error);
		case Decoder::Opcode::SBitset1B32: return LowerScalarBitset1B32(inst, block, error);
		case Decoder::Opcode::SBitset1B64: return LowerScalarBitset1B64(inst, block, error);
		...
		case Decoder::Opcode::VSwapB32: return LowerVectorSwapB32(inst, block, error);
		case Decoder::Opcode::VSwaprelB32: return LowerVectorSwapRelB32(inst, block, error);
