...
void CollectPixelInputs(const ShaderPixelInputInfo* pixel, ShaderInfo& info) {
	...
	if (pixel->HasPositionInput() || pixel->HasAaSampleMask() || pixel->HasAlphaToMaskDither()) {
		AddInput(info, StageInputKind::FragCoord, 0, 4, "gl_FragCoord");
	}
	if (pixel->HasPsShaderSampleExclusionMask() || pixel->HasAlphaToMaskDither()) {
		AddInput(info, StageInputKind::SampleMask, 0, 1, "gl_SampleMaskIn");
	}
...
void CollectOutputs(const Program& program, const ShaderPixelInputInfo* pixel, ShaderInfo& info) {
	if (pixel != nullptr &&
	    (pixel->HasAaSampleMask() || pixel->HasPsShaderSampleExclusionMask() ||
	     pixel->HasAlphaToMaskDither())) {
