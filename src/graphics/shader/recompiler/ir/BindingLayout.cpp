...
bool IsStorageBinding(DescriptorBindingKind kind) {
	return kind >= DescriptorBindingKind::Storage1D && kind <= DescriptorBindingKind::StorageUint3D;
}

uint32_t DescriptorSpan(const Program& program, DescriptorBindingKind kind, uint32_t resource) {
	if (!IsStorageBinding(kind) || resource >= program.info.images.size()) {
		return 1;
	}
	const auto& image = program.info.images[resource];
	return StorageMipCount(image.mip_mode);
}
...
uint32_t DescriptorCount(const Program& program, const DescriptorBinding& binding) {
	...
	for (const auto resource: binding.resources) {
		count += DescriptorSpan(program, binding.kind, resource);
	}
	return count;
}
...
bool DescriptorArrayIndex(const Program& program, const DescriptorBinding& binding,
	...
	index = 0;
	for (const auto current: binding.resources) {
		if (current == resource) {
			return true;
		}
		index += DescriptorSpan(program, binding.kind, current);
	}
	return false;
}
...
bool AllocateBindings(Program& program, const BindingLayoutOptions& options, std::string* error) {
	...
	next.address_base_dword =
	    next.buffer_offset_dword + (next.buffer_offset_count + 3u) / 4u;
	next.address_base_count = static_cast<uint32_t>(program.info.addresses.size());
