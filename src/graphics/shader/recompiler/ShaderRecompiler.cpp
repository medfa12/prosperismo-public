...
bool TryRecompile(std::span<const uint32_t> code, const CompileOptions& options,
	...
	if (options.allocated_vgpr_count == 0 || options.allocated_vgpr_count > 256u) {
		if (error != nullptr) {
			*error = "VGPR allocation must contain between 1 and 256 registers";
		}
		return false;
	}
	ir.allocated_vgpr_count = options.allocated_vgpr_count;
