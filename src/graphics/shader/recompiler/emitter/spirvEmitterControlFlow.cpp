...
void EmitInitialSampleMask(EmitterState& state) {
	if (state.stage != ShaderType::Pixel || state.sample_mask_variable == 0) {
		return;
	}
	...
	state.builder.AddFunction({OpStore, output, ConstantI32(state, -1)});
}

void EmitAaSampleMask(EmitterState& state) {
	if (state.stage != ShaderType::Pixel || state.pixel_input_info == nullptr ||
	    !state.pixel_input_info->HasAaSampleMask() || state.sample_mask_variable == 0) {
		return;
	}
	...
	const auto     mask     = state.pixel_input_info->aa_sample_mask;
	const uint32_t masks[4] = {
	    static_cast<uint32_t>(mask) & 0xffffu,
	    (static_cast<uint32_t>(mask) >> 16u) & 0xffffu,
	    static_cast<uint32_t>(mask >> 32u) & 0xffffu,
	    (static_cast<uint32_t>(mask >> 32u) >> 16u) & 0xffffu,
	};
	const auto is_right_top    = state.builder.AllocateId();
	const auto is_right_bottom = state.builder.AllocateId();
	const auto is_bottom       = state.builder.AllocateId();
	const auto top_mask        = state.builder.AllocateId();
	const auto bottom_mask     = state.builder.AllocateId();
	const auto selected        = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpIEqual, state.bool_type, is_right_top, quadrant, ConstantU32(state, 1u)});
	state.builder.AddFunction(
	    {OpIEqual, state.bool_type, is_right_bottom, quadrant, ConstantU32(state, 3u)});
	state.builder.AddFunction(
	    {OpUGreaterThanEqual, state.bool_type, is_bottom, quadrant, ConstantU32(state, 2u)});
	state.builder.AddFunction({OpSelect, state.uint_type, top_mask, is_right_top,
	                           ConstantU32(state, masks[1]), ConstantU32(state, masks[0])});
	state.builder.AddFunction({OpSelect, state.uint_type, bottom_mask, is_right_bottom,
	                           ConstantU32(state, masks[3]), ConstantU32(state, masks[2])});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, selected, is_bottom, bottom_mask, top_mask});
	...
	const auto signed_mask    = state.builder.AllocateId();
	state.builder.AddFunction({OpAccessChain, state.ptr_output_int, output,
	                           state.sample_mask_variable, ConstantU32(state, 0)});
	...
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, combined, selected, current_mask});
	state.builder.AddFunction({OpBitcast, state.int_type, signed_mask, combined});
	state.builder.AddFunction({OpStore, output, signed_mask});
}

void EmitPsShaderSampleExclusionMask(EmitterState& state) {
	if (state.stage != ShaderType::Pixel || state.pixel_input_info == nullptr ||
	    !state.pixel_input_info->HasPsShaderSampleExclusionMask() ||
	    state.sample_mask_variable == 0) {
		return;
	}

	const auto input = InputVariableForKind(state, IR::StageInputKind::SampleMask);
	if (input == 0) {
		return;
	}
	...
	state.builder.AddFunction({OpLoad, state.int_type, current_signed, output});
	state.builder.AddFunction({OpBitcast, state.uint_type, current_mask, current_signed});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, combined, covered, current_mask});
	covered = combined;
	...
	state.builder.AddFunction(
	    {OpNot, state.uint_type, allowed,
	     ConstantU32(state, state.pixel_input_info->ps_shader_sample_exclusion_mask)});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, masked, covered, allowed});
	state.builder.AddFunction(
	    {OpINotEqual, state.bool_type, active, masked, ConstantU32(state, 0)});
	state.builder.AddFunction({OpBitcast, state.int_type, result, masked});
	state.builder.AddFunction({OpStore, output, result});
...
void EmitInstruction(EmitterState& state, const IR::Instruction& inst) {
	switch (inst.op) {
		...
		case IR::Opcode::SwapU32: EmitSwapU32(state, inst); break;
		case IR::Opcode::SwapRelU32: EmitSwapRelU32(state, inst); break;
		...
		case IR::Opcode::QuadMaskB32: EmitQuadMaskB32(state, inst); break;
		case IR::Opcode::QuadMaskB64: EmitQuadMaskB64(state, inst); break;
		...
		case IR::Opcode::BitReverseU64: EmitBitReverseU64(state, inst); break;
		case IR::Opcode::BitCountZeroU32: EmitBitCountZeroU32(state, inst); break;
		case IR::Opcode::BitCountZeroU64: EmitBitCountZeroU64(state, inst); break;
		...
		case IR::Opcode::FindLsbZeroU32: EmitFindLsbZeroU32(state, inst); break;
		case IR::Opcode::FindLsbZeroU64: EmitFindLsbZeroU64(state, inst); break;
		...
		case IR::Opcode::FindMsbSignI32: EmitFindMsbSignI32(state, inst); break;
		case IR::Opcode::FindMsbSignI64: EmitFindMsbSignI64(state, inst); break;
		case IR::Opcode::SignExtendI8U32: EmitSignExtendI8U32(state, inst); break;
		case IR::Opcode::SignExtendI16U32: EmitSignExtendI16U32(state, inst); break;
		...
		case IR::Opcode::BitFieldExtractI64: EmitBitFieldExtractI64(state, inst); break;
		...
		case IR::Opcode::BitCompare0B64: EmitBitCompareB64(state, inst, false); break;
		case IR::Opcode::BitCompare1B64: EmitBitCompareB64(state, inst, true); break;
		case IR::Opcode::AlignBitU32: EmitAlignBitU32(state, inst); break;
		case IR::Opcode::AlignByteU32: EmitAlignByteU32(state, inst); break;
		case IR::Opcode::PermuteBytesU32: EmitPermuteBytesU32(state, inst); break;
		case IR::Opcode::MeanBytesU32: EmitMeanBytesU32(state, inst); break;
		case IR::Opcode::SadBytesU32: EmitSadBytesU32(state, inst); break;
		case IR::Opcode::SadBytesHighU32: EmitSadBytesHighU32(state, inst); break;
		case IR::Opcode::MaskedSadBytesU32: EmitMaskedSadBytesU32(state, inst); break;
		case IR::Opcode::QuadSadPackedU16U8: EmitQuadSadPackedU16U8(state, inst); break;
		case IR::Opcode::MaskedQuadSadPackedU16U8: EmitMaskedQuadSadPackedU16U8(state, inst); break;
		case IR::Opcode::SadWordsU32: EmitSadWordsU32(state, inst); break;
		case IR::Opcode::SaturatePackU8I16: EmitSaturatePackU8I16(state, inst); break;
		...
		case IR::Opcode::ShiftRightArithmeticI32:
			...
			break;
		case IR::Opcode::ShiftRightArithmeticI64: EmitShiftRightArithmeticI64(state, inst); break;
		...
		case IR::Opcode::PackU16U32: EmitClampPackU16U32(state, inst); break;
		...
		case IR::Opcode::FractF16:
		case IR::Opcode::SinF16:
		case IR::Opcode::CosF16: EmitSpecialF16(state, inst); break;
		...
		case IR::Opcode::SBufferLoadDword: EmitSBufferLoadDword(state, inst); break;
		...
		case IR::Opcode::ImageBvhIntersectRay: EmitImageBvhIntersectRay(state, inst); break;
...
void EmitFunction(EmitterState& state, const IR::Program& program) {
	...
	EmitInitialSampleMask(state);
	EmitAaSampleMask(state);
	EmitPsShaderSampleExclusionMask(state);
