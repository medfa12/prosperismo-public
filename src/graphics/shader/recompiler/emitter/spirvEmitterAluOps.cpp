...
void EmitAbsDiffI32(EmitterState& state, const IR::Instruction& inst) {
	...
	const auto cond = state.builder.AllocateId();
	const auto ret  = state.builder.AllocateId();
	// The subtraction wraps to 32 bits before its absolute value is taken.
	state.builder.AddFunction({OpISub, state.uint_type, diff, lhs, rhs});
	state.builder.AddFunction({OpSNegate, state.uint_type, neg, diff});
	state.builder.AddFunction(
	    {OpSLessThan, state.bool_type, cond, diff, ConstantU32(state, 0)});
	state.builder.AddFunction({OpSelect, state.uint_type, ret, cond, neg, diff});
	EmitStoreU32(state, inst.dst, ret);
}
...
void EmitBitReverseU64(EmitterState& state, const IR::Instruction& inst) {
	const auto low      = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high     = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto ret_low  = state.builder.AllocateId();
	const auto ret_high = state.builder.AllocateId();
	state.builder.AddFunction({OpBitReverse, state.uint_type, ret_low, high});
	state.builder.AddFunction({OpBitReverse, state.uint_type, ret_high, low});
	EmitStoreU32(state, inst.dst, ret_low);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), ret_high);
}
...
void EmitFindLsbZeroU32(EmitterState& state, const IR::Instruction& inst) {
	const auto src      = EmitValueLoad(state, inst.src[0]);
	const auto inverted = state.builder.AllocateId();
	const auto i32      = state.builder.AllocateId();
	const auto u32      = state.builder.AllocateId();
	state.builder.AddFunction({OpNot, state.uint_type, inverted, src});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, i32, state.glsl_std450, GlslFindILsb, inverted});
	state.builder.AddFunction({OpBitcast, state.uint_type, u32, i32});
	EmitStoreU32(state, inst.dst, u32);
}

void EmitFindLsbZeroU64(EmitterState& state, const IR::Instruction& inst) {
	const auto low           = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high          = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto low_not       = state.builder.AllocateId();
	const auto high_not      = state.builder.AllocateId();
	...
	const auto low_has_zero  = state.builder.AllocateId();
	const auto high_has_zero = state.builder.AllocateId();
	const auto high_result   = state.builder.AllocateId();
	const auto result        = state.builder.AllocateId();
	state.builder.AddFunction({OpNot, state.uint_type, low_not, low});
	state.builder.AddFunction({OpNot, state.uint_type, high_not, high});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, low_i32, state.glsl_std450, GlslFindILsb, low_not});
	state.builder.AddFunction({OpBitcast, state.uint_type, low_lsb, low_i32});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, high_i32, state.glsl_std450, GlslFindILsb, high_not});
	...
	state.builder.AddFunction(
	    {OpINotEqual, state.bool_type, low_has_zero, low_not, ConstantU32(state, 0)});
	state.builder.AddFunction(
	    {OpINotEqual, state.bool_type, high_has_zero, high_not, ConstantU32(state, 0)});
	state.builder.AddFunction({OpSelect, state.uint_type, high_result, high_has_zero, high_total,
	                           ConstantU32(state, 0xffffffffu)});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, result, low_has_zero, low_lsb, high_result});
...
void EmitFindMsbFromHighU32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction({OpINotEqual, state.bool_type, non_zero, src, ConstantU32(state, 0)});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, ret, non_zero, pos, ConstantU32(state, 0xffffffffu)});
	EmitStoreU32(state, inst.dst, ret);
}

void EmitFindMsbSignI32(EmitterState& state, const IR::Instruction& inst) {
	const auto src        = EmitValueLoad(state, inst.src[0]);
	const auto src_i32    = state.builder.AllocateId();
	const auto mask_i32   = state.builder.AllocateId();
	...
	const auto msb        = state.builder.AllocateId();
	const auto pos        = state.builder.AllocateId();
	const auto has_bit    = state.builder.AllocateId();
	const auto ret        = state.builder.AllocateId();
	state.builder.AddFunction({OpBitcast, state.int_type, src_i32, src});
	state.builder.AddFunction({OpShiftRightArithmetic, state.int_type, mask_i32, src_i32,
	                           ConstantU32(state, 31)});
	state.builder.AddFunction({OpBitcast, state.uint_type, mask, mask_i32});
	state.builder.AddFunction({OpBitwiseXor, state.uint_type, normalized, src, mask});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, msb_i32, state.glsl_std450, GlslFindUMsb, normalized});
	state.builder.AddFunction({OpBitcast, state.uint_type, msb, msb_i32});
	state.builder.AddFunction({OpISub, state.uint_type, pos, ConstantU32(state, 31), msb});
	state.builder.AddFunction(
	    {OpINotEqual, state.bool_type, has_bit, normalized, ConstantU32(state, 0)});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, ret, has_bit, pos, ConstantU32(state, 0xffffffffu)});
	EmitStoreU32(state, inst.dst, ret);
}
...
void EmitFindMsbFromHighU64(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction({OpISub, state.uint_type, low_pos, ConstantU32(state, 31), low_msb});
	state.builder.AddFunction(
...
void EmitFindMsbSignI64(EmitterState& state, const IR::Instruction& inst) {
	const auto low             = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high            = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto high_signed     = state.builder.AllocateId();
	const auto sign_mask_i32   = state.builder.AllocateId();
	const auto sign_mask       = state.builder.AllocateId();
	const auto normalized_low  = state.builder.AllocateId();
	const auto normalized_high = state.builder.AllocateId();
	const auto high_i32        = state.builder.AllocateId();
	const auto high_msb        = state.builder.AllocateId();
	const auto high_pos        = state.builder.AllocateId();
	const auto high_non_zero   = state.builder.AllocateId();
	...
	const auto low_ret         = state.builder.AllocateId();
	const auto ret             = state.builder.AllocateId();
	state.builder.AddFunction({OpBitcast, state.int_type, high_signed, high});
	state.builder.AddFunction({OpShiftRightArithmetic, state.int_type, sign_mask_i32, high_signed,
	                           ConstantU32(state, 31)});
	state.builder.AddFunction({OpBitcast, state.uint_type, sign_mask, sign_mask_i32});
	state.builder.AddFunction(
	    {OpBitwiseXor, state.uint_type, normalized_low, low, sign_mask});
	state.builder.AddFunction(
	    {OpBitwiseXor, state.uint_type, normalized_high, high, sign_mask});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, high_i32, state.glsl_std450, GlslFindUMsb, normalized_high});
	state.builder.AddFunction({OpBitcast, state.uint_type, high_msb, high_i32});
	state.builder.AddFunction(
	    {OpISub, state.uint_type, high_pos, ConstantU32(state, 31), high_msb});
	state.builder.AddFunction({OpINotEqual, state.bool_type, high_non_zero, normalized_high,
	                           ConstantU32(state, 0)});
	state.builder.AddFunction(
	    {OpExtInst, state.int_type, low_i32, state.glsl_std450, GlslFindUMsb, normalized_low});
	state.builder.AddFunction({OpBitcast, state.uint_type, low_msb, low_i32});
	state.builder.AddFunction({OpISub, state.uint_type, low_pos, ConstantU32(state, 31), low_msb});
	state.builder.AddFunction(
	    {OpIAdd, state.uint_type, low_total, low_pos, ConstantU32(state, 32)});
	state.builder.AddFunction({OpINotEqual, state.bool_type, low_non_zero, normalized_low,
	                           ConstantU32(state, 0)});
	state.builder.AddFunction({OpSelect, state.uint_type, low_ret, low_non_zero, low_total,
	                           ConstantU32(state, 0xffffffffu)});
	state.builder.AddFunction({OpSelect, state.uint_type, ret, high_non_zero, high_pos, low_ret});
...
void EmitSignExtendI8U32(EmitterState& state, const IR::Instruction& inst) {
	const auto src          = EmitValueLoad(state, inst.src[0]);
	const auto signed_src   = state.builder.AllocateId();
	const auto extended_i32 = state.builder.AllocateId();
	const auto extended_u32 = state.builder.AllocateId();
	state.builder.AddFunction({OpBitcast, state.int_type, signed_src, src});
	state.builder.AddFunction({OpBitFieldSExtract, state.int_type, extended_i32, signed_src,
	                           ConstantU32(state, 0), ConstantU32(state, 8)});
	state.builder.AddFunction({OpBitcast, state.uint_type, extended_u32, extended_i32});
	EmitStoreU32(state, inst.dst, extended_u32);
}

void EmitSignExtendI16U32(EmitterState& state, const IR::Instruction& inst) {
	const auto src          = EmitValueLoad(state, inst.src[0]);
	const auto signed_src   = state.builder.AllocateId();
	const auto extended_i32 = state.builder.AllocateId();
	const auto extended_u32 = state.builder.AllocateId();
...
void EmitBitCountZeroU64(EmitterState& state, const IR::Instruction& inst) {
	const auto low        = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high       = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto low_not    = state.builder.AllocateId();
	const auto high_not   = state.builder.AllocateId();
	...
	state.builder.AddFunction({OpNot, state.uint_type, low_not, low});
	state.builder.AddFunction({OpNot, state.uint_type, high_not, high});
	state.builder.AddFunction({OpBitCount, state.uint_type, low_count, low_not});
	state.builder.AddFunction({OpBitCount, state.uint_type, high_count, high_not});
	state.builder.AddFunction({OpIAdd, state.uint_type, ret, low_count, high_count});
	EmitStoreU32(state, inst.dst, ret);
}

void EmitBitCountZeroU32(EmitterState& state, const IR::Instruction& inst) {
	const auto bits     = EmitValueLoad(state, inst.src[0]);
	const auto inverted = state.builder.AllocateId();
	const auto count    = state.builder.AllocateId();
	state.builder.AddFunction({OpNot, state.uint_type, inverted, bits});
	state.builder.AddFunction({OpBitCount, state.uint_type, count, inverted});
...
uint32_t EmitMask5U32(EmitterState& state, uint32_t value) {
	const auto ret = state.builder.AllocateId();
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, ret, value, ConstantU32(state, 31)});
	return ret;
}
...
void EmitShiftRightArithmeticI64(EmitterState& state, const IR::Instruction& inst) {
	const auto low              = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high             = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto amount           = EmitAndConstant(state, EmitValueLoad(state, inst.src[1]), 63u);
	const auto dword_shift      = EmitAndConstant(state, amount, 31u);
	const auto less32           = state.builder.AllocateId();
	const auto non_zero         = state.builder.AllocateId();
	const auto carry_count_base = state.builder.AllocateId();
	...
	const auto low_less         = state.builder.AllocateId();
	const auto low_ge           = state.builder.AllocateId();
	const auto sign_fill        = state.builder.AllocateId();
	const auto ret_low          = state.builder.AllocateId();
	const auto ret_high         = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpULessThan, state.bool_type, less32, amount, ConstantU32(state, 32)});
	...
	state.builder.AddFunction({OpShiftRightLogical, state.uint_type, low_part, low, dword_shift});
	state.builder.AddFunction(
	    {OpShiftRightArithmetic, state.uint_type, high_part, high, dword_shift});
	state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, carry_part, high, carry_count});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, carry, non_zero, carry_part, ConstantU32(state, 0)});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, low_less, low_part, carry});
	state.builder.AddFunction(
	    {OpShiftRightArithmetic, state.uint_type, low_ge, high, dword_shift});
	state.builder.AddFunction({OpShiftRightArithmetic, state.uint_type, sign_fill, high,
	                           ConstantU32(state, 31)});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_low, less32, low_less, low_ge});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_high, less32, high_part, sign_fill});
	EmitStoreU32(state, inst.dst, ret_low);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), ret_high);
}
...
void EmitBitClearU64(EmitterState& state, const IR::Instruction& inst) {
	const auto index = EmitAndConstant(state, EmitValueLoad(state, inst.src[1]), 63u);
	if (state.per_invocation_masks && inst.dst.kind == IR::OperandKind::Register &&
	    IsMaskRegisterFile(inst.dst.reg.file)) {
		const auto selected_lane = state.builder.AllocateId();
		const auto keep_lane     = state.builder.AllocateId();
		const auto active        = state.builder.AllocateId();
		state.builder.AddFunction({OpIEqual, state.bool_type, selected_lane,
		                           EmitSubgroupLocalInvocationId(state), index});
		state.builder.AddFunction({OpLogicalNot, state.bool_type, keep_lane, selected_lane});
		state.builder.AddFunction({OpLogicalAnd, state.bool_type, active,
		                           EmitLaneMaskOperandActiveBool(state, inst.src[0]), keep_lane});
	...
	const auto low          = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high         = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto dword_index  = EmitAndConstant(state, index, 31u);
	const auto mask         = state.builder.AllocateId();
	const auto inverse      = state.builder.AllocateId();
	const auto cleared_low  = state.builder.AllocateId();
	const auto cleared_high = state.builder.AllocateId();
	const auto select_low   = state.builder.AllocateId();
	const auto ret_low      = state.builder.AllocateId();
	const auto ret_high     = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpShiftLeftLogical, state.uint_type, mask, ConstantU32(state, 1), dword_index});
	state.builder.AddFunction({OpNot, state.uint_type, inverse, mask});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, cleared_low, low, inverse});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, cleared_high, high, inverse});
	state.builder.AddFunction(
	    {OpULessThan, state.bool_type, select_low, index, ConstantU32(state, 32)});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_low, select_low, cleared_low, low});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_high, select_low, high, cleared_high});
	EmitStoreU32(state, inst.dst, ret_low);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), ret_high);
}
...
void EmitBitSetU64(EmitterState& state, const IR::Instruction& inst) {
	...
	if (state.per_invocation_masks && inst.dst.kind == IR::OperandKind::Register &&
		...
		state.builder.AddFunction({OpIEqual, state.bool_type, selected_lane,
		                           EmitSubgroupLocalInvocationId(state), index});
		state.builder.AddFunction({OpLogicalOr, state.bool_type, active,
		                           EmitLaneMaskOperandActiveBool(state, inst.src[0]), selected_lane});
		EmitPerInvocationMask(state, inst.dst, active);
		return;
	}
	...
	const auto dword_index = EmitAndConstant(state, index, 31u);
	const auto mask        = state.builder.AllocateId();
	const auto set_low     = state.builder.AllocateId();
	const auto set_high    = state.builder.AllocateId();
	const auto select_low  = state.builder.AllocateId();
	const auto ret_low     = state.builder.AllocateId();
	const auto ret_high    = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpShiftLeftLogical, state.uint_type, mask, ConstantU32(state, 1), dword_index});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, set_low, low, mask});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, set_high, high, mask});
	state.builder.AddFunction(
	    {OpULessThan, state.bool_type, select_low, index, ConstantU32(state, 32)});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_low, select_low, set_low, low});
	state.builder.AddFunction({OpSelect, state.uint_type, ret_high, select_low, high, set_high});
	EmitStoreU32(state, inst.dst, ret_low);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), ret_high);
}
...
void EmitLaneMaskPairFromBool(EmitterState& state, const IR::Operand& dst, uint32_t value_bool) {
	...
	for (uint32_t part = 0; part < 2u; part++) {
		...
		if (state.wave_size != 32u || part == 0u) {
			...
			state.builder.AddFunction({OpCompositeExtract, state.uint_type, result, ballot, part});
		}
...
void EmitUMadU64U32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction({OpIAddCarry, state.uint_pair_type, carry_pair, high_sum, carry_low});
...
uint32_t EmitRightAlignedMaskU32(EmitterState& state, uint32_t count) {
	...
	state.builder.AddFunction({OpISub, state.uint_type, mask, shifted_one, ConstantU32(state, 1)});
	return mask;
}
...
void EmitRightAlignedMaskU64(EmitterState& state, uint32_t count, uint32_t& out_low,
	...
	state.builder.AddFunction({OpISub, state.uint_type, high_base, count, ConstantU32(state, 32)});
	state.builder.AddFunction(
	...
	state.builder.AddFunction({OpBitFieldInsert, state.uint_type, out_high, ConstantU32(state, 0),
	                           ConstantU32(state, 0xffffffffu), ConstantU32(state, 0), high_count});
}
...
void EmitBitFieldExtractU64(EmitterState& state, const IR::Instruction& inst) {
	...
	const auto ret_low      = state.builder.AllocateId();
	const auto ret_high     = state.builder.AllocateId();
	state.builder.AddFunction({OpISub, state.uint_type, available, ConstantU32(state, 64), offset});
...
void EmitBitFieldExtractI64(EmitterState& state, const IR::Instruction& inst) {
	const auto src_low        = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto src_high       = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto field          = EmitValueLoad(state, inst.src[1]);
	const auto offset         = EmitBitFieldExtractConstant(state, field, 0, 6);
	...
	uint32_t   mask_low       = 0;
	uint32_t   mask_high      = 0;
	const auto extracted_low  = state.builder.AllocateId();
	const auto extracted_high = state.builder.AllocateId();
	state.builder.AddFunction({OpISub, state.uint_type, available, ConstantU32(state, 64), offset});
	state.builder.AddFunction({OpULessThan, state.bool_type, use_raw, raw_count, available});
	state.builder.AddFunction({OpSelect, state.uint_type, count, use_raw, raw_count, available});
	EmitShiftRightLogicalU64Values(state, src_low, src_high, offset, shifted_low, shifted_high);
	EmitRightAlignedMaskU64(state, count, mask_low, mask_high);
	state.builder.AddFunction(
	    {OpBitwiseAnd, state.uint_type, extracted_low, shifted_low, mask_low});
	state.builder.AddFunction(
	    {OpBitwiseAnd, state.uint_type, extracted_high, shifted_high, mask_high});

	const auto sign_in_low       = state.builder.AllocateId();
	const auto sign_word         = state.builder.AllocateId();
	const auto raw_sign_position = state.builder.AllocateId();
	const auto sign_position     = state.builder.AllocateId();
	const auto align_count       = state.builder.AllocateId();
	const auto aligned_sign      = state.builder.AllocateId();
	const auto sign_fill         = state.builder.AllocateId();
	const auto inverse_low       = state.builder.AllocateId();
	const auto inverse_high      = state.builder.AllocateId();
	const auto fill_low          = state.builder.AllocateId();
	const auto fill_high         = state.builder.AllocateId();
	const auto ret_low           = state.builder.AllocateId();
	const auto ret_high          = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpULessThan, state.bool_type, sign_in_low, count, ConstantU32(state, 33)});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, sign_word, sign_in_low, extracted_low, extracted_high});
	state.builder.AddFunction(
	    {OpISub, state.uint_type, raw_sign_position, count, ConstantU32(state, 1)});
	state.builder.AddFunction(
	    {OpBitwiseAnd, state.uint_type, sign_position, raw_sign_position, ConstantU32(state, 31)});
	state.builder.AddFunction(
	    {OpISub, state.uint_type, align_count, ConstantU32(state, 31), sign_position});
	state.builder.AddFunction(
	    {OpShiftLeftLogical, state.uint_type, aligned_sign, sign_word, align_count});
	state.builder.AddFunction({OpShiftRightArithmetic, state.uint_type, sign_fill, aligned_sign,
	                           ConstantU32(state, 31)});
	state.builder.AddFunction({OpNot, state.uint_type, inverse_low, mask_low});
	state.builder.AddFunction({OpNot, state.uint_type, inverse_high, mask_high});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, fill_low, sign_fill, inverse_low});
	state.builder.AddFunction({OpBitwiseAnd, state.uint_type, fill_high, sign_fill, inverse_high});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, ret_low, extracted_low, fill_low});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, ret_high, extracted_high, fill_high});
	EmitStoreU32(state, inst.dst, ret_low);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), ret_high);
}
...
void EmitBitFieldExtractI32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction({OpBitcast, state.int_type, signed_src, src});
	state.builder.AddFunction(
	    {OpBitFieldSExtract, state.int_type, extracted, signed_src, offset, count});
...
void EmitAlignU32(EmitterState& state, const IR::Instruction& inst, uint32_t shift) {
...
void EmitAlignBitU32(EmitterState& state, const IR::Instruction& inst) {
	...
	EmitAlignU32(state, inst, shift);
}

void EmitAlignByteU32(EmitterState& state, const IR::Instruction& inst) {
	const uint32_t byte_selector = (inst.src[2].op_sel ? 1u : 0u) |
	                               (inst.src[2].op_sel_hi ? 2u : 0u);
	auto           selected_byte = EmitValueLoad(state, inst.src[2]);
	if (byte_selector != 0u) {
		const auto shifted_source = state.builder.AllocateId();
		state.builder.AddFunction({OpShiftRightLogical, state.uint_type, shifted_source,
		                           selected_byte, ConstantU32(state, byte_selector * 8u)});
		selected_byte = shifted_source;
	}
	const auto byte_count = EmitAndConstant(state, selected_byte, 3u);
	const auto shift      = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpShiftLeftLogical, state.uint_type, shift, byte_count, ConstantU32(state, 3u)});
	EmitAlignU32(state, inst, shift);
}

uint32_t EmitPermuteByteU32(EmitterState& state, uint32_t source_high, uint32_t source_low,
                            uint32_t selector) {
	const auto data_index      = EmitAndConstant(state, selector, 3u);
	const auto data_offset     = state.builder.AllocateId();
	const auto data_high_bit   = EmitAndConstant(state, selector, 4u);
	const auto data_from_high  = state.builder.AllocateId();
	const auto data_source     = state.builder.AllocateId();
	const auto data_byte       = state.builder.AllocateId();
	state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, data_offset, data_index,
	                           ConstantU32(state, 3u)});
	state.builder.AddFunction({OpINotEqual, state.bool_type, data_from_high, data_high_bit,
	                           ConstantU32(state, 0u)});
	state.builder.AddFunction({OpSelect, state.uint_type, data_source, data_from_high, source_high,
	                           source_low});
	state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, data_byte, data_source,
	                           data_offset, ConstantU32(state, 8u)});

	const auto sign_index      = EmitAndConstant(state, selector, 1u);
	const auto sign_offset_raw = state.builder.AllocateId();
	const auto sign_offset     = state.builder.AllocateId();
	const auto sign_high_bit   = EmitAndConstant(state, selector, 2u);
	const auto sign_from_high  = state.builder.AllocateId();
	const auto sign_source     = state.builder.AllocateId();
	const auto sign_byte       = state.builder.AllocateId();
	const auto sign_bit        = state.builder.AllocateId();
	const auto sign_set        = state.builder.AllocateId();
	const auto sign_fill       = state.builder.AllocateId();
	state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, sign_offset_raw, sign_index,
	                           ConstantU32(state, 4u)});
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, sign_offset, sign_offset_raw,
	                           ConstantU32(state, 8u)});
	state.builder.AddFunction({OpINotEqual, state.bool_type, sign_from_high, sign_high_bit,
	                           ConstantU32(state, 0u)});
	state.builder.AddFunction({OpSelect, state.uint_type, sign_source, sign_from_high, source_high,
	                           source_low});
	state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, sign_byte, sign_source,
	                           sign_offset, ConstantU32(state, 8u)});
	state.builder.AddFunction(
	    {OpBitwiseAnd, state.uint_type, sign_bit, sign_byte, ConstantU32(state, 0x80u)});
	state.builder.AddFunction({OpINotEqual, state.bool_type, sign_set, sign_bit,
	                           ConstantU32(state, 0u)});
	state.builder.AddFunction({OpSelect, state.uint_type, sign_fill, sign_set,
	                           ConstantU32(state, 0xffu), ConstantU32(state, 0u)});

	const auto is_direct       = state.builder.AllocateId();
	const auto before_constant = state.builder.AllocateId();
	const auto is_zero         = state.builder.AllocateId();
	const auto constant_byte   = state.builder.AllocateId();
	const auto non_direct_byte = state.builder.AllocateId();
	const auto ret             = state.builder.AllocateId();
	state.builder.AddFunction({OpULessThan, state.bool_type, is_direct, selector,
	                           ConstantU32(state, 8u)});
	state.builder.AddFunction({OpULessThan, state.bool_type, before_constant, selector,
	                           ConstantU32(state, 12u)});
	state.builder.AddFunction(
	    {OpIEqual, state.bool_type, is_zero, selector, ConstantU32(state, 12u)});
	state.builder.AddFunction({OpSelect, state.uint_type, constant_byte, is_zero,
	                           ConstantU32(state, 0u), ConstantU32(state, 0xffu)});
	state.builder.AddFunction({OpSelect, state.uint_type, non_direct_byte, before_constant,
	                           sign_fill, constant_byte});
	state.builder.AddFunction(
	    {OpSelect, state.uint_type, ret, is_direct, data_byte, non_direct_byte});
	return ret;
}

void EmitPermuteBytesU32(EmitterState& state, const IR::Instruction& inst) {
	const auto source_high = EmitValueLoad(state, inst.src[0]);
	const auto source_low  = EmitValueLoad(state, inst.src[1]);
	const auto selectors   = EmitValueLoad(state, inst.src[2]);
	auto       ret         = ConstantU32(state, 0u);
	for (uint32_t lane = 0; lane < 4u; lane++) {
		const auto selector = state.builder.AllocateId();
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, selector, selectors,
		                           ConstantU32(state, lane * 8u), ConstantU32(state, 8u)});
		auto byte = EmitPermuteByteU32(state, source_high, source_low, selector);
		if (lane != 0u) {
			const auto shifted = state.builder.AllocateId();
			state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, shifted, byte,
			                           ConstantU32(state, lane * 8u)});
			byte = shifted;
		}
		const auto combined = state.builder.AllocateId();
		state.builder.AddFunction({OpBitwiseOr, state.uint_type, combined, ret, byte});
		ret = combined;
	}
	EmitStoreU32(state, inst.dst, ret);
}

void EmitMeanBytesU32(EmitterState& state, const IR::Instruction& inst) {
	const auto source0    = EmitValueLoad(state, inst.src[0]);
	const auto source1    = EmitValueLoad(state, inst.src[1]);
	const auto round_bits = EmitValueLoad(state, inst.src[2]);
	auto       ret        = ConstantU32(state, 0u);
	for (uint32_t lane = 0; lane < 4u; lane++) {
		const auto offset     = ConstantU32(state, lane * 8u);
		const auto source0_u8 = state.builder.AllocateId();
		const auto source1_u8 = state.builder.AllocateId();
		const auto round_u8   = state.builder.AllocateId();
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, source0_u8, source0,
		                           offset, ConstantU32(state, 8u)});
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, source1_u8, source1,
		                           offset, ConstantU32(state, 8u)});
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, round_u8, round_bits,
		                           offset, ConstantU32(state, 8u)});
		const auto round = EmitAndConstant(state, round_u8, 1u);
		const auto sum   = state.builder.AllocateId();
		const auto total = state.builder.AllocateId();
		auto       mean  = state.builder.AllocateId();
		state.builder.AddFunction({OpIAdd, state.uint_type, sum, source0_u8, source1_u8});
		state.builder.AddFunction({OpIAdd, state.uint_type, total, sum, round});
		state.builder.AddFunction({OpShiftRightLogical, state.uint_type, mean, total,
		                           ConstantU32(state, 1u)});
		if (lane != 0u) {
			const auto shifted = state.builder.AllocateId();
			state.builder.AddFunction(
			    {OpShiftLeftLogical, state.uint_type, shifted, mean, offset});
			mean = shifted;
		}
		const auto combined = state.builder.AllocateId();
		state.builder.AddFunction({OpBitwiseOr, state.uint_type, combined, ret, mean});
		ret = combined;
	}
	EmitStoreU32(state, inst.dst, ret);
}

void EmitSadPackedU32(EmitterState& state, const IR::Instruction& inst, uint32_t lane_width,
                      uint32_t result_shift, bool mask_zero_source1) {
	const auto source0 = EmitValueLoad(state, inst.src[0]);
	const auto source1 = EmitValueLoad(state, inst.src[1]);
	const auto accum   = EmitValueLoad(state, inst.src[2]);
	auto       ret     = result_shift == 0u ? accum : ConstantU32(state, 0u);
	for (uint32_t lane = 0; lane < 32u / lane_width; lane++) {
		const auto offset       = ConstantU32(state, lane * lane_width);
		const auto source0_lane = state.builder.AllocateId();
		const auto source1_lane = state.builder.AllocateId();
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, source0_lane, source0,
		                           offset, ConstantU32(state, lane_width)});
		state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, source1_lane, source1,
		                           offset, ConstantU32(state, lane_width)});

		const auto source0_high = state.builder.AllocateId();
		const auto source0_diff = state.builder.AllocateId();
		const auto source1_diff = state.builder.AllocateId();
		const auto difference   = state.builder.AllocateId();
		const auto combined     = state.builder.AllocateId();
		state.builder.AddFunction(
		    {OpUGreaterThan, state.bool_type, source0_high, source0_lane, source1_lane});
		state.builder.AddFunction(
		    {OpISub, state.uint_type, source0_diff, source0_lane, source1_lane});
		state.builder.AddFunction(
		    {OpISub, state.uint_type, source1_diff, source1_lane, source0_lane});
		state.builder.AddFunction({OpSelect, state.uint_type, difference, source0_high,
		                           source0_diff, source1_diff});
		auto contribution = difference;
		if (mask_zero_source1) {
			const auto active = state.builder.AllocateId();
			const auto masked = state.builder.AllocateId();
			state.builder.AddFunction({OpINotEqual, state.bool_type, active, source1_lane,
			                           ConstantU32(state, 0u)});
			state.builder.AddFunction({OpSelect, state.uint_type, masked, active, difference,
			                           ConstantU32(state, 0u)});
			contribution = masked;
		}
		state.builder.AddFunction({OpIAdd, state.uint_type, combined, ret, contribution});
		ret = combined;
	}
	if (result_shift != 0u) {
		const auto shifted  = state.builder.AllocateId();
		const auto combined = state.builder.AllocateId();
		state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, shifted, ret,
		                           ConstantU32(state, result_shift)});
		state.builder.AddFunction({OpIAdd, state.uint_type, combined, accum, shifted});
		ret = combined;
	}
	EmitStoreU32(state, inst.dst, ret);
}

void EmitSadBytesU32(EmitterState& state, const IR::Instruction& inst) {
	EmitSadPackedU32(state, inst, 8u, 0u, false);
}

void EmitSadBytesHighU32(EmitterState& state, const IR::Instruction& inst) {
	EmitSadPackedU32(state, inst, 8u, 16u, false);
}

void EmitMaskedSadBytesU32(EmitterState& state, const IR::Instruction& inst) {
	EmitSadPackedU32(state, inst, 8u, 0u, true);
}

static void EmitQuadSadPackedU16U8Impl(EmitterState& state, const IR::Instruction& inst,
                                      bool mask_zero_template) {
	const uint32_t image[] = {
	    EmitSequentialValueLoad(state, inst.src[0], 0),
	    EmitSequentialValueLoad(state, inst.src[0], 1),
	};
	const auto search_template = EmitValueLoad(state, inst.src[1]);
	const uint32_t accumulator[] = {
	    EmitSequentialValueLoad(state, inst.src[2], 0),
	    EmitSequentialValueLoad(state, inst.src[2], 1),
	};
	uint32_t packed[] = {
	    ConstantU32(state, 0u),
	    ConstantU32(state, 0u),
	...
	for (uint32_t shift = 0; shift < 4u; shift++) {
		auto sad = ConstantU32(state, 0u);
		for (uint32_t lane = 0; lane < 4u; lane++) {
			const uint32_t image_index   = shift + lane;
			const auto     image_byte    = state.builder.AllocateId();
			const auto     template_byte = state.builder.AllocateId();
			state.builder.AddFunction(
			    {OpBitFieldUExtract, state.uint_type, image_byte, image[image_index / 4u],
			     ConstantU32(state, (image_index % 4u) * 8u), ConstantU32(state, 8u)});
			state.builder.AddFunction(
			    {OpBitFieldUExtract, state.uint_type, template_byte, search_template,
			     ConstantU32(state, lane * 8u), ConstantU32(state, 8u)});

			const auto image_high    = state.builder.AllocateId();
			const auto image_diff    = state.builder.AllocateId();
			const auto template_diff = state.builder.AllocateId();
			const auto difference    = state.builder.AllocateId();
			state.builder.AddFunction(
			    {OpUGreaterThan, state.bool_type, image_high, image_byte, template_byte});
			state.builder.AddFunction(
			    {OpISub, state.uint_type, image_diff, image_byte, template_byte});
			state.builder.AddFunction(
			    {OpISub, state.uint_type, template_diff, template_byte, image_byte});
			state.builder.AddFunction({OpSelect, state.uint_type, difference, image_high,
			                           image_diff, template_diff});
			auto contribution = difference;
			if (mask_zero_template) {
				const auto active = state.builder.AllocateId();
				const auto masked = state.builder.AllocateId();
				state.builder.AddFunction({OpINotEqual, state.bool_type, active, template_byte,
				                           ConstantU32(state, 0u)});
				state.builder.AddFunction({OpSelect, state.uint_type, masked, active, difference,
				                           ConstantU32(state, 0u)});
				contribution = masked;
			...
			const auto combined = state.builder.AllocateId();
			state.builder.AddFunction({OpIAdd, state.uint_type, combined, sad, contribution});
			sad = combined;
		}

		const uint32_t part        = shift / 2u;
		const auto     accum_lane  = state.builder.AllocateId();
		const auto     accumulated = state.builder.AllocateId();
		const auto     result      = state.builder.AllocateId();
		state.builder.AddFunction(
		    {OpBitFieldUExtract, state.uint_type, accum_lane, accumulator[part],
		     ConstantU32(state, (shift % 2u) * 16u), ConstantU32(state, 16u)});
		state.builder.AddFunction({OpIAdd, state.uint_type, accumulated, accum_lane, sad});
		state.builder.AddFunction({OpBitwiseAnd, state.uint_type, result, accumulated,
		                           ConstantU32(state, 0xffffu)});
		auto positioned = result;
		if ((shift & 1u) != 0u) {
			positioned = state.builder.AllocateId();
			state.builder.AddFunction({OpShiftLeftLogical, state.uint_type, positioned, result,
			                           ConstantU32(state, 16u)});
		}
		const auto combined = state.builder.AllocateId();
		state.builder.AddFunction(
		    {OpBitwiseOr, state.uint_type, combined, packed[part], positioned});
		packed[part] = combined;
	}
	EmitStoreU32(state, inst.dst, packed[0]);
	EmitStoreU32(state, OffsetRegisterOperand(inst.dst, 1), packed[1]);
}

void EmitQuadSadPackedU16U8(EmitterState& state, const IR::Instruction& inst) {
	EmitQuadSadPackedU16U8Impl(state, inst, false);
}

void EmitMaskedQuadSadPackedU16U8(EmitterState& state, const IR::Instruction& inst) {
	EmitQuadSadPackedU16U8Impl(state, inst, true);
}

void EmitSadWordsU32(EmitterState& state, const IR::Instruction& inst) {
	EmitSadPackedU32(state, inst, 16u, 0u, false);
}
...
void EmitClampPackU16U32(EmitterState& state, const IR::Instruction& inst) {
	const auto src0    = EmitValueLoad(state, inst.src[0]);
	const auto src1    = EmitValueLoad(state, inst.src[1]);
	const auto lo      = EmitMinMaxU32Value(state, src0, ConstantU32(state, 0xffffu), false);
	const auto hi      = EmitMinMaxU32Value(state, src1, ConstantU32(state, 0xffffu), false);
	const auto shifted = EmitShiftLeftConstant(state, hi, 16);
	const auto ret     = state.builder.AllocateId();
	state.builder.AddFunction({OpBitwiseOr, state.uint_type, ret, lo, shifted});
	EmitStoreU32(state, inst.dst, ret);
...
void EmitSaturatePackU8I16(EmitterState& state, const IR::Instruction& inst) {
	const auto src        = EmitValueLoad(state, inst.src[0]);
	const auto signed_src = state.builder.AllocateId();
	state.builder.AddFunction({OpBitcast, state.int_type, signed_src, src});
	...
	for (uint32_t lane = 0; lane < 2u; lane++) {
		const auto extracted_i32 = state.builder.AllocateId();
		const auto extracted_u32 = state.builder.AllocateId();
		state.builder.AddFunction({OpBitFieldSExtract, state.int_type, extracted_i32, signed_src,
		                           ConstantU32(state, lane * 16u), ConstantU32(state, 16)});
		state.builder.AddFunction({OpBitcast, state.uint_type, extracted_u32, extracted_i32});
		const auto positive    = state.builder.AllocateId();
		const auto nonnegative = state.builder.AllocateId();
		const auto below_max   = state.builder.AllocateId();
		lanes[lane]            = state.builder.AllocateId();
		state.builder.AddFunction({OpSGreaterThan, state.bool_type, positive, extracted_u32,
		                           ConstantU32(state, 0)});
		state.builder.AddFunction({OpSelect, state.uint_type, nonnegative, positive, extracted_u32,
		                           ConstantU32(state, 0)});
		state.builder.AddFunction({OpSLessThan, state.bool_type, below_max, nonnegative,
		                           ConstantU32(state, 255)});
		state.builder.AddFunction({OpSelect, state.uint_type, lanes[lane], below_max, nonnegative,
		                           ConstantU32(state, 255)});
	}
	const auto shifted = EmitShiftLeftConstant(state, lanes[1], 8);
	const auto ret     = EmitOrU32(state, lanes[0], shifted);
	EmitStoreU32(state, inst.dst, ret);
}
...
void EmitPackU8F32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpFOrdGreaterThanEqual, state.bool_type, above_max, src, ConstantF32(state, 0x437f0000u)});
...
void EmitPackNormalizedF32(EmitterState& state, const IR::Instruction& inst, uint32_t ext_inst) {
	...
	state.builder.AddFunction({OpExtInst, state.uint_type, ret, state.glsl_std450, ext_inst, pair});
	EmitStoreU32(state, inst.dst, ret);
}
...
void EmitBitCompareB64(EmitterState& state, const IR::Instruction& inst, bool bit_set) {
	const auto low          = EmitSequentialValueLoad(state, inst.src[0], 0);
	const auto high         = EmitSequentialValueLoad(state, inst.src[0], 1);
	const auto index        = EmitAndConstant(state, EmitValueLoad(state, inst.src[1]), 63u);
	const auto dword_index  = EmitAndConstant(state, index, 31u);
	...
	const auto selected     = state.builder.AllocateId();
	const auto bit          = state.builder.AllocateId();
	const auto selected_bit = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpULessThan, state.bool_type, select_low, index, ConstantU32(state, 32)});
	state.builder.AddFunction({OpSelect, state.uint_type, selected, select_low, low, high});
	state.builder.AddFunction({OpBitFieldUExtract, state.uint_type, bit, selected, dword_index,
	                           ConstantU32(state, 1)});
	state.builder.AddFunction(
	    {OpIEqual, state.bool_type, selected_bit, bit, ConstantU32(state, bit_set ? 1u : 0u)});
	EmitCompareResult(state, inst.dst, selected_bit);
}
...
void EmitFindLsbU64(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpINotEqual, state.bool_type, high_non_zero, high, ConstantU32(state, 0)});
	state.builder.AddFunction({OpSelect, state.uint_type, high_result, high_non_zero, high_total,
	                           ConstantU32(state, 0xffffffffu)});
	state.builder.AddFunction(
...
void EmitConvertF32ToU32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpFOrdLessThanEqual, state.bool_type, below_zero, src, ConstantF32(state, 0)});
	state.builder.AddFunction(
	    {OpFOrdGreaterThanEqual, state.bool_type, above_max, src, ConstantF32(state, 0x4f800000u)});
	const auto zero_or_nan = EmitLogicalOrBool(state, EmitClassifyF32(state, src).nan, below_zero);
...
void EmitConvertF32ToI32(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpFOrdGreaterThanEqual, state.bool_type, above_max, src, ConstantF32(state, 0x4f000000u)});
	const auto high_sat =
...
void EmitConvertF16ToU16(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpFOrdGreaterThanEqual, state.bool_type, above_max, f32, ConstantF32(state, 0x477fff00u)});
...
void EmitConvertF16ToI16(EmitterState& state, const IR::Instruction& inst) {
	...
	state.builder.AddFunction(
	    {OpFOrdGreaterThanEqual, state.bool_type, above_max, f32, ConstantF32(state, 0x46fffe00u)});
	...
	state.builder.AddFunction({OpSelect, state.float_type, nan_safe,
	                           EmitClassifyF32(state, f32).nan, ConstantF32(state, 0), safe_value});
...
void EmitFloatExtInst(EmitterState& state, const IR::Instruction& inst, uint32_t ext_inst,
	...
	const auto f32 = state.builder.AllocateId();
	const auto u32 = state.builder.AllocateId();
	state.builder.AddFunction({OpExtInst, state.float_type, f32, state.glsl_std450, ext_inst, src});
	const auto result = ApplyResultModifiersF32(state, f32, inst.dst);
	state.builder.AddFunction({OpBitcast, state.uint_type, u32, result});
	EmitStoreU32(state, inst.dst, u32);
}
...
uint32_t EmitTrigF16Bits(EmitterState& state, uint32_t bits, uint32_t ext_inst) {
	auto src = EmitF16BitsToF32(state, bits);
	src      = EmitTrigCycleF32(state, src, ext_inst == GlslSin);
	const auto scaled = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpFMul, state.float_type, scaled, src, ConstantF32(state, 0x40c90fdbu)});
	const auto result = state.builder.AllocateId();
	state.builder.AddFunction(
	    {OpExtInst, state.float_type, result, state.glsl_std450, ext_inst, scaled});
	return EmitPackLowF32ToF16Bits(state, result);
}
...
void EmitSpecialF16(EmitterState& state, const IR::Instruction& inst) {
	...
	switch (inst.op) {
		...
		case IR::Opcode::RoundEvenF16:
			...
			break;
		case IR::Opcode::FractF16: ret = EmitF16MathExtInstBits(state, bits, GlslFract); break;
		case IR::Opcode::SinF16: ret = EmitTrigF16Bits(state, bits, GlslSin); break;
		case IR::Opcode::CosF16: ret = EmitTrigF16Bits(state, bits, GlslCos); break;
		default: break;
	}
	EmitStoreU32(state, inst.dst, ret);
}
