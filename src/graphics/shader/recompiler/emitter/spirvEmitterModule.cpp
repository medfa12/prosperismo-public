...
uint32_t DescriptorCount(const EmitterState& state, IR::DescriptorBindingKind kind) {
	...
	return binding != nullptr ? IR::DescriptorCount(state.program, *binding) : 0;
}
...
static bool MrtUsesSintOutput(const EmitterState& state, uint32_t index) {
	return state.stage == ShaderType::Pixel && state.pixel_input_info != nullptr &&
	       index < std::size(state.pixel_input_info->target_output_mode) &&
	       state.pixel_input_info->target_output_mode[index] == 8u;
}
...
uint32_t BuiltInForInput(IR::StageInputKind kind) {
	switch (kind) {
		...
		case IR::StageInputKind::SampleMask: return BuiltInSampleMask;
	...
	for (const auto& input: state.inputs) {
		...
		if (input.kind == IR::StageInputKind::Parameter) {
			...
			if (state.stage == ShaderType::Pixel && state.pixel_input_info != nullptr &&
			    PixelParameterUsesNoPerspective(
...
void AddOutputAnnotationsAndNames(EmitterState& state) {
	...
	for (const auto& binding: state.outputs) {
		if (binding.kind == IR::StageOutputKind::Parameter ||
			...
			if (binding.kind == IR::StageOutputKind::Mrt && state.stage == ShaderType::Pixel &&
			    state.pixel_input_info != nullptr &&
			    state.pixel_input_info->ps_dual_source_export_enable && binding.index < 2u) {
				const auto output = ShaderResolveMrtOutputBinding(true, binding.index);
				state.builder.AddAnnotation(
				    {OpDecorate, binding.variable_id, DecorationLocation, output.location});
				state.builder.AddAnnotation(
				    {OpDecorate, binding.variable_id, DecorationIndex, output.index});
				continue;
			}
			state.builder.AddAnnotation(
...
void EmitHeaderAndTypes(EmitterState& state) {
	...
	state.ptr_input_sample_mask_array  = state.builder.AllocateId();
	...
	const auto ptr_output_vec4_int     = state.builder.AllocateId();
	...
	if (state.stage == ShaderType::Pixel) {
		...
		if (state.pixel_input_info != nullptr && state.pixel_input_info->ps_early_z &&
		    ...
		    !state.pixel_input_info->HasAaSampleMask() &&
		    !state.pixel_input_info->HasPsShaderSampleExclusionMask() &&
		    !state.pixel_input_info->HasAlphaToMaskDither()) {
	...
	const bool has_input_sample_mask =
	    std::any_of(state.inputs.begin(), state.inputs.end(), [](const auto& input) {
		    return input.kind == IR::StageInputKind::SampleMask;
	    });
	if (has_input_sample_mask || state.sample_mask_variable != 0) {
		state.builder.AddType(
		    {OpTypeArray, state.sample_mask_array_type, state.int_type, ConstantU32(state, 1)});
		state.builder.AddType({OpTypePointer, state.ptr_input_sample_mask_array,
		                       StorageClassInput, state.sample_mask_array_type});
	...
	for (const auto& input: state.inputs) {
		...
		switch (input.kind) {
			...
			case IR::StageInputKind::SampleMask: ptr_type = state.ptr_input_sample_mask_array; break;
	...
	state.builder.AddType(
	    {OpTypePointer, ptr_output_vec4_int, StorageClassOutput, state.vec4_int_type});
	state.builder.AddType(
	...
	for (const auto& binding: state.outputs) {
		if (binding.kind == IR::StageOutputKind::Parameter ||
			...
			const auto pointer_type =
			    binding.kind != IR::StageOutputKind::Mrt  ? state.ptr_output_vec4_float
			    : MrtUsesUintOutput(state, binding.index) ? ptr_output_vec4_uint
			    : MrtUsesSintOutput(state, binding.index) ? ptr_output_vec4_int
