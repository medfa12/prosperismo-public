...
// Compute LDS is shared workgroup storage and can alias DS atomics, so ordinary accesses remain
// in the SPIR-V atomic domain. Graphics-stage LDS is emulated by a function-local scratch array;
// Vulkan does not permit atomics on Function storage and no other invocation can alias it.
uint32_t EmitLdsLoadU32(EmitterState& state, uint32_t pointer) {
...
void EmitLdsStoreU32(EmitterState& state, uint32_t pointer, uint32_t value) {
	if (state.needs_function_lds) {
...
uint32_t EmitMemoryLoadDwordValueU32(EmitterState& state, const IR::Instruction& inst,
	...
	if (mem.kind == IR::ResourceKind::Lds) {
		return EmitLdsLoadU32(state, EmitLdsElementPointer(state, index));
	}
	if (mem.kind == IR::ResourceKind::Gds) {
...
uint32_t EmitMemoryLoadSubDwordValueU32(EmitterState& state, const IR::Instruction& inst,
	...
	auto LoadFromPointer = [&](uint32_t pointer, bool atomic) {
		...
		if (atomic) {
			word = EmitLdsLoadU32(state, pointer);
		} else {
...
void EmitMemoryStoreU32(EmitterState& state, const IR::Instruction& inst, IR::ResourceKind kind,
	...
	EmitIfCondition(state, in_bounds, [&]() {
		...
		if (store_inst.memory.kind == IR::ResourceKind::Lds) {
			EmitLdsStoreU32(state, pointer, value);
		} else {
			state.builder.AddFunction({OpStore, pointer, value});
		}
	});
...
void EmitDsReadAddtidB32(EmitterState& state, const IR::Instruction& inst) {
	...
	const auto pointer = EmitLdsElementPointer(state, index);
	const auto value   = EmitLdsLoadU32(state, pointer);
	EmitStoreU32(state, inst.dst, value);
}
