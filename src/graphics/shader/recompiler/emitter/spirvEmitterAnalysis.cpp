...
void CollectRelativeVectorRegisters(const IR::Program&            program,
	...
	bool has_relative_vector_access = false;
	for (const auto& block: program.blocks) {
		for (const auto& inst: block.instructions) {
			if (inst.op == IR::Opcode::MoveRelSourceU32 || inst.op == IR::Opcode::MoveRelDestU32 ||
			    inst.op == IR::Opcode::SwapRelU32) {
				has_relative_vector_access = true;
				break;
			}
		}
		if (has_relative_vector_access) {
			break;
		}
	}
	if (!has_relative_vector_access) {
	...
	for (uint32_t reg = 0; reg < program.allocated_vgpr_count; reg++) {
		CollectRegister(registers, {IR::RegisterFile::Vector, reg});
	}
}
...
uint32_t PairDwordSourceCount(IR::Opcode op, uint32_t src_count) {
	switch (op) {
		...
		case IR::Opcode::BitFieldExtractU64:
		case IR::Opcode::BitFieldExtractI64: return 1;
		case IR::Opcode::BitClearU64: return 1;
...
void CollectRegisters(const IR::Program& program, std::vector<RegisterBinding>& registers) {
	...
	for (const auto& block: program.blocks) {
		for (const auto& inst: block.instructions) {
			...
			if (inst.op == IR::Opcode::BitCountZeroU64 || inst.op == IR::Opcode::BitCountU64 ||
			    inst.op == IR::Opcode::FindLsbZeroU64 || inst.op == IR::Opcode::FindLsbU64 ||
			    inst.op == IR::Opcode::FindMsbFromHighU64 ||
			    inst.op == IR::Opcode::FindMsbSignI64) {
				CollectSequentialRegisters(registers, inst.src[0], 2);
			}
			if (inst.op == IR::Opcode::CompareEqU64 || inst.op == IR::Opcode::CompareGtU64 ||
			    inst.op == IR::Opcode::CompareNeU64) {
				CollectSequentialRegisters(registers, inst.src[0], 2);
				CollectSequentialRegisters(registers, inst.src[1], 2);
			}
			if (inst.op == IR::Opcode::BitCompare0B64) {
				CollectSequentialRegisters(registers, inst.src[0], 2);
			}
			...
			if (inst.op == IR::Opcode::QuadSadPackedU16U8 ||
			    inst.op == IR::Opcode::MaskedQuadSadPackedU16U8) {
				CollectSequentialRegisters(registers, inst.dst, 2);
				CollectSequentialRegisters(registers, inst.src[0], 2);
				CollectSequentialRegisters(registers, inst.src[2], 2);
			}
			...
			if ((inst.op == IR::Opcode::SLoadDword || inst.op == IR::Opcode::SBufferLoadDword) &&
	...
	CollectRelativeVectorRegisters(program, registers);
}
...
bool ProgramNeedsSubgroupBallot(const IR::Program& program) {
	if (program.lane_mask_mode == ShaderLaneMaskMode::PerInvocation) {
		for (const auto& block: program.blocks) {
			for (const auto& inst: block.instructions) {
				if (inst.op == IR::Opcode::WqmB32 || inst.op == IR::Opcode::WqmB64 ||
				    ((inst.op == IR::Opcode::QuadMaskB32 || inst.op == IR::Opcode::QuadMaskB64) &&
...
bool ProgramNeedsSubgroupLocalInvocationId(const IR::Program& program) {
	for (const auto& block: program.blocks) {
		for (const auto& inst: block.instructions) {
			if (program.lane_mask_mode == ShaderLaneMaskMode::PerInvocation &&
			    (inst.op == IR::Opcode::BitClearU64 || inst.op == IR::Opcode::BitSetU64) &&
			...
			if (inst.op == IR::Opcode::WqmB32 || inst.op == IR::Opcode::WqmB64 ||
			    inst.op == IR::Opcode::DsSwizzleB32 || inst.op == IR::Opcode::DsConsume ||
			    inst.op == IR::Opcode::DsAppend || inst.op == IR::Opcode::WriteLaneU32 ||
			    inst.op == IR::Opcode::Permlane16B32 || inst.op == IR::Opcode::Permlanex16B32 ||
			    inst.op == IR::Opcode::Export || InstructionHasDppSource(inst)) {
				return true;
			}
...
DescriptorResourceBinding ResourceForDescriptor(const EmitterState&       state,
	...
	if (!IR::DescriptorArrayIndex(state.program, *descriptor, resource, array_index)) {
		ExitDescriptorBindingFailure(state, kind, resource,
		                             "resource is absent from descriptor group");
	}
	return {descriptor, array_index};
}
