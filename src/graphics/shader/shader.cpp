...
// compiler aborts the process outright on some unoptimized modules. The Kyty launcher defaults to
// Performance, while an explicit None still has to mean no optimization; the call site retains the
// measured large-module threshold to avoid regressing ordinary shader compilation.
...
static uint64_t SpirvHash(const std::vector<uint32_t>& spirv,
	                      Config::ShaderOptimizationType optimization_type) {
	uint64_t h = 1469598103934665603ULL; // FNV-1a
	for (const auto w: spirv) {
		h ^= w;
		h *= 1099511628211ULL;
	}
	// The same source module optimized for size and performance must never share a cache entry.
	h ^= static_cast<uint64_t>(optimization_type);
	h *= 1099511628211ULL;
	return h;
}
...
static bool SpirvOptimizeBinary(const char* label, uint64_t shader_hash,
	                            Config::ShaderOptimizationType optimization_type,
	                            std::vector<uint32_t>& spirv) {
	if (optimization_type == Config::ShaderOptimizationType::None) {
		return true;
	}

	const auto cache_key = SpirvHash(spirv, optimization_type);
...
static void ps_check(const HW::PsStageRegisters& ps, const HW::ShaderRegisters& sh) {
	for (const auto mode: sh.target_output_mode) {
		EXIT_NOT_IMPLEMENTED(!ShaderIsKnownTargetOutputMode(mode));
	}
...
static void ShaderGetStaticInputInfoPS(
    ...
    ShaderPixelInputInfo& ps_info, bool global_perspective_disable, uint64_t aa_sample_mask,
    uint32_t ps_shader_sample_exclusion_mask, uint32_t alpha_to_mask_control,
    uint8_t alpha_to_mask_num_samples, bool alpha_to_mask_round_mode) {
	KYTY_PROFILER_FUNCTION();

	ps_info                                 = {};
	ps_info.aa_sample_mask                  = aa_sample_mask;
	ps_info.ps_shader_sample_exclusion_mask = ps_shader_sample_exclusion_mask;
	const auto alpha_to_mask     = HW::AlphaToMaskControl::FromRegister(alpha_to_mask_control);
	ps_info.alpha_to_mask_enable = alpha_to_mask.enable;
	ps_info.alpha_to_mask_dither_enable          = alpha_to_mask.dither_enable;
	ps_info.alpha_to_mask_disable                = sh.db_shader_control.alpha_to_mask_disable;
	ps_info.alpha_to_mask_top_left_threshold     = alpha_to_mask.top_left_threshold;
	ps_info.alpha_to_mask_top_right_threshold    = alpha_to_mask.top_right_threshold;
	ps_info.alpha_to_mask_bottom_left_threshold  = alpha_to_mask.bottom_left_threshold;
	ps_info.alpha_to_mask_bottom_right_threshold = alpha_to_mask.bottom_right_threshold;
	ps_info.alpha_to_mask_num_samples =
	    static_cast<uint8_t>(HW::DecodeAlphaToMaskSampleCount(alpha_to_mask_num_samples));
	ps_info.alpha_to_mask_round_mode = alpha_to_mask_round_mode ? 0u : 4u;
	...
	ps_info.ps_no_perspective    = ResolvePixelNoPerspective(
	    (sh.ps_input_ena & sh.ps_input_addr & 0x00000020u) != 0, global_perspective_disable);
	...
	ps_info.ps_dual_source_export_enable = sh.db_shader_control.shader_dual_export_enable;
	ps_info.ps_early_z =
	...
	if (ps_info.ps_dual_source_export_enable && ps_info.target_output_mode[1] != 0) {
		// The second export is another blend input for MRT0, not an independent render target.
		// Apply the physical MRT0 component order to both blend sources.
		ps_info.target_export_mapping[1] = ps_info.target_export_mapping[0];
	}
...
static std::string ShaderDescribeSpecialization(const ShaderRecompiler::IR::Program& program) {
	...
	for (uint32_t i = 0; i < program.info.images.size(); i++) {
		...
		ret += fmt::format(
		    " i{}[kind={} dim={} mips={} swizzle=0x{:03x}]", i, static_cast<uint32_t>(image.kind),
		    static_cast<uint32_t>(image.dimension),
		    ShaderRecompiler::IR::StorageMipCount(image.mip_mode), image.storage_swizzle);
	}
...
bool ShaderCompileInfoPS(const HW::PixelShaderInfo& regs, const HW::ShaderRegisters& sh,
                         ...
                         bool global_perspective_disable, uint64_t aa_sample_mask,
                         uint32_t ps_shader_sample_exclusion_mask, uint32_t alpha_to_mask_control,
                         uint8_t alpha_to_mask_num_samples, bool alpha_to_mask_round_mode) {
	spirv = {};

	ShaderGetStaticInputInfoPS(regs, sh, vs_info, target_export_mapping, ps_info,
	                           global_perspective_disable, aa_sample_mask,
	                           ps_shader_sample_exclusion_mask, alpha_to_mask_control,
	                           alpha_to_mask_num_samples, alpha_to_mask_round_mode);
...
void ShaderDbgDumpInputInfo(const ShaderPixelInputInfo& info) {
	...
	LOGF("\t input_num            = %u\n"
	     ...
	     info.ps_sample_shading ? "true" : "false",
	     info.ps_dual_source_export_enable ? "true" : "false",
...
static bool ShaderRecompilerTextDumpEnabled() {
	// Graphics debug dump is a separate binary-dump option. Shader logging controls only the
	// detailed decoded/IR text routed below.
	return Config::GetShaderLogDirection() != Config::ShaderLogDirection::Silent;
}

static void EmitShaderRecompilerTextLog(const char* type, const char* label, uint64_t shader_hash,
	                                    const ShaderRecompiler::CompileResult& result,
	                                    size_t spirv_words, uint32_t wave_size = 0) {
	const auto direction = Config::GetShaderLogDirection();
	if (direction == Config::ShaderLogDirection::Silent) {
		return;
	}

	const auto summary =
	    wave_size == 0
	        ? fmt::format("{} SPIR-V words={}\n", label, spirv_words)
	        : fmt::format("{} SPIR-V words={} wave_size={}\n", label, spirv_words, wave_size);

	if (direction == Config::ShaderLogDirection::Console) {
	...
	const auto folder = Config::GetShaderLogFolder();
	Common::File::CreateDirectories(folder);
	const auto path =
	    folder /
	    fmt::format("{:04d}_shader_{}_{:016x}.log", id.fetch_add(1), type, shader_hash);

	Common::File file;
	file.Create(path);
	if (file.IsInvalid()) {
		const auto path_text = Common::PathToString(path);
		LOGF_COLOR(Log::Color::BrightRed, "Can't create shader log file: %s\n", path_text.c_str());
		return;
	}
	file.Printf("%s decoded RDNA2:\n%s\n%s IR:\n%s\n%s", label, result.decoded_dump.c_str(), label,
	            result.ir_dump.c_str(), summary.c_str());
	file.Close();
}
...
bool ShaderCompileSpirvVS(const HW::VertexShaderInfo& regs, const HW::ShaderRegisters& sh,
	...
	options.allocated_vgpr_count = ShaderDecodeAllocatedVgprCount(regs.gs_regs.rsrc1.vgprs, 64u);
	...
	EmitShaderRecompilerTextLog("vs", "ShaderRecompiler VS", options.shader_hash, result,
	                            spirv.size());
	return true;
}
...
bool ShaderCompileSpirvPS(const HW::PixelShaderInfo& regs, const HW::ShaderRegisters& sh,
	...
	options.lane_mask_mode       = lane_mask_mode;
	options.allocated_vgpr_count = ShaderDecodeAllocatedVgprCount(regs.ps_regs.rsrc1.vgprs, 64u);
	...
	EmitShaderRecompilerTextLog("ps", "ShaderRecompiler PS", options.shader_hash, result,
	                            spirv.size());
	return true;
}
...
bool ShaderCompileSpirvCS(const HW::ComputeShaderInfo& regs, const HW::ShaderRegisters& sh,
	...
	options.allocated_vgpr_count =
	    ShaderDecodeAllocatedVgprCount(regs.cs_regs.vgprs, input_info.wave_size);
	...
	if (!ShaderRecompiler::TryRecompile(code, options, result, &error)) {
		...
		LOGF_COLOR(Log::Color::BrightRed,
		           "ShaderRecompiler CS unsupported, dispatch skipped "
	...
	if (result.spirv.size() >= SPIRV_WARN_WORDS) {
		...
		LOGF_COLOR(Log::Color::BrightYellow,
		           ...
		           guest_insts == 0
		               ? 0.0
		               : static_cast<double>(spirv_insts) / static_cast<double>(guest_insts),
	...
	if (result.spirv.size() >= SPIRV_WARN_WORDS) {
		SpirvOptimizeBinary("ShaderRecompiler CS", options.shader_hash,
		                    Config::GetShaderOptimizationType(), result.spirv);
	}
	...
	if (std::find(skip_hashes.begin(), skip_hashes.end(), options.shader_hash) !=
	    skip_hashes.end()) {
		LOGF_COLOR(Log::Color::BrightRed, "ShaderRecompiler CS skipped by hash 0x%016" PRIx64 "\n",
		           options.shader_hash);
		return false;
	}
	...
	EmitShaderRecompilerTextLog("cs", "ShaderRecompiler CS", options.shader_hash, result,
	                            spirv.size(), options.wave_size);
	return true;
}
...
bool ShaderCompileGeometryReplayCS(std::span<const uint32_t>        merged_code,
	...
	ShaderId program_id;
	program_id.ids = {replay.vertex_slots,
	                  replay.primitive_slots,
	                  replay.lds_dword_count,
	                  replay.threads_num,
	                  regs.gs_regs.rsrc1.vgprs,
	                  static_cast<uint32_t>(replay.es_addr),
	...
	options.allocated_vgpr_count =
	    ShaderDecodeAllocatedVgprCount(regs.gs_regs.rsrc1.vgprs, info.wave_size);
	...
	EmitShaderRecompilerTextLog("gereplaycs", "ShaderRecompiler GeReplayCS", options.shader_hash,
	                            result, result.spirv.size(), options.wave_size);
...
bool ShaderCompileGeometryReplayVS(const ShaderGeometryReplayCompileInfo& replay,
	...
	const auto key =
	    MakeShaderStageProgramKey(ShaderType::Vertex, synthetic_hash, program_id, lane_mask_mode);
	...
	EmitShaderRecompilerTextLog("gereplayvs", "ShaderRecompiler GeReplayVS", options.shader_hash,
	                            result, result.spirv.size());
	input_info.stage.program =
	    std::make_shared<const ShaderRecompiler::IR::Program>(std::move(result.program));
	input_info.stage.resources =
	    std::make_shared<const ShaderRecompiler::IR::ResourceSnapshot>(std::move(result.resources));
...
ShaderId ShaderGetIdVS(const HW::VertexShaderInfo& regs, const ShaderVertexInputInfo& input_info,
	...
	ret.ids.push_back(regs.gs_regs.rsrc1.vgprs);
...
ShaderId ShaderGetIdPS(const HW::PixelShaderInfo& regs, const ShaderPixelInputInfo& input_info,
	...
	ret.ids.push_back(regs.ps_regs.rsrc1.vgprs);
	ret.ids.push_back(input_info.descriptor_set);
	ret.ids.push_back(input_info.push_constant_offset);
	ret.ids.push_back(static_cast<uint32_t>(input_info.aa_sample_mask));
	ret.ids.push_back(static_cast<uint32_t>(input_info.aa_sample_mask >> 32u));
	ret.ids.push_back(input_info.ps_shader_sample_exclusion_mask);
	ret.ids.push_back(static_cast<uint32_t>(input_info.alpha_to_mask_enable));
	ret.ids.push_back(static_cast<uint32_t>(input_info.alpha_to_mask_dither_enable));
	ret.ids.push_back(static_cast<uint32_t>(input_info.alpha_to_mask_disable));
	ret.ids.push_back(input_info.alpha_to_mask_top_left_threshold);
	ret.ids.push_back(input_info.alpha_to_mask_top_right_threshold);
	ret.ids.push_back(input_info.alpha_to_mask_bottom_left_threshold);
	ret.ids.push_back(input_info.alpha_to_mask_bottom_right_threshold);
	ret.ids.push_back(input_info.alpha_to_mask_num_samples);
	ret.ids.push_back(input_info.alpha_to_mask_round_mode);
	...
	ret.ids.push_back(PixelNoPerspectiveCacheIdentity(input_info.ps_no_perspective));
	...
	ret.ids.push_back(static_cast<uint32_t>(input_info.ps_dual_source_export_enable));
...
ShaderId ShaderGetIdCS(const HW::ComputeShaderInfo& regs, const ShaderComputeInputInfo& input_info,
	...
	ret.ids.push_back(regs.cs_regs.vgprs);
