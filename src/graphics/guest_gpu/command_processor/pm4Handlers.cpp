...
static void HwCtxSetAaAndPsSampleMaskRegister(CommandProcessor& cp, uint32_t cmd_offset,
                                              uint32_t value) {
	auto exclusion_mask = cp.GetCtx().GetPsShaderSampleExclusionMask();
	if (UpdatePsShaderSampleExclusionMask(exclusion_mask, cmd_offset, value)) {
		cp.GetCtx().SetPsShaderSampleExclusionMask(exclusion_mask);
		return;
	}
	auto mask = cp.GetCtx().GetAaSampleMask();
	if (UpdateAaSampleMaskRegister(mask, cmd_offset, value)) {
		cp.GetCtx().SetAaSampleMask(mask);
	}
}
...
static void HwCtxSetPsShaderRateRegister(CommandProcessor& cp, uint32_t cmd_offset,
                                         uint32_t value) {
	if (cmd_offset == Pm4::PA_SC_MODE_CNTL_1) {
		cp.GetCtx().SetPsShaderRateControl(HW::PsShaderRateControl::FromRegister(value));
	}
}
...
static void HwCtxSetPaScExtendedControlRegister(CommandProcessor& cp, uint32_t cmd_offset,
                                                uint32_t value) {
	if (cmd_offset == Pm4::PA_SC_CONSERVATIVE_RASTERIZATION_CNTL) {
		cp.GetCtx().SetConservativeRasterizationControl(
		    HW::ConservativeRasterizationControl::FromRegister(value));
		return;
	}
...
KYTY_HW_CTX_PARSER(HwCtxSetPaScExtendedControl) {
	...
	for (uint32_t i = 0; i < num_values; i++) {
		HwCtxSetPaScExtendedControlRegister(cp, cmd_offset + i, buffer[i]);
	}
...
KYTY_HW_CTX_PARSER(HwCtxSetAlphaToMask) {
	...
	for (uint32_t i = 0; i < num_values; i++) {
		if (cmd_offset + i == Pm4::DB_ALPHA_TO_MASK) {
			cp.GetCtx().SetAlphaToMaskControl(HW::AlphaToMaskControl::FromRegister(buffer[i]));
		}
	}
...
KYTY_HW_CTX_PARSER(HwCtxSetScanModeControl1) {
	...
	for (uint32_t i = 0; i < num_values; i++) {
		HwCtxSetPsShaderRateRegister(cp, cmd_offset + i, buffer[i]);
	}
...
KYTY_HW_CTX_PARSER(HwCtxSetAaMask) {
	...
	for (uint32_t i = 0; i < num_values; i++) {
		HwCtxSetAaAndPsSampleMaskRegister(cp, cmd_offset + i, buffer[i]);
	}
...
KYTY_HW_CTX_PARSER(HwCtxSetLineControl) {
	...
	cp.GetCtx().SetLineWidth(HW::DecodeLineWidthEighths(line_width));
...
KYTY_HW_CTX_PARSER(HwCtxSetPolyOffsetRegisters) {
	...
	for (uint32_t i = 0; i < num_values; i++) {
		EXIT_NOT_IMPLEMENTED(!cp.GetCtx().SetPolygonOffsetRegister(cmd_offset + i, buffer[i]));
	}
...
static void HwSetPrimitiveResetControl(CommandProcessor& cp, uint32_t value) {
	EXIT_NOT_IMPLEMENTED((value & ~0x3u) != 0u);
	cp.GetUcfg().SetPrimitiveResetControl(HW::PrimitiveResetControl::FromRegister(value));
}

KYTY_HW_UC_PARSER(HwUcSetMultiPrimIbReset) {
	EXIT_NOT_IMPLEMENTED(cmd_offset != Pm4::GE_MULTI_PRIM_IB_RESET_EN);
	auto num_values = KYTY_PM4_LEN(cmd_id) - 2u;
	EXIT_NOT_IMPLEMENTED(num_values != 1u);
	HwSetPrimitiveResetControl(cp, buffer[0]);
	return num_values;
}

KYTY_HW_CTX_PARSER(HwCtxSetPrimitiveResetIndex) {
	EXIT_NOT_IMPLEMENTED(cmd_offset != Pm4::VGT_MULTI_PRIM_IB_RESET_INDX);
	cp.GetCtx().SetPrimitiveResetIndex(buffer[0]);
	return 1;
}
...
KYTY_CP_OP_PARSER(CpOpAcquireMem) {
	...
	if (cmd_id == 0xc0055800u) {
		return 6;
	}

	const auto gl2_action = ResolveAcquireMemGl2Action(cmd_id, buffer[6]);
	EXIT_NOT_IMPLEMENTED(gl2_action == AcquireMemGl2Action::Unsupported);
	if (gl2_action == AcquireMemGl2Action::GlobalBarrier ||
	    AcquireMemCbDbRequiresGlobalBarrier(buffer[0]) ||
	    AcquireMemShaderDataRequiresGlobalBarrier(buffer[6])) {
...
KYTY_CP_OP_PARSER(CpOpRewind) {
	...
	cp.Rewind(*reinterpret_cast<const volatile uint32_t*>(buffer));
...
KYTY_CP_OP_PARSER(CpOpBranch) {
	...
	if (take_then) {
		cp.ProcessIndirectBuffer(then_buffer, then_num_dw,
		                         ResolveConditionalBranchTargetAction());
...
KYTY_CP_OP_PARSER(CpOpCondWrite) {
	KYTY_PROFILER_FUNCTION();

	EXIT_NOT_IMPLEMENTED(cmd_id != KYTY_PM4(9u, Pm4::IT_COND_WRITE, Pm4::R_ZERO));

	constexpr uint32_t ControlMask = 0x00000317u;
	const uint32_t     control     = buffer[0];
	const uint32_t     compare     = control & 0x7u;
	const uint32_t     write_space = (control >> 8u) & 0x3u;
	EXIT_NOT_IMPLEMENTED((control & ~ControlMask) != 0 || (control & 0x10u) == 0);
	EXIT_NOT_IMPLEMENTED(write_space != 1u);

	const auto read_address =
	    buffer[1] | (static_cast<uint64_t>(buffer[2]) << 32u);
	const auto write_address =
	    buffer[5] | (static_cast<uint64_t>(buffer[6]) << 32u);
	EXIT_NOT_IMPLEMENTED(read_address == 0 || write_address == 0);
	EXIT_NOT_IMPLEMENTED((read_address & 0x3u) != 0 || (write_address & 0x3u) != 0);

	const auto read_value = *reinterpret_cast<const volatile uint32_t*>(read_address);
	auto& destination     = *reinterpret_cast<volatile uint32_t*>(write_address);
	const auto result =
	    ExecuteCondWriteGl2(compare, read_value, buffer[3], buffer[4], buffer[7], destination);
	EXIT_NOT_IMPLEMENTED(result == CondWriteResult::Unsupported);

	return 8;
}

KYTY_CP_OP_PARSER(CpOpAtomicMem) {
	KYTY_PROFILER_FUNCTION();

	AtomicMemPacket packet {};
	EXIT_NOT_IMPLEMENTED(!DecodeAtomicMemPacket(cmd_id, buffer, packet));
	EXIT_NOT_IMPLEMENTED(packet.address == 0);

	switch (packet.operation) {
		case AtomicMemOperation::CompareSwap32:
		case AtomicMemOperation::Add32:
		case AtomicMemOperation::Sub32: {
			EXIT_NOT_IMPLEMENTED((packet.address & 0x3u) != 0);
			auto& destination = *reinterpret_cast<uint32_t*>(packet.address);
			EXIT_NOT_IMPLEMENTED(ExecuteAtomicMemPacket(packet, destination) !=
			                     AtomicMemResult::Completed);
			break;
		}
		case AtomicMemOperation::CompareSwap64:
		case AtomicMemOperation::Add64:
		case AtomicMemOperation::Sub64: {
			EXIT_NOT_IMPLEMENTED((packet.address & 0x7u) != 0);
			auto& destination = *reinterpret_cast<uint64_t*>(packet.address);
			EXIT_NOT_IMPLEMENTED(ExecuteAtomicMemPacket(packet, destination) !=
			                     AtomicMemResult::Completed);
			break;
		}
		case AtomicMemOperation::Unsupported: EXIT("unsupported ATOMIC_MEM operation\n");
	}
...
KYTY_CP_OP_PARSER(CpOpCopyData) {
	...
	if (ResolveCopyDataImmediate64Action(src_sel, dst_sel, num_bytes) ==
	    CopyDataImmediate64Action::TwoDwordFills) {
		if (dst == 0 || dst > UINT64_MAX - sizeof(uint64_t)) {
			EXIT("invalid 64-bit immediate copyData destination 0x%016" PRIx64 "\n", dst);
		}
		const auto dma_dst = CopyDataDstToDma(dst_sel);
		const auto words   = SplitCopyDataImmediate64(src);
		cp.DmaData(0, dma_dst, dst_cache, dst, dma_src, src_cache, words[0], sizeof(uint32_t), 1,
		           write_confirm, 1);
		cp.DmaData(0, dma_dst, dst_cache, dst + sizeof(uint32_t), dma_src, src_cache, words[1],
...
KYTY_CP_OP_PARSER(CpOpIndirectBuffer) {
	...
	const auto jump_action = ResolveIndirectBufferJumpAction(control);
	...
	if (indirect_log_count.fetch_add(1) < 128) {
		LOGF("\t indirect buffer: addr=0x%016" PRIx64 ", num_dw=%" PRIu32
		     ", control=0x%08" PRIx32 ", chain=%u\n",
		     reinterpret_cast<uint64_t>(indirect_buffer), indirect_num_dw, control,
		     jump_action == IndirectBufferJumpAction::Chain ? 1u : 0u);
	}
	...
	cp.ProcessIndirectBuffer(indirect_buffer, indirect_num_dw, jump_action);
...
KYTY_CP_OP_PARSER(CpOpNop) {
	...
	if (r == Pm4::R_ZERO) {
		if (Pm4::IsSingleDwordNop(cmd_id)) {
			return 0;
		}
...
KYTY_CP_OP_PARSER(CpOpReleaseMem) {
	...
	EXIT_NOT_IMPLEMENTED(!IsReleaseMemPacketHeader(cmd_id));
	...
	const auto conditional_interrupt =
	    Sync::ResolveEndOfPipeConditionalInterruptAction(interrupt_selector);
	if (conditional_interrupt == Sync::EndOfPipeConditionalInterruptAction::Compare32LessOrEqual) {
		EXIT_NOT_IMPLEMENTED(release_dst != ReleaseMemDstMemory);
		EXIT_NOT_IMPLEMENTED(gcr_cntl != 0);
		EXIT_IF(dst_gpu_addr == nullptr);
		cp.TriggerEopEventIfLessOrEqual32AtEndOfPipe(dst_gpu_addr, static_cast<uint32_t>(value),
		                                                interrupt_context_id);
		cp.BufferFlush();
		return 7;
	}
	if (conditional_interrupt == Sync::EndOfPipeConditionalInterruptAction::Compare64LessOrEqual) {
		EXIT_NOT_IMPLEMENTED(release_dst != ReleaseMemDstMemory);
		EXIT_NOT_IMPLEMENTED(gcr_cntl != 0);
		EXIT_IF(dst_gpu_addr == nullptr);
		cp.TriggerEopEventIfLessOrEqual64AtEndOfPipe(dst_gpu_addr, value,
		                                                interrupt_context_id);
		cp.BufferFlush();
		return 7;
	}
...
KYTY_CP_OP_PARSER(CpOpWaitRegMem32) {
	...
	WaitRegMem32Packet packet {};
	EXIT_NOT_IMPLEMENTED(!DecodeWaitRegMem32Packet(cmd_id, buffer, packet));
	EXIT_NOT_IMPLEMENTED((packet.control & 0x10u) == 0);

	auto* addr = reinterpret_cast<const uint32_t*>(packet.address);
	cp.WaitRegMem(packet.control & 0x7u, addr, packet.reference, packet.mask,
	              packet.poll_interval, CpOpWaitRegMemWaitOp<uint32_t>(packet.control));
...
KYTY_CP_OP_PARSER(CpOpMemSemaphore) {
	...
	EXIT_NOT_IMPLEMENTED(!DecodeMemSemaphorePacket(cmd_id, buffer, packet));
	EXIT_NOT_IMPLEMENTED(packet.address == 0 || (packet.address & 0x7u) != 0);

	auto* counter = reinterpret_cast<uint64_t*>(packet.address);
	switch (packet.operation) {
		case MemSemaphoreOperation::Signal:
			cp.SignalMemSemaphore(counter, packet.signal_type, packet.wait_for_mailbox);
			break;
		case MemSemaphoreOperation::Wait: cp.WaitMemSemaphore(counter); break;
		case MemSemaphoreOperation::Unsupported:
			EXIT("unsupported MEM_SEMAPHORE operation\n");
	}
...
void GraphicsInitJmpTablesCxIndirect() {
	...
	g_hw_ctx_indirect_func[Pm4::PA_SU_POLY_OFFSET_DB_FMT_CNTL] = [](KYTY_HW_CTX_INDIRECT_ARGS) {
		EXIT_NOT_IMPLEMENTED(!cp.GetCtx().SetPolygonOffsetRegister(cmd_offset, value));
	};
	...
	g_hw_ctx_indirect_func[Pm4::PA_SC_MODE_CNTL_1] = [](KYTY_HW_CTX_INDIRECT_ARGS) {
		HwCtxSetPsShaderRateRegister(cp, cmd_offset, value);
	};
	...
	g_hw_ctx_indirect_func[Pm4::PA_SC_AA_MASK_X0Y0_X1Y0] = [](KYTY_HW_CTX_INDIRECT_ARGS) {
		HwCtxSetAaAndPsSampleMaskRegister(cp, cmd_offset, value);
	};
	...
	g_hw_ctx_indirect_func[Pm4::PA_SC_CONSERVATIVE_RASTERIZATION_CNTL] =
	    [](KYTY_HW_CTX_INDIRECT_ARGS) {
		    HwCtxSetPaScExtendedControlRegister(cp, cmd_offset, value);
	    };
	...
	g_hw_ctx_indirect_func[Pm4::DB_ALPHA_TO_MASK] = [](KYTY_HW_CTX_INDIRECT_ARGS) {
		cp.GetCtx().SetAlphaToMaskControl(HW::AlphaToMaskControl::FromRegister(value));
	};
...
void GraphicsInitJmpTablesShIndirect() {
	...
	// sce::Agc::ShShaderBaseEs treats its third register (RSRC1_ES) as an
	// invariant kDefault word. The executable ES address is carried by LO/HI;
	// merged-stage resource state is programmed through the GS registers.
...
void GraphicsInitJmpTablesUcIndirect() {
	...
	g_hw_uc_indirect_func[Pm4::GE_MULTI_PRIM_IB_RESET_EN] = [](KYTY_HW_UC_INDIRECT_ARGS) {
		HwSetPrimitiveResetControl(cp, value);
	};
