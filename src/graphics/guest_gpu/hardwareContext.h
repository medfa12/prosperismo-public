...
[[nodiscard]] constexpr float DecodeLineWidthEighths(uint32_t eighths) {
	return static_cast<float>(eighths) / 8.0f;
}
...
struct PolygonOffsetControl {
	uint32_t db_format_raw    = 0;
	uint32_t clamp_raw        = 0;
	uint32_t front_scale_raw  = 0;
	uint32_t front_offset_raw = 0;
	uint32_t back_scale_raw   = 0;
	uint32_t back_offset_raw  = 0;

	[[nodiscard]] bool SetRegister(uint32_t register_index, uint32_t value) {
		switch (register_index) {
			case Pm4::PA_SU_POLY_OFFSET_DB_FMT_CNTL: db_format_raw = value; break;
			case Pm4::PA_SU_POLY_OFFSET_CLAMP: clamp_raw = value; break;
			case Pm4::PA_SU_POLY_OFFSET_FRONT_SCALE: front_scale_raw = value; break;
			case Pm4::PA_SU_POLY_OFFSET_FRONT_OFFSET: front_offset_raw = value; break;
			case Pm4::PA_SU_POLY_OFFSET_BACK_SCALE: back_scale_raw = value; break;
			case Pm4::PA_SU_POLY_OFFSET_BACK_OFFSET: back_offset_raw = value; break;
			default: return false;
		}
		return true;
	}

	[[nodiscard]] float Clamp() const { return std::bit_cast<float>(clamp_raw); }
	[[nodiscard]] float FrontScale() const { return std::bit_cast<float>(front_scale_raw); }
	[[nodiscard]] float FrontOffset() const { return std::bit_cast<float>(front_offset_raw); }
	[[nodiscard]] float BackScale() const { return std::bit_cast<float>(back_scale_raw); }
	[[nodiscard]] float BackOffset() const { return std::bit_cast<float>(back_offset_raw); }
};

struct PipelineDepthBiasState {
...
[[nodiscard]] inline PipelineDepthBiasState ResolvePipelineDepthBias(
    const ModeControl& mode, const PolygonOffsetControl& control,
    bool depth_bias_clamp_supported = true) {
	const auto finite = [](uint32_t raw) { return (raw & 0x7f800000u) != 0x7f800000u; };
	const bool front_visible = !mode.cull_front;
	const bool back_visible  = !mode.cull_back;
	...
	if (front_visible && back_visible) {
		...
		enable = mode.poly_offset_front_enable;
		if (enable && (control.FrontScale() != control.BackScale() ||
		               control.FrontOffset() != control.BackOffset())) {
			return {.representable = false};
		}
		scale_raw  = control.front_scale_raw;
		offset_raw = control.front_offset_raw;
	} else if (front_visible) {
		enable     = mode.poly_offset_front_enable;
		scale_raw  = control.front_scale_raw;
		offset_raw = control.front_offset_raw;
	} else {
		enable     = mode.poly_offset_back_enable;
		scale_raw  = control.back_scale_raw;
		offset_raw = control.back_offset_raw;
	}

	if (!enable) {
		return {};
	}
	if (!finite(scale_raw) || !finite(offset_raw) || !finite(control.clamp_raw) ||
	    (!depth_bias_clamp_supported && control.Clamp() != 0.0f)) {
		return {.representable = false};
	}
	return {.representable   = true,
	        .enable          = true,
	        .constant_factor = std::bit_cast<float>(offset_raw),
	        .clamp           = control.Clamp(),
	        .slope_factor    = std::bit_cast<float>(scale_raw) / 16.0f};
}
...
struct AlphaToMaskControl {
	...
	uint8_t top_left_threshold     = 2;
	uint8_t top_right_threshold    = 2;
	uint8_t bottom_left_threshold  = 2;
	uint8_t bottom_right_threshold = 2;

	[[nodiscard]] static constexpr AlphaToMaskControl FromRegister(uint32_t value) {
		return {.raw           = value,
		        .enable        = (value & 0x00000001u) != 0,
		        .dither_enable = (value & 0x00010000u) != 0,
		        .top_left_threshold = static_cast<uint8_t>((value >> 8u) & 0x3u),
		        .top_right_threshold = static_cast<uint8_t>((value >> 10u) & 0x3u),
		        .bottom_left_threshold = static_cast<uint8_t>((value >> 12u) & 0x3u),
		        .bottom_right_threshold = static_cast<uint8_t>((value >> 14u) & 0x3u)};
	}
};

[[nodiscard]] constexpr uint32_t DecodeAlphaToMaskSampleCount(uint8_t encoded) {
	return encoded <= 4u ? (1u << encoded) : 1u;
}
...
struct PsShaderRateControl {
	bool per_sample = false;

	[[nodiscard]] static constexpr PsShaderRateControl FromRegister(uint32_t value) {
		return {.per_sample = (value & 0x00010000u) != 0};
	}
};

struct ConservativeRasterizationControl {
	uint32_t mode               = 0;
	uint32_t uncertainty_region = 0;

	[[nodiscard]] static constexpr ConservativeRasterizationControl FromRegister(uint32_t value) {
		return {.mode = value & 0x00000021u, .uncertainty_region = value & 0x000f0000u};
	}
};
...
struct PrimitiveResetControl {
	bool enable         = false;
	bool match_all_bits = false;

	[[nodiscard]] static constexpr PrimitiveResetControl FromRegister(uint32_t value) {
		return {.enable = (value & 0x1u) != 0u, .match_all_bits = (value & 0x2u) != 0u};
	}
};
...
public:
	...
	void SetPrimitiveResetIndex(uint32_t value) { m_primitive_reset_index = value; }
	...
	[[nodiscard]] const ConservativeRasterizationControl&
	GetConservativeRasterizationControl() const {
		return m_conservative_rasterization_control;
	}
	void SetConservativeRasterizationControl(const ConservativeRasterizationControl& control) {
		m_conservative_rasterization_control = control;
	}
	[[nodiscard]] const PolygonOffsetControl& GetPolygonOffsetControl() const {
		return m_polygon_offset_control;
	}
	[[nodiscard]] bool SetPolygonOffsetRegister(uint32_t register_index, uint32_t value) {
		return m_polygon_offset_control.SetRegister(register_index, value);
	}
	...
	[[nodiscard]] const AlphaToMaskControl& GetAlphaToMaskControl() const {
		return m_alpha_to_mask_control;
	}
	void SetAlphaToMaskControl(const AlphaToMaskControl& control) {
		m_alpha_to_mask_control = control;
	}
	...
	[[nodiscard]] const PsShaderRateControl& GetPsShaderRateControl() const {
		return m_ps_shader_rate_control;
	}
	void SetPsShaderRateControl(const PsShaderRateControl& control) {
		m_ps_shader_rate_control = control;
	}
	...
	[[nodiscard]] uint64_t GetAaSampleMask() const { return m_aa_sample_mask; }
	void                   SetAaSampleMask(uint64_t mask) { m_aa_sample_mask = mask; }
	[[nodiscard]] uint32_t GetPsShaderSampleExclusionMask() const {
		return m_ps_shader_sample_exclusion_mask;
	}
	void SetPsShaderSampleExclusionMask(uint32_t mask) { m_ps_shader_sample_exclusion_mask = mask; }
...
private:
	...
	PsShaderRateControl m_ps_shader_rate_control;

	AaSampleControl m_aa_sample_control;
	AaConfig        m_aa_config;
	uint64_t        m_aa_sample_mask = UINT64_MAX;
	uint32_t        m_ps_shader_sample_exclusion_mask = 0;

	uint32_t m_shader_stages = 0;
	uint32_t m_primitive_reset_index = UINT32_MAX;
	...
	ConservativeRasterizationControl m_conservative_rasterization_control;
	PolygonOffsetControl            m_polygon_offset_control;
	EqaaControl                     m_eqaa_control;
	AlphaToMaskControl              m_alpha_to_mask_control;
...
public:
	...
	void SetPrimitiveResetControl(const PrimitiveResetControl& control) {
		m_primitive_reset_control = control;
	}
	[[nodiscard]] const PrimitiveResetControl& GetPrimitiveResetControl() const {
		return m_primitive_reset_control;
	}
...
private:
	...
	PrimitiveResetControl m_primitive_reset_control;
