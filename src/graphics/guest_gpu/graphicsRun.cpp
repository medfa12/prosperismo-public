...
void CommandProcessor::WaitMemSemaphore(uint64_t* counter) {
	EXIT_IF(counter == nullptr || (reinterpret_cast<uintptr_t>(counter) & 0x7u) != 0);
	const auto result = TryConsumeMemSemaphore(*counter);
	EXIT_IF(result == MemSemaphoreWaitResult::Unsupported);
	if (result == MemSemaphoreWaitResult::Waiting) {
		SuspendPm4();
	}
}

void CommandProcessor::SignalMemSemaphore(uint64_t* counter,
	...
	Sync::SignalMemSemaphoreAtEndOfPipe(m_submit_id, CurrentBuffer(), counter, signal_type);
	if (wait_for_mailbox) {
		BufferFlushAndWait();
...
void CommandProcessor::WriteData(uint32_t* dst, const uint32_t* src, uint32_t dw_num,
	...
	const auto address_mode = ResolveWriteDataAddressMode(increment, dst_sel);
	EXIT_NOT_IMPLEMENTED(address_mode == WriteDataAddressMode::Unsupported);
	...
	EXIT_IF(!CommitWriteDataPayload(dst, src, dw_num, address_mode));
}
...
void CommandProcessor::DmaData(uint8_t engine, uint8_t dst_sel, uint8_t dst_cache_policy,
	...
	const auto dst_action = ResolveDmaDataDestinationAction(dst_sel);
	if (dst_action == DmaDataDestinationAction::Unsupported) {
		EXIT("unsupported dmaData destination selector 0x%02" PRIx8 "\n", dst_sel);
	}
	if (dst_action == DmaDataDestinationAction::Discard) {
		return;
	}
	...
	const bool dst_gds = dst_action == DmaDataDestinationAction::Gds;
...
Pm4ProcessResult CommandProcessor::Process(Pm4Execution& execution, uint32_t* buffer,
	...
	execution.m_cursor_replaced = false;
...
void CommandProcessor::ProcessIndirectBuffer(uint32_t* buffer, uint32_t size_dw,
                                             IndirectBufferJumpAction action) {
	EXIT_IF(g_current_execution == nullptr);
	if (size_dw == 0) {
		return;
	}
	...
	if (ReplaceIndirectBufferCursorForChain(action, buffer, size_dw,
	                                        execution.m_buffer_stack.back())) {
		execution.m_cursor_replaced = true;
		return;
	}
...
void CommandProcessor::Rewind(uint32_t control) {
	if (ResolveRewindAction(control) == RewindAction::Suspend) {
		SuspendPm4();
	}
}
...
void CommandProcessor::ProcessPm4(Pm4Execution& execution, size_t stop_depth) {
	while (execution.m_buffer_stack.size() > stop_depth) {
		...
		EXIT_NOT_IMPLEMENTED(!Pm4::PacketFitsInDwords(packet_header, remaining_dw));
		...
		if (execution.m_cursor_replaced) {
			execution.m_cursor_replaced = false;
			execution.m_made_progress   = true;
			continue;
		}
	...
	switch (event_write_source) {
		...
		case 0x04:
			if constexpr (sizeof(T) == sizeof(uint64_t)) {
				const auto clock_action =
				    Sync::ResolveEndOfPipeReferenceClockAction(cache_action, with_interrupt);
				switch (cache_action) {
					case 0x00:
						if (((eop_event_type == 0x04 && event_index == 0x05) ||
						     (eop_event_type == 0x28 && event_index == 0x00)) &&
						    clock_action != Sync::EndOfPipeReferenceClockAction::Unsupported) {
							Sync::WriteAtEndOfPipeClockCounter(
							    m_submit_id, CurrentBuffer(), static_cast<uint64_t*>(dst_gpu_addr),
							    clock_action == Sync::EndOfPipeReferenceClockAction::WriteAndInterrupt,
							    interrupt_context_id);
							return;
						}
						break;
					case 0x38:
						if (((eop_event_type == 0x04 &&
							...
							Sync::WriteAtEndOfPipeClockCounterWithWriteBack(
							    m_submit_id, CurrentBuffer(), static_cast<uint64_t*>(dst_gpu_addr),
							    clock_action ==
							        Sync::EndOfPipeReferenceClockAction::WriteBackAndInterrupt,
							    interrupt_context_id);
							return;
						}
						break;
...
void CommandProcessor::TriggerEopEventIfLessOrEqual32AtEndOfPipe(
    const void* address, uint32_t reference, uint32_t interrupt_context_id) {
	CheckBuffer();

	Sync::TriggerEopEventIfLessOrEqual32AtEndOfPipe(CurrentBuffer(), address, reference,
	                                               interrupt_context_id);
}

void CommandProcessor::TriggerEopEventIfLessOrEqual64AtEndOfPipe(
    const void* address, uint64_t reference, uint32_t interrupt_context_id) {
	CheckBuffer();

	Sync::TriggerEopEventIfLessOrEqual64AtEndOfPipe(CurrentBuffer(), address, reference,
	                                               interrupt_context_id);
}
...
void CommandProcessor::TriggerEvent(uint32_t event_type, uint32_t event_index) {
	...
	if (EventWriteRequiresDfsmFlushBarrier(event_type)) {
		EmitGlobalBarrier();
		return;
	}
