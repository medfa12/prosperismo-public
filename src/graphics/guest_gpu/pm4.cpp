...
constexpr auto MakeOpcodeNames() {
	...
	names[IT_ATOMIC_MEM]                = "IT_ATOMIC_MEM";
	...
	names[IT_COND_WRITE]                = "IT_COND_WRITE";
...
void DumpPm4PacketStream(Common::File* file, uint32_t* cmd_buffer, uint32_t start_dw,
	...
	while (dw != 0) {
		...
		// Type-2 padding and the AGC single-DWORD NOP are both header-only packets.
		EXIT_NOT_IMPLEMENTED(dw < 2 && packet_type != PacketType::Type2 &&
		                     !IsSingleDwordNop(cmd_id));
		...
		switch (packet_type) {
			case PacketType::Type3: {
				...
				len                 = GetPacketSizeDwords(cmd_id) - 1u;
