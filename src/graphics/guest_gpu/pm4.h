...
#define KYTY_PM4_LEN(cmd_id) (::Libs::Graphics::Pm4::GetPacketSizeDwords(cmd_id))
...
constexpr uint32_t IT_ATOMIC_MEM                = 0x1E;
...
constexpr uint32_t IT_COND_WRITE                = 0x45;
...
[[nodiscard]] constexpr bool IsSingleDwordNop(uint32_t cmd_id) {
	return cmd_id == KYTY_PM4(1u, IT_NOP, R_ZERO);
}

[[nodiscard]] constexpr uint32_t GetPacketSizeDwords(uint32_t cmd_id) {
	return IsSingleDwordNop(cmd_id) ? 1u : (((cmd_id >> 16u) & 0x3fffu) + 2u);
}

[[nodiscard]] constexpr bool PacketFitsInDwords(uint32_t cmd_id, uint32_t remaining_dw) {
	return GetPacketSizeDwords(cmd_id) <= remaining_dw;
}
...
constexpr uint32_t PA_SC_MODE_CNTL_1_PS_ITER_SAMPLE_SHIFT      = 16;
constexpr uint32_t PA_SC_MODE_CNTL_1_PS_ITER_SAMPLE_MASK       = 0x1;
