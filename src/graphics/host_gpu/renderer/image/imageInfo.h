...
[[nodiscard]] constexpr bool ImageMetadataIdentityMatches(const ImageMetadataInfo& left,
                                                          const ImageMetadataInfo& right) noexcept {
	if (left.kind != right.kind) {
		return false;
	}
	switch (left.kind) {
		case ImageMetadataKind::None: return true;
		case ImageMetadataKind::Htile: return left.range == right.range;
		case ImageMetadataKind::Dcc: return left.range.address == right.range.address;
	}
	return false;
}
...
struct ImageInfo {
	...
	[[nodiscard]] int32_t MipOf(const ImageInfo& container) const noexcept {
		...
		const bool layered = container.IsLayered();
		...
		if (layered) {
			...
			slice_stride = container.data.size / container.resources.layers;
			if (slice_stride == 0) {
				return -1;
			}
		}
		...
		for (uint32_t level = 0; level < container.resources.levels; level++) {
			...
			if (layered) {
				// Array slices repeat the complete mip layout. Mip offsets are relative to each
				// full slice, rather than identifying one contiguous all-layer mip range.
				if (layout.offset > slice_stride || slice_size > slice_stride - layout.offset ||
				    data.address < mip_base) {
					continue;
				}
				const auto address_delta = data.address - mip_base;
				if (address_delta % slice_stride != 0 ||
				    address_delta / slice_stride >= container.resources.layers) {
					continue;
				}
			} else {
				if (mip_base > UINT64_MAX - layout.size) {
					continue;
				}
	...
	[[nodiscard]] int32_t SliceOf(const ImageInfo& container, int32_t mip) const noexcept {
		...
		const auto address_delta = data.address - mip_base;
		if (container.IsLayered()) {
			if (container.data.size == 0 ||
			    container.data.size % container.resources.layers != 0) {
				return -1;
			}
			const auto slice_stride = container.data.size / container.resources.layers;
			if (slice_stride == 0 || layout.offset > slice_stride ||
			    slice_size > slice_stride - layout.offset || address_delta % slice_stride != 0) {
				return -1;
			}
			const auto slice = address_delta / slice_stride;
			if (slice >= container.resources.layers || slice > INT32_MAX) {
				return -1;
			}
