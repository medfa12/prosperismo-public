...
void CreatePipelineInternal(
	...
	provoking_vertex.sType =
	    vk::StructureType::ePipelineRasterizationProvokingVertexStateCreateInfoEXT;
	provoking_vertex.provokingVertexMode = static_params.provoking_vertex_last
	                                           ? vk::ProvokingVertexModeEXT::eLastVertex
	                                           : vk::ProvokingVertexModeEXT::eFirstVertex;
#if defined(__APPLE__)
	provoking_vertex.pNext = nullptr;
	const void* rasterization_tail =
	    static_params.provoking_vertex_last ? static_cast<const void*>(&provoking_vertex) : nullptr;
#else
	provoking_vertex.pNext = &clip_ext;
	const void* rasterization_tail = static_params.provoking_vertex_last
	                                     ? static_cast<const void*>(&provoking_vertex)
	                                     : static_cast<const void*>(&clip_ext);
#endif

	vk::PipelineRasterizationConservativeStateCreateInfoEXT conservative_rasterization {};
	conservative_rasterization.sType =
	    vk::StructureType::ePipelineRasterizationConservativeStateCreateInfoEXT;
	conservative_rasterization.pNext = rasterization_tail;
	conservative_rasterization.flags = {};
	conservative_rasterization.conservativeRasterizationMode =
	    static_params.conservative_rasterization_mode;
	conservative_rasterization.extraPrimitiveOverestimationSize = 0.0f;
	...
	rasterizer.pNext = static_params.conservative_rasterization_mode !=
	                           vk::ConservativeRasterizationModeEXT::eDisabled
	                       ? static_cast<const void*>(&conservative_rasterization)
	                       : rasterization_tail;
	rasterizer.flags                   = {};
	...
	multisampling.minSampleShading      = static_params.sample_shading_minimum;
	multisampling.pSampleMask           = nullptr;
	multisampling.alphaToCoverageEnable =
	    static_params.alpha_to_coverage_enable ? VK_TRUE : VK_FALSE;
	...
	for (uint32_t i = 0; i < static_params.color_count; i++) {
		...
		color_blend_attachment[i].alphaBlendOp = GetBlendOp(static_params.alpha_comb_fcn[i]);
	}
	...
	color_blending.logicOpEnable     = static_params.logic_op_enable ? VK_TRUE : VK_FALSE;
	color_blending.logicOp           = static_params.logic_op;
