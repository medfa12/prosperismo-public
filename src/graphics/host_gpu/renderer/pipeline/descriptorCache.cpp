...
std::vector<uint32_t> LayoutKey(DescriptorCache::Stage               stage,
	...
	for (const auto& binding: program.bindings.descriptors) {
		...
		key.push_back(ShaderRecompiler::IR::DescriptorCount(program, binding));
...
DescriptorCache::GetDescriptorSetLayoutInternal(Stage                                stage,
	...
	for (const auto& descriptor: program.bindings.descriptors) {
		bindings.push_back({descriptor.binding, DescriptorType(descriptor.kind),
		                    ShaderRecompiler::IR::DescriptorCount(program, descriptor),
		                    StageFlags(stage), nullptr});
	}
	if (bindings.empty()) {
		return nullptr;
	}
...
void DescriptorCache::CreatePool() {
	...
	const vk::DescriptorPoolSize sizes[] = {
	    ...
	    {vk::DescriptorType::eStorageImage,
	     MaxSets * ShaderRecompiler::IR::ShaderInfo::MaxImages * 16u},
...
VulkanDescriptorSet& DescriptorCache::GetDescriptor(Stage                                stage,
	...
	for (const auto& binding: program.bindings.descriptors) {
		descriptor_count += ShaderRecompiler::IR::DescriptorCount(program, binding);
	}
	...
	for (const auto& binding: program.bindings.descriptors) {
		...
		write.descriptorCount   = ShaderRecompiler::IR::DescriptorCount(program, binding);
			...
			default: {
				for (const auto resource: binding.resources) {
					...
					if (texture.mip_views.empty()) {
						...
						for (const auto view: texture.mip_views) {
							EXIT_IF(!texture.image_id || view == nullptr ||
							        texture.layout == vk::ImageLayout::eUndefined);
							image_infos.push_back({nullptr, view, texture.layout});
						}
					}
				}
				break;
