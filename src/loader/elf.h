...
constexpr uint8_t ELF_ABI_VERSION_NEXT_GEN      = 2;
constexpr uint8_t ELF_ABI_VERSION_MAX_SUPPORTED = 3;
...
constexpr Elf64_Word PT_GNU_RELRO     = 0x6474e552;
...
struct Elf64_Rela // NOLINT(readability-identifier-naming)
{
	...
	[[nodiscard]] Elf64_Word GetType() const {
		return static_cast<Elf64_Word>(r_info & 0xffffffff);
	}
...
public:
	...
	void Save(const std::filesystem::path& file_name);
	...
	[[nodiscard]] std::vector<const Elf64_Dyn*> GetDynList(Elf64_Sxword tag) const;
	[[nodiscard]] bool HasDynValue(Elf64_Sxword tag) const { return GetDynValue(tag) != nullptr; }

	KYTY_CLASS_NO_COPY(Elf64);

	[[nodiscard]] const Elf64_Dyn* GetDynamic() const {
		return reinterpret_cast<const Elf64_Dyn*>(m_dynamic.get());
	}
	[[nodiscard]] const Elf64_Ehdr* GetEhdr() const { return m_ehdr.get(); }
	[[nodiscard]] const Elf64_Phdr* GetPhdr() const { return m_phdr.get(); }
	[[nodiscard]] const Elf64_Shdr* GetShdr() const { return m_shdr.get(); }
	[[nodiscard]] char*             GetStrTable() const { return m_str_table.get(); }
...
private:
	...
	std::unique_ptr<SelfSegment[]> m_self_segments;
	std::unique_ptr<Elf64_Ehdr>    m_ehdr;
