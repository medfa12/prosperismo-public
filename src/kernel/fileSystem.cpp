...
struct File {
	...
	uint64_t                            dents_size;
	bool                                dents_size_known;
	std::vector<uint64_t>               dents_valid_offsets;
};
...
static int DecodeFileSeekTarget(uint64_t base, int64_t offset, int64_t* target) {
	EXIT_IF(target == nullptr);

	if (base > static_cast<uint64_t>(INT64_MAX)) {
	...
	if (offset < 0) {
		...
		*target = static_cast<int64_t>(base - magnitude);
		return OK;
	}

	const auto positive = static_cast<uint64_t>(offset);
	if (base > static_cast<uint64_t>(INT64_MAX) - positive) {
		return KERNEL_ERROR_EOVERFLOW;
	}

	*target = static_cast<int64_t>(base + positive);
	return OK;
}
...
static bool PathHasOverlongComponent(const char* path) {
	constexpr size_t NAME_MAX_LENGTH = 255;
	size_t           component_size  = 0;

	for (const char* c = path;; ++c) {
		if (*c == '/' || *c == '\0') {
			if (component_size > NAME_MAX_LENGTH) {
				return true;
			}
			component_size = 0;
			if (*c == '\0') {
...
static bool PathExceedsKernelLimits(const char* path) {
	constexpr size_t PATH_MAX_LENGTH = 1024;
	return std::strlen(path) >= PATH_MAX_LENGTH || PathHasOverlongComponent(path);
}
...
int FileDescriptors::Flush(int d) {
	Common::LockGuard lock(m_mutex);
	...
	Common::LockGuard file_lock(file->mutex);
	return file->f.Flush() ? OK : KERNEL_ERROR_EIO;
}
...
int KYTY_SYSV_ABI KernelFsync(int d) {
	...
	return g_files->Flush(d);
}

int KYTY_SYSV_ABI KernelFdatasync(int d) {
	PRINT_NAME();
	return KernelFsync(d);
}
...
int64_t KYTY_SYSV_ABI KernelPwrite(int d, const void* buf, size_t nbytes, int64_t offset) {
	...
	auto     pos           = file->f.Tell();
	uint32_t bytes_written = 0;
	file->f.Seek(static_cast<uint64_t>(offset));
	file->f.Write(buf, static_cast<uint32_t>(nbytes), &bytes_written);
	if (file->sync_writes) {
		file->f.Flush();
	}
...
int64_t KYTY_SYSV_ABI KernelLseek(int d, int64_t offset, int whence) {
	...
	if (file->directory) {
		...
		uint64_t base = 0;
		if (whence == 1) {
			base = file->dents_offset;
		} else if (whence == 2) {
			if (!file->dents_size_known) {
				return KERNEL_ERROR_EINVAL;
			}
			base = file->dents_size;
		}
		...
		if (std::find(file->dents_valid_offsets.begin(), file->dents_valid_offsets.end(), target) ==
		    file->dents_valid_offsets.end()) {
			return KERNEL_ERROR_EINVAL;
		}

		file->dents_offset = target;
	...
	int64_t target = 0;
	int     result = DecodeFileSeekTarget(base, offset, &target);
	if (result != OK) {
		return result;
	}
...
int KYTY_SYSV_ABI KernelFtruncate(int d, int64_t length) {
	...
	if (length < 0 || file->directory || file->special != SpecialFile::None || !file->writable) {
		return KERNEL_ERROR_EINVAL;
	}
...
int KYTY_SYSV_ABI KernelTruncate(const char* path, int64_t length) {
	...
	const auto real_file = g_mount_points->GetRealFilename(path);
	const auto real_dir  = g_mount_points->GetRealDirectory(path);

	if (Common::File::IsDirectoryExisting(real_file) ||
	    Common::File::IsDirectoryExisting(real_dir)) {
		return KERNEL_ERROR_EISDIR;
	}

	if (!Common::File::IsFileExisting(real_file)) {
		return KERNEL_ERROR_ENOENT;
	}

	Common::File file(real_file, Common::File::Mode::ReadWrite);
	if (file.IsInvalid()) {
		return KERNEL_ERROR_EACCES;
	}

	const bool ok = file.Truncate(static_cast<uint64_t>(length));
	file.Close();
	return ok ? OK : KERNEL_ERROR_EIO;
}
...
int KYTY_SYSV_ABI KernelGetdirentries(int fd, char* buf, int nbytes, int64_t* basep) {
	...
	file->dents_size              = directory_size;
	file->dents_size_known        = true;
	...
	if (std::find(file->dents_valid_offsets.begin(), file->dents_valid_offsets.end(),
	              file->dents_offset) == file->dents_valid_offsets.end()) {
		file->dents_valid_offsets.push_back(file->dents_offset);
	}
...
int KYTY_SYSV_ABI KernelMkdir(const char* path, uint16_t mode) {
	...
	if (PathExceedsKernelLimits(path)) {
		return KERNEL_ERROR_ENAMETOOLONG;
	}
...
int KYTY_SYSV_ABI KernelRmdir(const char* path) {
	...
	if (!Common::File::GetDirEntries(real_name).empty()) {
