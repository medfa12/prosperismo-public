...
constexpr int PROT_CPU_GPU =
    PROT_CPU_READ | PROT_CPU_WRITE | PROT_CPU_EXEC | PROT_GPU_READ | PROT_GPU_WRITE;
constexpr int PROT_DIRECT_SUPPORTED =
    PROT_CPU_GPU | PROT_AMPR_READ | PROT_AMPR_WRITE | PROT_ACP_READ | PROT_ACP_WRITE;
...
static bool DecodeDirectMemoryProtection(int prot, VirtualMemory::Mode* mode,
                                         GpuAccessMode* gpu_mode) {
	if ((prot & ~PROT_DIRECT_SUPPORTED) != 0) {
		return false;
	}
	// AMPR/ACP permissions are guest metadata and do not grant host CPU/GPU access.
	return DecodeMemoryProtection(prot & PROT_CPU_GPU, mode, gpu_mode);
}
...
static int UnmapMemoryRange(uint64_t vaddr, size_t len) {
	...
	if (range.type == VirtualRangeType::Pooled) {
		...
		if (DecodeDirectMemoryProtection(range.protection, &direct_mode, &direct_gpu) &&
...
int KYTY_SYSV_ABI KernelMapDirectMemory(void** addr, size_t len, int prot, int flags,
	...
	if (!DecodeDirectMemoryProtection(prot, &mode, &gpu_mode)) {
		return KERNEL_ERROR_EINVAL;
	}
...
static bool ReplaceFixedRangeWithReserved(uint64_t start, uint64_t size) {
	...
	while (current < end) {
		...
		if (range.type == VirtualRangeType::Direct) {
			...
			DecodeDirectMemoryProtection(replaced.range.protection, &replaced.mode,
			                             &replaced.gpu_mode);
		} else {
			DecodeMemoryProtection(replaced.range.protection, &replaced.mode,
			                       &replaced.gpu_mode);
		}
		if (range.type == VirtualRangeType::Direct &&
