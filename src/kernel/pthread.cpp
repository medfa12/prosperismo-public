...
static uint64_t KernelGetElapsedTsc() {
	...
	static const uint64_t scale = []() -> uint64_t {
...
int KYTY_SYSV_ABI PthreadMutexattrGetprotocol(PthreadMutexattr* attr, int* protocol) {
	// PRINT_NAME();

	if (attr == nullptr || *attr == nullptr || protocol == nullptr) {
		return KERNEL_ERROR_EINVAL;
	}

	*protocol = (*attr)->pprotocol;
	return OK;
}
...
int KYTY_SYSV_ABI PthreadMutexattrSetprotocol([[maybe_unused]] PthreadMutexattr* attr,
	...
	// Host priority protocols are not used here. Retain the guest ABI value because the host
	// constants differ on winpthreads (INHERIT/PROTECT are 8/16 instead of 1/2).
	(*attr)->pprotocol = protocol;
	return OK;
}
...
int KYTY_SYSV_ABI PthreadAttrSetinheritsched(PthreadAttr* attr, int inherit_sched) {
	...
	switch (inherit_sched) {
		...
		default: return KERNEL_ERROR_ENOTSUP;
	}
...
int KYTY_SYSV_ABI PthreadAttrSetschedparam(PthreadAttr* attr, const KernelSchedParam* param) {
	...
	if (param->sched_priority < 256 || param->sched_priority > 767) {
	...
	if (result == 0) {
		attr_value->sched_priority = param->sched_priority;
		return OK;
	}
	return KERNEL_ERROR_EINVAL;
}
...
int KYTY_SYSV_ABI PthreadAttrSetschedpolicy(PthreadAttr* attr, int policy) {
	...
	if (result == 0) {
		KernelSchedParam default_param {};
		result = pthread_attr_setschedparam(&attr_value->p, &default_param);
	}

	if (result == 0) {
		attr_value->policy = policy;
		attr_value->sched_priority = 700;
		return OK;
	}
	return KERNEL_ERROR_EINVAL;
}
...
int KYTY_SYSV_ABI PthreadRwlockattrSettype(PthreadRwlockattr* attr, int type) {
	...
	if (attr == nullptr || *attr == nullptr || (type != 1 && type != 2)) {
		return KERNEL_ERROR_EINVAL;
	}
...
int KYTY_SYSV_ABI PthreadCondSignalto(PthreadCond* cond, Pthread thread) {
	...
	return PthreadCondSignaltoResult(notify);
}
...
int KYTY_SYSV_ABI pthread_mutexattr_getprotocol(LibKernel::PthreadMutexattr* attr, int* protocol) {
	PRINT_NAME();

	return POSIX_PTHREAD_CALL(LibKernel::PthreadMutexattrGetprotocol(attr, protocol));
}
