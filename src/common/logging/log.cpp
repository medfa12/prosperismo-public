...
static void SetupLogger() {
	...
	switch (g_direction) {
		case Direction::Silent:
			g_logger = MakeLogger("prosperismo", std::make_shared<spdlog::sinks::null_sink_mt>());
			break;
		case Direction::Console:
			g_logger = MakeLogger("prosperismo", std::make_shared<spdlog::sinks::stdout_sink_mt>());
			break;
		case Direction::File:
			if (!g_output_file.empty()) {
				g_logger = MakeFileLogger("prosperismo", g_output_file);
			}
			break;
	}
}
