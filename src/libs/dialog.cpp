...
constexpr int COMMON_DIALOG_ERROR_ALREADY_SYSTEM_INITIALIZED =
    static_cast<int>(0x80b80002u);

static std::atomic_bool g_common_dialog_initialized {false};

int KYTY_SYSV_ABI CommonDialogInitialize() {
	PRINT_NAME();

	bool expected = false;
	if (!g_common_dialog_initialized.compare_exchange_strong(expected, true,
	                                                         std::memory_order_relaxed)) {
		return COMMON_DIALOG_ERROR_ALREADY_SYSTEM_INITIALIZED;
	}
