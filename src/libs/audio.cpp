...
constexpr uint32_t AUDIO_OUT_OUTPUTS_MAX       = 33;
...
int KYTY_SYSV_ABI AudioOutOutputs(AudioOutOutputParam* param, uint32_t num) {
	...
	if (num > AUDIO_OUT_OUTPUTS_MAX) {
		return AUDIO_OUT_ERROR_PORT_FULL;
	}

	EXIT_NOT_IMPLEMENTED(param == nullptr);

	Audio::OutputParam params[AUDIO_OUT_OUTPUTS_MAX];
...
int KYTY_SYSV_ABI AudioInGetSilentState(int handle) {
	...
	// Audio input currently has no host capture device. Report that documented silent reason rather
	// than pretending the valid port is receiving audible input.
