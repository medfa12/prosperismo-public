...
static KYTY_SYSV_ABI KernelModule KernelLoadStartModule(const char* module_file_name, size_t args,
	...
	if (program != nullptr) {
		EXIT_IF(!rt->AddProgramLoadReference(program));
		if (res != nullptr) {
			*res = OK;
		}
...
static int KYTY_SYSV_ABI KernelStopUnloadModule(KernelModule handle, size_t args, const void* argp,
	...
	const auto release_action = rt->PrepareProgramUnload(program);
	if (release_action == Loader::RuntimeLinkerLifecycle::ModuleReleaseAction::Retained) {
		if (res != nullptr) {
			*res = OK;
		}
		return OK;
...
int KYTY_SYSV_ABI KernelGetModuleInfoForUnwind(uint64_t addr, int flags,
	...
	if (program == nullptr || program->elf == nullptr) {
		if (addr < 0x800000000ull) {
			...
			std::snprintf(info->name, sizeof(info->name), "%s", "ProsperismoHostBoundary");
...
SyncOnAddressRegistry& GetSyncOnAddressRegistry() {
	static SyncOnAddressRegistry registry;
	return registry;
}
...
uint64_t KYTY_SYSV_ABI KernelSyncOnAddressWaitV1(uint64_t op, uint64_t address, uint64_t value,
	...
	(void) flags;
	GetSyncOnAddressRegistry().Wait(address, timeout);
	return 0;
}

uint64_t KYTY_SYSV_ABI KernelSyncOnAddressWakeV1(uint64_t address, uint64_t count) {
	if (address == 0) {
		return static_cast<uint64_t>(LibKernel::KERNEL_ERROR_EINVAL);
	}
	GetSyncOnAddressRegistry().Wake(address, count);
	return 0;
}
...
LIB_DEFINE(InitLibKernel_1_Posix) {
	...
	LIB_FUNC("yDaWxUE50s0", Posix::pthread_mutexattr_getprotocol);
...
// Guest fibers may request stacks smaller than the HLE call path can safely use.  Those fibers
// run on a private replacement stack; keep its ownership in a process-wide side table so the
// allocation can be released when the guest finalizes the fiber.
static std::mutex                              g_fiber_oversized_stack_mutex;
static std::unordered_map<FiberObject*, void*> g_fiber_oversized_stacks;
...
static void FiberOversizedStackForget(FiberObject* fiber) {
	std::lock_guard lock(g_fiber_oversized_stack_mutex);
	auto            it = g_fiber_oversized_stacks.find(fiber);
	if (it != g_fiber_oversized_stacks.end()) {
		std::free(it->second);
		g_fiber_oversized_stacks.erase(it);
	}
}
...
static void* FiberOversizedStack(FiberObject* fiber, uint64_t* size_out) {
	...
	std::lock_guard lock(g_fiber_oversized_stack_mutex);
	auto&           slot = g_fiber_oversized_stacks[fiber];
	if (slot == nullptr) {
...
int32_t KYTY_SYSV_ABI FiberFinalize(FiberObject* fiber) {
	...
	// FiberFinalize is the ownership boundary for the host-side state that cannot fit in the
	// guest's fixed 256-byte SceFiber object.  The jump buffer and the replacement stack are both
	// safe to retire only after the state transition above proves that the fiber is idle.
	FiberJmpForget(fiber);
	FiberOversizedStackForget(fiber);
...
LIB_DEFINE(InitLibKernel_1_Pthread) {
	...
	LIB_FUNC("GoTmFeui+hQ", LibKernel::PthreadMutexattrGetprotocol);
	...
	LIB_FUNC(LibKernel::PTHREAD_RWLOCK_TIMEDRDLOCK_NID, LibKernel::PthreadRwlockTimedrdlock);
	LIB_FUNC(LibKernel::PTHREAD_RWLOCK_TIMEDWRLOCK_NID, LibKernel::PthreadRwlockTimedwrlock);
	LIB_FUNC(LibKernel::PTHREAD_RWLOCK_TRYRDLOCK_NID, LibKernel::PthreadRwlockTryrdlock);
	...
	LIB_FUNC(LibKernel::PTHREAD_RWLOCKATTR_GETTYPE_NID, LibKernel::PthreadRwlockattrGettype);
...
LIB_DEFINE(InitLibKernel_1) {
	...
	LIB_FUNC("fTx66l5iWIA", LibKernel::FileSystem::KernelFsync);
	LIB_FUNC("30Rh4ixbKy4", LibKernel::FileSystem::KernelFdatasync);
	LIB_FUNC("uvT2iYBBnkY", LibKernel::FileSystem::KernelSync);
	...
	LIB_FUNC("Hc4CaR6JBL0", Posix::KernelSyncOnAddressWaitV1);
	LIB_FUNC("q2y-wDIVWZA", Posix::KernelSyncOnAddressWakeV1);
