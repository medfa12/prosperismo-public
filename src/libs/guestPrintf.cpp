...
static int kyty_printf_internal(bool sn, char* sn_s, size_t sn_n, const char* format,
	...
	if (sn) {
		// snprintf returns the length that would have been written, even when the
		// destination is too small.  The previous host snprintf call treated normal
		// truncation as an emulator failure, which can divert guest libc callers into
		// an error/trap path before they reach their normal continuation.
		if (sn_s != nullptr && sn_n != 0u) {
			const size_t copy_size = idx < sn_n - 1u ? idx : sn_n - 1u;
			if (copy_size != 0u) {
				std::memcpy(sn_s, buffer.data(), copy_size);
			}
			sn_s[copy_size] = '\0';
		}
	} else {
