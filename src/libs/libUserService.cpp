...
static KYTY_SYSV_ABI int UserServiceInitialize(const void* /*params*/) {
	...
	if (g_initialized) {
		return USER_SERVICE_ERROR_ALREADY_INITIALIZED;
	}
...
static KYTY_SYSV_ABI int UserServiceInitialize2(int /*thread_priority*/,
...
static KYTY_SYSV_ABI int UserServiceTerminate() {
	...
	if (!g_initialized) {
		return USER_SERVICE_ERROR_NOT_INITIALIZED;
	}
...
static KYTY_SYSV_ABI int UserServiceGetUserName(int user_id, char* name, size_t size) {
	...
	int s = snprintf(name, size, "%s", "Prosperismo");
