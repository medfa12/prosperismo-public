...
#else
// The guest formatter consumes an explicitly materialized x86-64 SysV register
// save area.  Native arm64 builds still need that storage type even though the
// host compiler cannot include x86 SIMD intrinsics; keep each lane opaque and
// exactly 16 bytes, preserving the guest layout without claiming host ABI
// compatibility for the variadic entry point itself.
struct alignas(16) KytyVaFloat128 {
	uint64_t lo;
	uint64_t hi;
};
#endif
...
#define VA_ARGS                                                                                    \
	uint64_t rdi, uint64_t rsi, uint64_t rdx, uint64_t rcx, uint64_t r8, uint64_t r9,              \
	    uint64_t overflow_arg_area, KytyVaFloat128 xmm0, KytyVaFloat128 xmm1,                       \
	    KytyVaFloat128 xmm2, KytyVaFloat128 xmm3, KytyVaFloat128 xmm4, KytyVaFloat128 xmm5,         \
	    KytyVaFloat128 xmm6, KytyVaFloat128 xmm7, ...
