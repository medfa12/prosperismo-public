...
static int ConvertMessageFlags(int flags) {
	...
	flags &= ~(guest_msg_peek | guest_msg_dontroute | guest_msg_waitall | guest_msg_dontwait |
	           guest_msg_nosignal);
	if (!HostSocketMessageFlagsSupported(flags)) {
...
int KYTY_SYSV_ABI Socket(int family, int type, int protocol) {
	...
	return fd;
#else
	switch (ResolveHostSocketAction(family, type, protocol)) {
		case HostSocketAction::UnsupportedFamily:
			return SetPosixSocketError(Posix::POSIX_EAFNOSUPPORT);
		case HostSocketAction::UnsupportedType: return SetPosixSocketError(Posix::POSIX_EPROTOTYPE);
		case HostSocketAction::UnsupportedProtocol:
			return SetPosixSocketError(Posix::POSIX_EPROTONOSUPPORT);
		case HostSocketAction::CreateIpv4Datagram: break;
	}

	NativeSocket socket = ::socket(AF_INET, SOCK_DGRAM, 0);
	if (socket == INVALID_NATIVE_SOCKET) {
		return SetPosixSocketError();
	}
...
int KYTY_SYSV_ABI Bind(int s, const void* addr, uint32_t addrlen) {
	...
	if (::bind(socket, reinterpret_cast<const sockaddr*>(&host_addr),
	           static_cast<socklen_t>(host_addrlen)) != 0) {
		return SetPosixSocketError();
	}
...
int KYTY_SYSV_ABI Setsockopt(int s, int level, int optname, const void* optval, uint32_t optlen) {
	...
	int host_level   = 0;
	int host_optname = 0;
	switch (ResolveHostSocketOption(level, optname)) {
		case HostSocketOption::ReceiveBuffer:
			host_level   = SOL_SOCKET;
			host_optname = SO_RCVBUF;
			break;
		case HostSocketOption::Linger:
			host_level   = SOL_SOCKET;
			host_optname = SO_LINGER;
			break;
		case HostSocketOption::SendBuffer:
			host_level   = SOL_SOCKET;
			host_optname = SO_SNDBUF;
			break;
		case HostSocketOption::Broadcast:
			host_level   = SOL_SOCKET;
			host_optname = SO_BROADCAST;
			break;
		case HostSocketOption::Ipv4HeaderIncluded:
			host_level   = IPPROTO_IP;
			host_optname = IP_HDRINCL;
			break;
		case HostSocketOption::Unsupported: return SetPosixSocketError(Posix::POSIX_ENOPROTOOPT);
	}
...
#else
	if (!HostSocketMessageFlagsSupported(flags)) {
		return SetPosixSocketError(Posix::POSIX_EOPNOTSUPP);
	}

	const size_t host_len = static_cast<size_t>(len);
	ssize_t      result   = 0;
	if (addr == nullptr) {
		result = ::send(socket, buf, host_len, 0);
	} else {
		sockaddr_storage host_addr {};
		int              host_addrlen = 0;
		if (ConvertGuestSockaddr(addr, addrlen, &host_addr, &host_addrlen) != 0) {
		...
		result = ::sendto(socket, buf, host_len, 0, reinterpret_cast<const sockaddr*>(&host_addr),
		                  static_cast<socklen_t>(host_addrlen));
	}
	if (result < 0) {
		return SetPosixSocketError();
	}
...
int64_t KYTY_SYSV_ABI Recvfrom(int s, void* buf, uint64_t len, int flags, void* addr,
	...
	ssize_t      result   = 0;
	if (addr == nullptr) {
		result = ::recv(socket, buf, host_len, 0);
	} else {
		sockaddr_storage host_addr {};
		socklen_t        host_addrlen = sizeof(host_addr);
		result = ::recvfrom(socket, buf, host_len, 0, reinterpret_cast<sockaddr*>(&host_addr),
		                    &host_addrlen);
		if (result >= 0 &&
		    ConvertHostSockaddr(&host_addr, static_cast<int>(host_addrlen), addr, addrlen) != 0) {
			return -1;
		}
	}
	if (result < 0) {
