...
static int32_t ValidateDecoderConfig(const Videodec2DecoderConfigInfo* config,
	...
	const int32_t queue_depth_result =
	    ValidateInputQueueDepth(config->decode_input_queue_depth);
	if (queue_depth_result != OK) {
		return queue_depth_result;
	}
...
static int32_t KYTY_SYSV_ABI Decode(Videodec2Decoder decoder, const Videodec2InputData* input_data,
	...
	const auto alignment_result =
	    ValidateFrameBufferAlignment(reinterpret_cast<uintptr_t>(frame_buffer->frame_buffer));
