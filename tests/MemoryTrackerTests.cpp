...
uint32_t Protection(const void *address) {
  ...
  if (result != KERN_SUCCESS || addr < region || addr >= region + size) {
    return 0;
  }
  return (info.protection & VM_PROT_WRITE) != 0  ? PAGE_READWRITE
...
#if defined(__APPLE__)
  const int extra = address != nullptr ? MAP_FIXED : 0;
#else
...
bool ProtectAddressSpace(uint64_t vaddr, uint64_t size,
  ...
  if (mode == Common::VirtualMemory::Mode::Read ||
      mode == Common::VirtualMemory::Mode::ExecuteRead) {
    protection = PAGE_READONLY;
  } else if (mode == Common::VirtualMemory::Mode::ReadWrite ||
             mode == Common::VirtualMemory::Mode::ExecuteReadWrite) {
...
void TestCleanHostWritePreparation() {
  TrackerHarness harness;
  auto &tracker = harness.tracker;
  auto &page_manager = harness.page_manager;
  const auto page_size = page_manager.GetPageSize();
  Check(!tracker.TryMarkCleanRegionAsCpuModified(0x0000000203000010ull, 8),
        "an untracked host write was accepted as clean");
  auto *memory = Allocate(page_manager, 1);
  const auto address = reinterpret_cast<uint64_t>(memory);

  Check(!tracker.TryMarkCleanRegionAsCpuModified(address + 16, 8),
        "an already CPU-dirty host write was accepted as clean");
  tracker.ForEachUploadRange(
  ...
  Check(!tracker.IsRegionCpuModified(address, page_size) &&
            ...
            Protection(memory) == PAGE_READONLY,
        "clean host-write setup did not arm the write watcher");
  Check(tracker.TryMarkCleanRegionAsCpuModified(address + 16, 8) &&
            tracker.IsRegionCpuModified(address, page_size) &&
            IsWritable(memory),
        "a clean tracked host write did not release its watcher");

  tracker.ForEachUploadRange(
      address, page_size, true, [](uint64_t, uint64_t) noexcept {},
      []() noexcept {});
  Check(tracker.IsRegionGpuModified(address, page_size) &&
            !tracker.TryMarkCleanRegionAsCpuModified(address + 16, 8) &&
            Protection(memory) == PAGE_NOACCESS,
        "a GPU-owned host write was incorrectly accepted as clean");
...
void TestGpuDownloadProtectionMirrors() {
  ...
  Check(visited.size() == 1 && visited[0].address == address &&
            ...
            g_protection_log[0].mode == Common::VirtualMemory::Mode::ExecuteRead &&
            !tracker.IsRegionGpuModified(address, page_size) &&
  ...
  Check(
      g_protection_log.size() == 1 && g_protection_log[0].address == address &&
          g_protection_log[0].size == page_size &&
          g_protection_log[0].mode == Common::VirtualMemory::Mode::ExecuteReadWrite &&
...
void TestGpuUnmarkUsesRegionMask() {
  ...
  Check(g_protection_calls == 2 && g_protection_log.size() == 2 &&
            ...
            g_protection_log[1].mode == Common::VirtualMemory::Mode::ExecuteRead &&
...
void CheckDeathCase(const char *name) {
  ...
  if (pid == 0) {
    ...
    (void)_NSGetExecutablePath(nullptr, &path_size);
...
int main(int argc, char **argv) {
  ...
  if (argc == 2 && std::strcmp(argv[1], "--clean-host-write-only") == 0) {
    TestCleanHostWritePreparation();
    std::puts("MemoryTrackerTests: clean host-write cases passed");
    return 0;
  }
  TestRangeSet();
