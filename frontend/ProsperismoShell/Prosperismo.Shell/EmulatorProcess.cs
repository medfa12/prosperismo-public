...
/// <summary>
/// Pure input to the Windows process-creation path. Kept separate from Win32
/// handles so its command and environment contract can be verified on every
/// development platform.
/// </summary>
internal sealed record WindowsProcessStartPlan(
    IReadOnlyList<string> Arguments,
    IReadOnlyDictionary<string, string> Environment,
    bool UsesMitigatedChildProtocol);
...
internal sealed class EmulatorProcess : IDisposable
{
    ...
    private const string MitigatedChildFlag = "--prosperismo-mitigated-child";
    private const string MitigatedChildEnvironment = "PROSPERISMO_MITIGATED_CHILD";
    private const string LegacyMitigatedChildExecutableStem = "Prosperismo";
        ...
        IReadOnlyDictionary<string, string>? environment = null)
    {
        ...
        {
            StartWindows(exePath, arguments, workingDirectory, environment);
            return;
        }

        StartFallback(exePath, arguments, workingDirectory, environment);
    }
    ...
    private unsafe void StartWindows(
        string exePath,
        IReadOnlyList<string> arguments,
        string? workingDirectory,
        IReadOnlyDictionary<string, string>? environment)
    {
        ...
        try
        {
            var startPlan = BuildWindowsStartPlan(
                exePath,
                arguments,
                environment,
            ...
            var commandLine = new StringBuilder(BuildCommandLine(exePath, startPlan.Arguments));
            environmentBlock = AllocateWindowsEnvironmentBlock(startPlan.Environment);
    ...
    private void StartPipeReader(nint handle, bool isError)
    {
        ...
        {
            IsBackground = true,
            Name = isError ? "Prosperismo stderr reader" : "Prosperismo stdout reader",
        };
        readerThread.Start();
    }
    ...
    /// <summary>
    /// Builds the complete Windows-only process input without calling Win32.
    /// The marker is a legacy host protocol, not a process-mitigation switch:
    /// native <c>prosperismo_emulator</c> intentionally never receives it.
    /// The actual Windows mitigation policy remains enabled for all isolated
    /// emulator children.
    /// </summary>
    internal static WindowsProcessStartPlan BuildWindowsStartPlan(
        string executablePath,
        IReadOnlyList<string> arguments,
        IReadOnlyDictionary<string, string>? suppliedEnvironment,
        IEnumerable<KeyValuePair<string, string>> inheritedEnvironment)
    {
        ...
        var childArguments = new List<string>(arguments);
        ...
        if (suppliedEnvironment is not null)
        {
            foreach (var pair in suppliedEnvironment)
            {
                ValidateEnvironmentEntry(pair.Key, pair.Value);
                childEnvironment[pair.Key] = pair.Value;
            }
        ...
        var usesMitigatedChildProtocol = SupportsMitigatedChildProtocol(executablePath);
        if (usesMitigatedChildProtocol)
        {
            childArguments.Insert(0, MitigatedChildFlag);
            childEnvironment[MitigatedChildEnvironment] = "1";
        }

        return new WindowsProcessStartPlan(
            childArguments,
            childEnvironment,
            usesMitigatedChildProtocol);
    }

    internal static string BuildCommandLine(string processPath, IReadOnlyList<string> arguments)
    {
    ...
    private static IReadOnlyDictionary<string, string> CaptureCurrentEnvironment()
    {
    ...
    private static nint AllocateWindowsEnvironmentBlock(IReadOnlyDictionary<string, string> environment)
    {
        ...
        // CreateProcessW requires a double-null-terminated Unicode block.
        builder.Append('\0');
        return Marshal.StringToHGlobalUni(builder.ToString());
    }

    private static bool SupportsMitigatedChildProtocol(string executablePath)
    {
        // Do not infer support from a catch-all "non-native" category. Only
        // the historical managed host explicitly understood this protocol.
        // This helper is deliberately testable on non-Windows hosts, where
        // Path.GetFileNameWithoutExtension does not treat '\\' as a separator.
        var fileNameStart = executablePath.LastIndexOfAny(['\\', '/']) + 1;
        var fileName = executablePath[fileNameStart..];
        var extensionStart = fileName.LastIndexOf('.');
        var executableStem = extensionStart > 0
            ? fileName[..extensionStart]
            : fileName;
        return string.Equals(
            executableStem,
            LegacyMitigatedChildExecutableStem,
            StringComparison.OrdinalIgnoreCase);
    }

    private static void ValidateEnvironmentEntry(string? key, string? value)
    {
        if (string.IsNullOrEmpty(key) || key.IndexOf('\0') >= 0 || value is null || value.IndexOf('\0') >= 0)
        {
            throw new ArgumentException("Environment variables must have a non-empty name and cannot contain null characters.");
        }
    }
