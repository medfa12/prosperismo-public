...
{
    private const string ApplyArgument = "--prosperismo-apply-update";
    private const string LatestReleaseUrl = "https://api.github.com/repos/medfa12/prosperismo/releases/latest";
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "Prosperismo.Update");
    ...
    public static bool TryApply(string[] args, out int exitCode)
    {
        ...
        var backup = Path.Combine(Path.GetTempPath(), $"Prosperismo.UpdateBackup-{Environment.ProcessId}");
        ...
        try
        {
            if (int.TryParse(args[1], out var oldPid))
            {
                try
                {
                    ...
                    {
                        throw new TimeoutException("Prosperismo did not close within 30 seconds.");
                    }
                }
            ...
            foreach (var file in Directory.EnumerateFiles(source, "*", SearchOption.AllDirectories))
            {
                ...
                if (relative.Equals("gui-settings.json", StringComparison.OrdinalIgnoreCase) ||
                    relative.Equals("notification-history.json", StringComparison.OrdinalIgnoreCase) ||
            ...
            }) ?? throw new InvalidOperationException("The updated Prosperismo could not be started.");
    ...
    {
        var url = $"https://api.github.com/repos/medfa12/prosperismo/compare/{currentSha}...{releaseSha}";
    ...
    private static HttpClient CreateHttpClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("Prosperismo", "0.0.1"));
    ...
    private static PlatformInfo CurrentPlatform()
    {
        ...
        {
            throw new PlatformNotSupportedException("Prosperismo releases require an x64 process.");
        }

        if (OperatingSystem.IsWindows()) return new("win-x64", ".zip", "Prosperismo.exe");
        if (OperatingSystem.IsLinux()) return new("linux-x64", ".tar.gz", "Prosperismo");
        if (OperatingSystem.IsMacOS()) return new("osx-x64", ".tar.gz", "Prosperismo");
