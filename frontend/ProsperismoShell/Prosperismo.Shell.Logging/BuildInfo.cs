...
{
    private const string ProjectUrl = "https://github.com/medfa12/prosperismo";
    private const string CanonicalRepository = "medfa12/prosperismo";
    ...
    static BuildInfo()
    {
        ...
        CommitSha = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildSha"));
        if (CommitSha is { Length: > 7 })
        {
            CommitSha = CommitSha[..7];
        }

        Branch = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildBranch"));
        Repository = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildRepository"));
        Configuration = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildConfiguration"));

        var serverUrl = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildServerUrl"));
        var runId = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildRunId"));
        ...
        var eventName = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildEventName"));
        var gitRef = Normalize(metadata.GetValueOrDefault("Prosperismo.BuildRef"));
    ...
    /// <code>
    /// Prosperismo UNOFFICIAL f11ac59 — https://github.com/medfa12/prosperismo
    ///
    /// Built from branch "main" of "medfa12/prosperismo" by GitHub Actions workflow run https://github.com/medfa12/prosperismo/actions/runs/123.
    /// </code>
    ...
    public static string Banner
    {
        get
        {
            ...
            var header = $"Prosperismo {version} — {ProjectUrl}";
    ...
    private static Dictionary<string, string> ReadMetadata()
    {
        ...
        foreach (var assembly in assemblies)
        {
            ...
            {
                if (attribute.Key.StartsWith("Prosperismo.Build", StringComparison.Ordinal) &&
