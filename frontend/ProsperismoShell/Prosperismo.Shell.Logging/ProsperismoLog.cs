...
public static class ProsperismoLog
{
    private static readonly ConcurrentDictionary<string, ProsperismoLogger> LoggerByCategory =
    ...
    private static IProsperismoLogSink _sink = ResolveSinkFromEnvironment();

    /// <summary>
    /// Entries below this level are dropped. When a PROSPERISMO_LOG_FILE sink is
    ...
    public static IProsperismoLogSink Sink
    {
        get
        {
    ...
    public static void Configure(LogLevel? minimumLevel = null, IProsperismoLogSink? sink = null)
    {
    ...
    public static ProsperismoLogger For(string category)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(category);
        return LoggerByCategory.GetOrAdd(category, static key => new ProsperismoLogger(key));
    }
    ...
    {
        var raw = Environment.GetEnvironmentVariable("PROSPERISMO_LOG_LEVEL");
    ...
    private static bool ResolveColorEnabledFromEnvironment()
    {
        ...
        var raw = Environment.GetEnvironmentVariable("PROSPERISMO_LOG_NO_COLOR");
        return !IsTrueLike(raw);
    }

    private static IProsperismoLogSink ResolveSinkFromEnvironment()
    {
        var consoleSink = new ConsoleLogSink(
            useColors: ResolveColorEnabledFromEnvironment(),
            includeTimestamp: false);

        var logFilePath = Environment.GetEnvironmentVariable("PROSPERISMO_LOG_FILE");
        if (!string.IsNullOrWhiteSpace(logFilePath))
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.Error.WriteLine($"[PROSPERISMO_LOG] Failed to open log file '{logFilePath}': {ex.Message}");
            }
        }
    ...
    /// </summary>
    private sealed class MinimumLevelFilterSink : IProsperismoLogSink
    {
        private readonly IProsperismoLogSink _inner;

        internal MinimumLevelFilterSink(IProsperismoLogSink inner) => _inner = inner;
