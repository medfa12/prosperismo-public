...
public static class WindowsDualSenseReader
{
    ...
    private static bool TryParseReport(ReadOnlySpan<byte> report, out HostGamepadState state)
    {
        ...
        var buttons = DecodeButtons(buttons0, buttons1, buttons2);
    ...
    /// <summary>
    /// Decodes the three standard DualSense button bytes. Linux
    /// hid-playstation.c confirms DS_BUTTONS1_CREATE as BIT(4) and
    /// DS_BUTTONS2_PS_HOME as BIT(0).
    /// </summary>
    internal static HostGamepadButtons DecodeButtons(byte buttons0, byte buttons1, byte buttons2)
    {
        ...
        buttons |= (buttons1 & 0x10) != 0 ? HostGamepadButtons.Create : 0;
        ...
        buttons |= (buttons2 & 0x01) != 0 ? HostGamepadButtons.PsButton : 0;
        return buttons;
    }
