...
namespace Prosperismo.HLE.Host.Windows;
...
public static partial class WindowsXInputReader
{
    ...
    // XInputGetStateEx (ordinal 100) exposes the Guide bit that the public
    // XInputGetState entry point masks. Prosperismo treats it as the host
    // equivalent of the PS button; it is never forwarded as a guest ABI bit.
    ...
    private static bool _stateExUnavailable;
    ...
    private static void ReadLoop()
    {
        try
        {
            while (true)
            {
                ...
                while (GetState((uint)slot, out var state) == ErrorSuccess)
                {
    ...
    private static int FindConnectedSlot()
    {
        ...
        {
            if (GetState((uint)index, out _) == ErrorSuccess)
            {
                return index;
            }
        }
    ...
    {
        var buttons = DecodeButtons(pad.Buttons, pad.LeftTrigger, pad.RightTrigger);
    ...
    /// <summary>
    /// Translates the XInput button word into the host-neutral shell seam.
    /// Kept separate from stick translation so the Guide mapping is testable
    /// on non-Windows build hosts where XInput itself cannot be loaded.
    /// </summary>
    internal static HostGamepadButtons DecodeButtons(
        ushort xinputButtons,
        byte leftTrigger = 0,
        byte rightTrigger = 0)
    {
        var buttons = HostGamepadButtons.None;
        buttons |= (xinputButtons & XinputDpadUp) != 0 ? HostGamepadButtons.Up : 0;
        buttons |= (xinputButtons & XinputDpadDown) != 0 ? HostGamepadButtons.Down : 0;
        buttons |= (xinputButtons & XinputDpadLeft) != 0 ? HostGamepadButtons.Left : 0;
        buttons |= (xinputButtons & XinputDpadRight) != 0 ? HostGamepadButtons.Right : 0;
        buttons |= (xinputButtons & XinputStart) != 0 ? HostGamepadButtons.Options : 0;
        buttons |= (xinputButtons & XinputBack) != 0 ? HostGamepadButtons.TouchPad : 0;
        buttons |= (xinputButtons & XinputLeftThumb) != 0 ? HostGamepadButtons.L3 : 0;
        buttons |= (xinputButtons & XinputRightThumb) != 0 ? HostGamepadButtons.R3 : 0;
        buttons |= (xinputButtons & XinputLeftShoulder) != 0 ? HostGamepadButtons.L1 : 0;
        buttons |= (xinputButtons & XinputRightShoulder) != 0 ? HostGamepadButtons.R1 : 0;
        buttons |= (xinputButtons & XinputGuide) != 0 ? HostGamepadButtons.PsButton : 0;
        buttons |= (xinputButtons & XinputA) != 0 ? HostGamepadButtons.Cross : 0;
        buttons |= (xinputButtons & XinputB) != 0 ? HostGamepadButtons.Circle : 0;
        buttons |= (xinputButtons & XinputX) != 0 ? HostGamepadButtons.Square : 0;
        buttons |= (xinputButtons & XinputY) != 0 ? HostGamepadButtons.Triangle : 0;
        buttons |= leftTrigger > TriggerThreshold ? HostGamepadButtons.L2 : 0;
        buttons |= rightTrigger > TriggerThreshold ? HostGamepadButtons.R2 : 0;
        return buttons;
    }

    private static uint GetState(uint userIndex, out XInputState state)
    {
        if (!_stateExUnavailable)
        {
            try
            {
    ...
    // The extended state call is exported by ordinal and includes the Guide
    // button in wButtons. Fall back to the public entry point when an alternate
    // XInput implementation does not provide it.
    [LibraryImport("xinput1_4.dll", EntryPoint = "#100")]
    private static partial uint XInputGetStateEx(uint userIndex, out XInputState state);
