...
public static class GuestWriteWatch
{
    ...
    private static readonly ulong WatchBase = Parse(
        Environment.GetEnvironmentVariable("PROSPERISMO_WATCH_WRITE"));

    private static readonly bool WatchPoolHeaders = IsEnabled("PROSPERISMO_WATCH_POOL_HEADER");

    private static readonly ulong[] PoolSlots = new ulong[64];
    private static int _poolSlotCount;

    private static readonly bool WatchValuePattern = IsEnabled("PROSPERISMO_WATCH_VALUE_PATTERN");

    private static readonly bool WatchValue1 = IsEnabled("PROSPERISMO_WATCH_VALUE1");
    ...
    private static readonly bool WatchBulkTorn = IsEnabled("PROSPERISMO_WATCH_BULK_TORN");

    private static readonly ulong BulkDestHigh = Parse(
        Environment.GetEnvironmentVariable("PROSPERISMO_WATCH_BULK_DEST_HI"));
    private static int _bulkTornReports;
    private static int _bulkShiftReports;

    // The effect census counts bytes rather than inspecting them, and it needs the
    // same "a managed write just landed" notification the watches use, so it arms
    // this too. Read per managed guest write, so it stays a single flag read.
    public static bool Armed =>
        WatchBase != 0 || WatchPoolHeaders || WatchValuePattern || WatchValue1 || WatchBulkTorn ||
        Diagnostics.HleEffectCensus.CountsGuestWrites;
    ...
    {
        Diagnostics.HleEffectCensus.NoteGuestWrite(data.Length);
