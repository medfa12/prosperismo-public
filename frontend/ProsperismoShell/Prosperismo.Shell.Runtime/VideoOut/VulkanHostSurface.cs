...
{
    // SEAM: in the donor this forwarded to the in-process C# renderer's
    // VulkanVideoPresenter. Prosperismo runs each game in a separate C++
    // emulator process, so game frames arrive over that process boundary
    // instead. Until that transport exists, no frame is ever presented and
    // the shell correctly keeps showing its launch affordance.
    //
    // The surface registry below is real: hosts attach and detach normally,
    // so the shell's own lifetime handling is exercised.
    private static readonly HashSet<VulkanHostSurface> Attached = new();
    private static readonly object Gate = new();

    /// <summary>
    /// Raised after the first game frame reaches an embedded host surface.
    /// Never raised yet - see the seam note above.
    /// </summary>
    ...
    /// <summary>
    /// True once game frames are routed into a host surface. False until the
    /// C++ emulator frame transport is implemented.
    /// </summary>
    public static bool IsEmbedded => false;

    /// <summary>Invoked by the future frame transport.</summary>
    internal static void RaiseFirstFramePresented(VulkanHostSurface surface) =>
        FirstFramePresented?.Invoke(surface);
}
