...
public sealed record Gen5BufferMemoryControl(
    ...
    bool Slc) : Gen5InstructionControl
{
    public uint? InstructionFormat { get; init; }
}
    ...
    bool MergedTailCall = false)
{
    ...
    private IReadOnlySet<uint> ComputeRuntimeScalarRegisters()
    {
        ...
        foreach (var instruction in Instructions)
        {
            ...
            // Wave-mask writers - compares with an explicit sdst (VOP3/SDWA)
            // and the VOP3B carry-out - write the named SGPR and, in wave64,
            // also register+1. The wave width is a translation-time value the
            // decoded program cannot see, so both halves are marked runtime;
            // over-marking only makes the scalar evaluator more conservative.
            uint? maskRegister = instruction.Control switch
            {
                Gen5Vop3Control { ScalarDestination: { } carry } => carry,
                Gen5SdwaControl { ScalarDestination: { } mask } => mask,
                _ => null,
            };
            if (maskRegister is null &&
                instruction.Opcode.StartsWith("VCmp", StringComparison.Ordinal) &&
                instruction.Destinations is
                    [{ Kind: Gen5OperandKind.ScalarRegister } compareDestination])
            {
                maskRegister = compareDestination.Value;
            }

            if (maskRegister is { } maskPair)
            {
                if (maskPair < ScalarRegisterCount)
                {
                    registers.Add(maskPair);
                }

                if (maskPair + 1 < ScalarRegisterCount)
                {
                    registers.Add(maskPair + 1);
                }
            }
        }
