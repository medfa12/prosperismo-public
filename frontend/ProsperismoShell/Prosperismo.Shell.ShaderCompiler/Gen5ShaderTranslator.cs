...
namespace Prosperismo.ShaderCompiler;
...
public static class Gen5ShaderTranslator
{
    ...
    /// shader. Used by the PROSPERISMO_DUMP_SHADER_ADDR boot probe to capture the
    ...
    // the runtime EXEC probe (PROSPERISMO_FORCE_PACKED_STORE_EXEC_VALUES): if the
    ...
    // Shared with DecodeVop3: every VOP1 operation is also legal in the
    // VOP3-encoded form at opcode 0x180+op, and both encodings must agree on
    // the mnemonic, so the table lives in one place.
    ...
    // Shared with DecodeVop3: VOP2 operations are also legal in the
    // VOP3-encoded form at opcode 0x100+op, except the literal-carrying and
    // carry forms DecodeVop3 handles explicitly. One table, one mnemonic.
    ...
    // Shared with DecodeVop3: every compare is also legal in the VOP3-encoded
    // form at the same opcode (0x000-0x0FF), which the encoding rules force
    // whenever sdst is not VCC or a source carries a modifier.
        ...
        out string error)
    {
        ...
        name = isVop3B
            ...
            : opcode switch
        {
            // The RDNA2 encoding rules force ordinary compiler output into
            // VOP3: any compare whose sdst is not VCC or whose source carries
            // a modifier is VOP3-encoded VOPC (0x000-0x0FF), and VOP2/VOP1
            // operations with scalar operands or modifiers land at 0x100+op /
            // 0x180+op. Resolve those ranges through the base-encoding name
            // tables so each mnemonic has one source of truth, then fall
            // through to the VOP3-only operations.
            < 0x100 => VopcName(opcode),
            // v_madmk/v_madak/v_fmamk/v_fmaak exist only as VOP2 literal
            // forms, and v_dot2c_f32_f16 has no VOP3 row in the public
            // opcode table. Those slots stay undecoded rather than aliasing
            // a neighbouring operation.
            0x102 or 0x120 or 0x121 or 0x12C or 0x12D => string.Empty,
            >= 0x100 and < 0x140 => Vop2Name(opcode - 0x100),
            >= 0x180 and < 0x200 => Vop1Name(opcode - 0x180),
            0x141 => "VMadF32",
            0x143 => "VMadU32U24",
            0x144 => "VCubeidF32",
            0x145 => "VCubescF32",
        ...
        // An opcode with no table entry is a named decode error here, never a
        // placeholder that passes decode and drops the whole shader later at
        // SPIR-V emission with no opcode identity in the diagnostic.
        return FinishDecode(
            name,
            isVop3B
                ? $"unknown-vop3b op=0x{opcode:X3}"
                : $"unknown-vop3 op=0x{opcode:X3}",
            out error);
    }

    private static bool IsVop3BOpcode(uint opcode) =>
        opcode is 0x128 or 0x129 or 0x12A or 0x16D or 0x16E or 0x176 or 0x177
            or 0x30F or 0x310 or 0x319;
        ...
        out string error)
    {
        ...
        name = opcode switch
        {
            ...
            0x37 => "DsRead2B32",
            0x38 => "DsRead2St64B32",
            // DIFFERENTIAL: Kyty PS5 MemoryOps.cpp and shadPS4 opcodes.h.
            0x3E => "DsAppend",
            0x3F => "DsOrderedCount",
            0x4D => "DsWriteB64",
            // DIFFERENTIAL: RDNA2 table and acelogic Gen5ShaderTranslator.cs.
            0xB2 => "DsPermuteB32",
            // ASSUMED: RDNA2 table; no Prospero numeric diagram is available.
            0xB3 => "DsBpermuteB32",
            0xDE => "DsWriteB96",
            0xDF => "DsWriteB128",
            0xFE => "DsReadB96",
    ...
    private static bool DecodeSmem(uint word, out string name, out uint sizeDwords, out string error)
    {
        ...
        name = opcode switch
        {
            ...
            0x0B => "SBufferLoadDwordx8",
            0x0C => "SBufferLoadDwordx16",
            // EXTRACTED: SDK 12.000 Scalar Memory Utility Operations lists
            // s_memrealtime; Prospero's gfx1013 encoding is 0x25, corroborated
            // by the SDK compiler output embedded in the 4.03 slide shader.
            0x25 => "SMemRealtime",
            _ => string.Empty,
        };
    ...
    {
        // EXTRACTED: SDK 12.000 Specification, ===== 25.pdf p5 =====.
        // OPM at word0 bit 0 is opcode bit 7 above the seven-bit OP field.
        ...
        uint[] words)
    {
        ...
        switch (encoding)
        {
            ...
            case Gen5ShaderEncoding.Smem:
            {
                ...
                {
                    // Unlike the ordinary SMEM family, s_memrealtime has no
                    // buffer source. The sbase-shaped field is reserved and
                    // SDATA names the 64-bit destination (the NPXS40087 ripple
                    // shader uses vcc, s106:s107). Treating it as a zero-width
                    // scalar load invents a guest buffer from whatever values
                    // happen to be in the reserved source field.
                    sources = [];
                    destinations =
                    [
            ...
            case Gen5ShaderEncoding.Vop3:
            {
                ...
                if (vop3Opcode < 0x100)
                {
                    // VOP3-encoded VOPC: the vdst byte carries the scalar
                    // destination of the compare mask, which the encoding
                    // rules make anything other than VCC only through this
                    // form. (VCMPX still writes EXEC and ignores the field.)
                    destinations = [Gen5Operand.Scalar(word & 0xFF)];
                }
                else if (opcode is "VReadlaneB32" or "VReadfirstlaneB32")
                {
                    // V_READLANE and the promoted V_READFIRSTLANE use the
                    // VOP3A vdst byte even though the destination register is
                    // scalar. Bits 8-14 are the distinct sdst field used by
                    // VOP3B encodings.
                    destinations = [Gen5Operand.Scalar(word & 0xFF)];
                }
            ...
            case Gen5ShaderEncoding.Ds:
            {
                ...
                sources = opcode switch
                {
                    ...
                    // ds_swizzle_b32 is a DS_1A form: its single input lives in
                    // the ADDR field, not DATA0 (LLVM DS_1A_RET_NoM0; shadPS4
                    // frontend/translate/data_share.cpp:302 also reads
                    // inst.src[0], which its DS decode fills from ADDR - see
                    // DS_READ at data_share.cpp:253). It touches no LDS memory.
                    "DsSwizzleB32" => [Gen5Operand.Vector(vectorAddress)],
                    // ds_permute_b32/ds_bpermute_b32 are DS_1A1D: ADDR carries
                    // the per-lane byte-addressed lane selector and DATA0 the
                    // value being moved (LLVM DS_1A1D_PERMUTE). Without DATA0
                    // the emitter had no value operand at all.
            ...
            case Gen5ShaderEncoding.Mtbuf:
            {
                ...
                var dwordCount = opcode switch
                {
                    ...
                    // The store forms carry the same component count as their
                    // load twins. Without these the control reported zero dwords
                    // and the emitter's store loop would have written nothing.
                ...
                control = new Gen5BufferMemoryControl(
                    ...
                    ((extra >> 22) & 1) != 0)
                {
                    // EXTRACTED: SDK 12.000 Specification,
                    // ===== 25.pdf p5 =====.
                    InstructionFormat = (word >> 19) & 0x7F,
                };
                break;
            }
