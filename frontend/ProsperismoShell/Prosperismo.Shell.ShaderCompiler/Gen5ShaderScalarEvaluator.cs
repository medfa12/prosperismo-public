...
public static class Gen5ShaderScalarEvaluator
{
    ...
    private static readonly bool _strictScalarLoad =
        string.Equals(
            Environment.GetEnvironmentVariable("PROSPERISMO_STRICT_SCALAR_LOAD"),
            "1",
    ...
    private static readonly bool _strictBufferLoad =
        string.Equals(
            Environment.GetEnvironmentVariable("PROSPERISMO_STRICT_BUFFER_LOAD"),
            "1",
            StringComparison.Ordinal);
    ...
    // descriptors by default; PROSPERISMO_CFG_RESOURCE_DISCOVERY=0 is a diagnostic
    ...
    private static readonly bool _cfgResourceDiscovery =
        !string.Equals(
            Environment.GetEnvironmentVariable("PROSPERISMO_CFG_RESOURCE_DISCOVERY"),
            "0",
            StringComparison.Ordinal);
    ...
    {
        if (!string.Equals(
                Environment.GetEnvironmentVariable("PROSPERISMO_TRACE_VERTEX_RAW"),
                "1",
                StringComparison.Ordinal) ||
            bindings.Count != 3 ||
    ...
    /// <summary>
    /// Smallest span the scalar evaluator will attempt for a global-memory read.
    /// </summary>
    /// <remarks>
    /// A descriptor fetch is tens of bytes, not kilobytes. The floor only exists so a
    /// wildly wrong pointer does not succeed on a stray readable byte.
    /// </remarks>
    private const int MinGlobalMemoryReadBytes = 64;
        ...
        out int dataLength)
    {
        ...
        // Keep halving past a page. Every attempt needs the WHOLE span readable from
        // the exact base, so stopping at 4096 failed any buffer smaller than a page or
        // merely close enough to the end of its mapping that 4 KB overran it - and it
        // reported read-failed exactly as though the address were bogus. Superliminal's
        // composite shaders load extended user data from an unaligned base and need
        // about 128 bytes; that read failed here, left s72.. empty, and every descriptor
        // built from it was garbage, which is why the menu background never composited.
        for (var size = MaxGlobalMemoryBindingBytes; size >= MinGlobalMemoryReadBytes; size >>= 1)
        {
        ...
        // Nothing readable at any span. "read-failed" alone cannot say whether the
        // address is unmapped or merely short, and those need different fixes, so probe
        // a single byte: if even that misses, the address is not mapped at this moment
        // and the fault is mapping lifetime or the address-space view, not extent.
        Span<byte> probe = stackalloc byte[1];
        var singleByteReadable = ctx.Memory.TryRead(baseAddress, probe);
        ReportGlobalMemoryReadFailure(baseAddress, singleByteReadable);
    ...
    private static readonly HashSet<ulong> _reportedGlobalReadFailures = new();

    private static void ReportGlobalMemoryReadFailure(ulong baseAddress, bool singleByteReadable)
    {
        lock (_reportedGlobalReadFailures)
        {
            if (_reportedGlobalReadFailures.Count >= 64 ||
                !_reportedGlobalReadFailures.Add(baseAddress))
            {
                return;
            }
        }
        ...
        Console.Error.WriteLine(
            $"[LOADER][WARN] agc.global_read_failed base=0x{baseAddress:X16} " +
            $"single_byte_readable={singleByteReadable} " +
            $"min_span={MinGlobalMemoryReadBytes} - " +
            (singleByteReadable
                ? "mapped but too short for any attempted span"
                : "not mapped at evaluation time"));
    }
        ...
        out string error)
    {
        ...
        // s_ashr_i64 is a 64-bit shift whose name ends in I64, so it never
        // reached the B64 grouping above and fell through to the 32-bit switch,
        // which rejected it and failed the whole evaluation.
        if (instruction.Opcode is "SLshlB64" or "SLshrB64" or "SAshrI64")
        {
            ...
            value = instruction.Opcode switch
            {
                ...
                // Arithmetic: sign-replicating, so shift as signed.
                _ => unchecked((ulong)((long)value >> shiftAmount)),
            };
            WriteScalarPair(registers, destination.Value, value, ref execMask);
            scalarConditionCode = value != 0;
            return true;
        ...
        out string error)
    {
        ...
        // EXTRACTED: SDK 12.000 Specification, ===== 48.pdf p3 =====.
        // DIFFERENTIAL: inspiration/KytyPS5 spirvEmitterMemory.cpp:139-145.
        var byteOffset = isBufferLoad
            ? unchecked(
                (uint)(
                    (dynamicOffset & ~3u) +
                    (unchecked((uint)control.ImmediateOffsetBytes) & ~3u)))
            : unchecked(immediateOffset + dynamicOffset);
        var address = isBufferLoad
            ? unchecked((baseAddress & ~3UL) + byteOffset)
            : unchecked(baseAddress + byteOffset) & ~3UL;
        var bufferUnbound =
            isBufferLoad &&
            (!hasBufferDescriptor ||
        ...
        for (var index = 0; index < instruction.Destinations.Count; index++)
        {
                    ...
                    out var value))
            {
                if (!_strictScalarLoad)
                {
                    ...
                    //
                    // Buffer loads used to take this path even under strict mode,
                    // which is the worst case rather than the safe one: an
                    // s_buffer_load_dwordx16 that misses zero-fills 16 SGPRs, and
                    // when those SGPRs are a descriptor table that hands the GPU
                    // four null V#s. Astro Bot loses the device on exactly that -
                    // shader 0x5008F0900 reports read-failed at pc=0x124C and the
                    // next QueueSubmit takes the device down with it. Strict mode
                    // now covers them so the draw is refused instead.
                    TraceScalarPointerFallback(
                        "read-failed",
                        state,
                        instruction,
