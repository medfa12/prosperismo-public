...
public static partial class Gen5SpirvTranslator
{
    ...
    {
        // DIFFERENTIAL: Khronos GLSL.std.450 assigns NMin and NMax 79 and 80.
        private const uint GlslNMin = 79;
        private const uint GlslNMax = 80;
            ...
            out string error)
        {
            ...
            if (instruction.Opcode is "VPermlane16B32" or "VPermlanex16B32" &&
                _waveLaneCount == 64)
            {
                // EmitPermlane16 derives the target lane as
                // ((GuestWaveLane() & ~15) + selector) & 31. In a wave64
                // program GuestWaveLane() runs 0..63, so for the upper half the
                // row base is 32 or 48 and the trailing "& 31" - which is there
                // only to keep the OpGroupNonUniformShuffle id inside a 32-lane
                // host subgroup - silently folds it onto the LOW half's row.
                // Lanes 32..63 would then permute against a different row of a
                // different half-wave. Refuse rather than emit that.
                ReportDiagnosticOnce(
                    $"permlane-wave64:{instruction.Opcode}",
                    ...
                    "the 16-lane row base of guest lanes 32..63 has to be " +
                    "folded back into the host subgroup, which permutes " +
                    "against the wrong row. Translation fails rather than " +
                    "permuting across the wrong lane grouping.");
                error =
            ...
            switch (instruction.Opcode)
            {
                ...
                case "VMulF32":
                    ...
                    break;
                case "VMulLegacyF32":
                    result = EmitFloatResult(instruction, EmitLegacyMultiply(instruction));
                    break;
                // EXTRACTED: ===== 0111.pdf p46 ===== and ===== 0111.pdf p49 =====
                // return the numeric operand when exactly one source is NaN.
                case "VMinF32":
                    result = EmitFloatExtBinary(instruction, GlslNMin);
                    break;
                case "VMaxF32":
                    result = EmitFloatExtBinary(instruction, GlslNMax);
                    break;
                case "VMadF32":
                case "VMadMkF32":
                case "VMadAkF32":
                ...
                case "VMacF32":
                {
                    ...
                    result = EmitFloatResult(
                        instruction,
                        EmitUnfusedMultiplyAdd(instruction, addend));
                    break;
                }
                case "VMacLegacyF32":
                {
                    ...
                    result = EmitFloatResult(
                        instruction,
                        EmitUnfusedLegacyMultiplyAdd(instruction, addend));
                    break;
                }
                case "VFmacF32":
                {
                ...
                case "VMin3F32":
                    result = EmitFloatTernaryExt(instruction, GlslNMin);
                    break;
                case "VMax3F32":
                    result = EmitFloatTernaryExt(instruction, GlslNMax);
                    break;
                case "VAndB32":
                ...
                case "VMed3F32":
                {
                    ...
                    var low = Ext(GlslNMin, _floatType, left, middle);
                    var high = Ext(GlslNMax, _floatType, left, middle);
                    var median = Ext(
                        GlslNMax,
                        _floatType,
                        low,
                        Ext(GlslNMin, _floatType, high, right));
                    // EXTRACTED: ===== 0111.pdf p48 ===== requires vsrc2 when
                    // any of the three sources is NaN.
                ...
                // Sum-of-absolute-differences. D = SAD(S0, S1) + S2, with the
                // difference taken per sub-word and always unsigned. The
                // unsigned max-minus-min form is the same one shadPS4 uses for
                // V_SAD_U32 (frontend/translate/vector_alu.cpp:1525-1527).
                case "VSadU8":
                case "VSadHiU8":
                {
                    var left = GetRawSource(instruction, 0);
                    ...
                    {
                        var shift = UInt((uint)(byteIndex * 8));
                        sum = IAdd(
                            sum,
                            UnsignedAbsoluteDifference(
                                BitwiseAnd(ShiftRightLogical(left, shift), UInt(0xFF)),
                                BitwiseAnd(ShiftRightLogical(right, shift), UInt(0xFF))));
                    }

                    // v_sad_hi_u8 places the 4-byte SAD in the upper half before
                    // accumulating: D = (SAD_U8(S0, S1, 0) << 16) + S2.
                    if (instruction.Opcode == "VSadHiU8")
                    {
                        sum = ShiftLeftLogical(sum, UInt(16));
                    }

                    result = IAdd(sum, GetRawSource(instruction, 2));
                    break;
                }
                case "VSadU16":
                {
                    var left = GetRawSource(instruction, 0);
                    var right = GetRawSource(instruction, 1);
                    var sum = UnsignedAbsoluteDifference(
                ...
                // Integer pack: D = {clamp16(S1), clamp16(S0)}.
                //
                // ASSUMED, not extracted: the AMD ISA pseudo-code for these two
                // reads as a plain narrowing cast, and neither reference tree
                // here settles it - shadPS4 routes them through a generic
                // Pack2x16 whose Uint/Sint behaviour is not defined in the
                // frontend, and KytyPS5 decodes the opcode without emitting it.
                // Saturating is the reading taken because it is what makes
                // v_cvt_pk_*_* a distinct opcode from the bit-pack idioms
                // (v_perm_b32, s_pack_ll_b32_b16) that the compiler would emit
                // otherwise. Only a hardware trace or the AMD ISA manual can
                // decide between clamp and truncate for inputs outside 16 bits;
                // in range the two agree exactly.
                case "VCvtPkU16U32":
                {
                ...
                case "VCvtPkI16I32":
                {
                    var low = BitwiseAnd(SaturateToInt16(GetRawSource(instruction, 0)), UInt(0xFFFF));
                    var high = BitwiseAnd(SaturateToInt16(GetRawSource(instruction, 1)), UInt(0xFFFF));
                    result = BitwiseOr(low, ShiftLeftLogical(high, UInt(16)));
                    break;
                }

                // D.f32 = S0.f16[0]*S1.f16[0] + S0.f16[1]*S1.f16[1] + D.f32.
                // The "c" forms accumulate into the destination, like v_mac/v_fmac.
                case "VDot2cF32F16":
                {
                    var left = GetRawSource(instruction, 0);
                    var right = GetRawSource(instruction, 1);
                    var lowProduct = _module.AddInstruction(
                        SpirvOp.FMul,
                        _floatType,
                        Bitcast(_floatType, EmitHalfToFloat(BitwiseAnd(left, UInt(0xFFFF)))),
                        Bitcast(_floatType, EmitHalfToFloat(BitwiseAnd(right, UInt(0xFFFF)))));
                    var highProduct = _module.AddInstruction(
                        SpirvOp.FMul,
                        _floatType,
                        Bitcast(_floatType, EmitHalfToFloat(ShiftRightLogical(left, UInt(16)))),
                        Bitcast(_floatType, EmitHalfToFloat(ShiftRightLogical(right, UInt(16)))));
                ...
                // Relative VGPR addressing: the index is the encoded register
                // number plus M0 (SGPR 124). Masked to 8 bits because an RDNA2
                // instruction can only name 256 VGPRs.
                case "VMovrelsB32":
                    if (!TryGetMovrelSourceRegister(instruction, out var relativeSource, out error))
                    {
                        return false;
                    }

                    result = LoadVDynamic(relativeSource);
                    break;
                case "VMovreldB32":
                case "VMovrelsdB32":
                {
                    if (instruction.Control is Gen5DppControl or Gen5Dpp8Control or Gen5SdwaControl)
                    {
                        // The DPP/SDWA post-pass below rewrites a *fixed*
                        // destination; it cannot express a relative one.
                        error =
                            $"{instruction.Opcode} with DPP/SDWA is not implemented";
                        return false;
                    }

                    uint moved;
                    if (instruction.Opcode == "VMovrelsdB32")
                    {
                        if (!TryGetMovrelSourceRegister(instruction, out var source, out error))
                        {
                            return false;
                        }

                        moved = LoadVDynamic(source);
                    }
                    else
                    {
                        moved = GetRawSource(instruction, 0);
                    }

                    StoreVDynamic(RelativeVectorRegister(destination), moved);
                    return true;
                }
                case "VPkAddF16":
                case "VPkMulF16":
        ...
        // EXTRACTED: ===== 38.pdf p9 ===== saturates to [+0, 1] and flushes NaN
        // to +0 in DX10_CLAMP mode. Ordered comparisons implement both rules.
        ...
        private uint EmitUnfusedMultiplyAdd(
            Gen5ShaderInstruction instruction,
            uint addend)
        {
            // EXTRACTED: ===== 0111.pdf p39 ===== through ===== 0111.pdf p45 =====
            // define MAD and MAC as v_mul_f32 followed by v_add_f32, with
            // rounding and NaN handling applied between the operations.
            var product = EmitPreciseFloat(
                SpirvOp.FMul,
                GetFloatSource(instruction, 0),
                GetFloatSource(instruction, 1));
            return EmitPreciseFloat(SpirvOp.FAdd, product, addend);
        }

        private uint EmitLegacyMultiply(Gen5ShaderInstruction instruction)
        {
            var left = GetFloatSource(instruction, 0);
            ...
            var eitherZero = _module.AddInstruction(
                ...
                _module.AddInstruction(
                    ...
                    Float(0)));
            var product = EmitPreciseFloat(SpirvOp.FMul, left, right);
            // EXTRACTED: ===== 0111.pdf p41 =====, ===== 0111.pdf p42 =====,
            // and ===== 0111.pdf p51 ===== require 0.0*x to be +0.0 for NaN
            // and Inf as well as finite x.
        ...
        private uint EmitUnfusedLegacyMultiplyAdd(
            Gen5ShaderInstruction instruction,
            uint addend) =>
            EmitPreciseFloat(
                SpirvOp.FAdd,
                EmitLegacyMultiply(instruction),
                addend);
        ...
        /// <summary>|a - b| for unsigned 32-bit operands, without overflow.</summary>
        private uint UnsignedAbsoluteDifference(uint left, uint right) =>
            _module.AddInstruction(
                SpirvOp.ISub,
                _uintType,
                Ext(41, _uintType, left, right),   // GLSL UMax
                Ext(38, _uintType, left, right));  // GLSL UMin

        /// <summary>Clamps a 32-bit value to the signed 16-bit range.</summary>
        private uint SaturateToInt16(uint value) =>
            Bitcast(
                _uintType,
                Ext(
                    45,  // GLSL SClamp
                    ...
                    Bitcast(_intType, UInt(unchecked((uint)short.MinValue))),
                    Bitcast(_intType, UInt((uint)short.MaxValue))));

        /// <summary>
        /// The VGPR index an M0-relative access resolves to: the encoded
        /// register number plus M0 (SGPR 124), wrapped to the 256-entry VGPR
        /// file an RDNA2 instruction can name.
        /// </summary>
        private uint RelativeVectorRegister(uint baseRegister) =>
            BitwiseAnd(IAdd(UInt(baseRegister), LoadS(124)), UInt(0xFF));

        private bool TryGetMovrelSourceRegister(
            Gen5ShaderInstruction instruction,
            out uint index,
            out string error)
        {
            ...
            {
                // v_movrels* index a VGPR by *register number*; a constant or an
                // SGPR source has no register number to offset.
                error = $"{instruction.Opcode} requires a vector register source";
                return false;
            }

            index = RelativeVectorRegister(instruction.Sources[0].Value);
            return true;
        }

        private uint LoadVDynamic(uint index) =>
            Load(
                _uintType,
                _module.AddInstruction(
                    SpirvOp.AccessChain,
        ...
        private void StoreVDynamic(uint index, uint value)
        {
            ...
            out string error)
        {
            ...
            // The always-false / always-true forms. The EXEC-writing integer
            // twins (v_cmpx_f_i32 0x90, v_cmpx_f_u32 0xD0, v_cmpx_t_i32 0x97,
            // v_cmpx_t_u32 0xD7) were missing here, so they fell into the
            // integer table below, matched nothing, and dropped the draw - even
            // though v_cmpx_t_* is the standard "enable every lane" idiom.
            else if (opcode is
            ...
            if (_state.Program.Address == 0x0000000500781200ul &&
                ((instruction.Pc == 0x4D4 &&
                  Environment.GetEnvironmentVariable(
                      "PROSPERISMO_FORCE_TITLE_COMPARE_4D4") == "1") ||
                 (instruction.Pc == 0x540 &&
                  Environment.GetEnvironmentVariable(
                      "PROSPERISMO_FORCE_TITLE_COMPARE_540") == "1")))
            {
                condition = _module.ConstantBool(true);
            }
            ...
            {
                // VOP3-encoded compares name their scalar destination pair in
                // the vdst byte; the 32-bit VOPC form always writes VCC.
                var compareDestination = instruction.Control is Gen5SdwaControl
                    { ScalarDestination: { } scalarDestination }
                    ? scalarDestination
                    : instruction.Destinations is
                        [{ Kind: Gen5OperandKind.ScalarRegister } destination]
                        ? destination.Value
                        : 106u;
            ...
            out string error)
        {
            ...
            if (instruction.Opcode is "SSetpcB64" or "SSwappcB64")
            {
                ...
                // s_swappc_b64 does the same and saves the return address. This
                // backend does not execute guest addresses: TryEmitBlock lowers
                // control flow to a dispatch loop over *statically discovered
                // block indices* (Store(_programCounter, UInt(blockIndex))), and
                // block indices are compile-time constants. A runtime SGPR pair
                // cannot be turned into one, and there is no return-address
                // stack for the call form either.
                //
                // Supporting these means resolving the target statically - the
                // s_getpc_b64 / s_add_u32 / s_addc_u32 / s_setpc_b64 long-jump
                // idiom is constant-foldable by Gen5ShaderScalarEvaluator - and
                // then teaching block discovery about the extra edge, plus
                // inlining the callee for s_swappc_b64. Until that exists,
                // failing is the honest outcome: emitting a fallthrough would
                // silently run the wrong code.
                ReportDiagnosticOnce(
                    $"scalar-indirect-pc:{instruction.Opcode}",
                    $"[SPIRV][ERROR] program=0x{_state.Program.Address:X16} " +
                    $"pc=0x{instruction.Pc:X} {instruction.Opcode} is a shader " +
                    "subroutine jump/call through an SGPR pair. The translator " +
                    "lowers control flow to a dispatch loop over statically " +
                    "discovered blocks, which cannot take a runtime PC, and it " +
                    "has no return-address stack. The draw is dropped rather " +
                    "than silently falling through to the next instruction.");
                error =
                    $"{instruction.Opcode} needs an indirect program counter, " +
                    "which the structured block dispatcher cannot express";
                return false;
            }
            ...
            if (instruction.Opcode.EndsWith("B64", StringComparison.Ordinal) ||
                // s_ashr_i64 is a 64-bit operation whose name ends in I64, so the
                // B64 test missed it and it fell into the 32-bit SOP2 switch and
                // failed the whole shader.
            ...
            if (instruction.Opcode.EndsWith("SaveexecB32", StringComparison.Ordinal))
            {
                ...
                // EXTRACTED: ===== 38.pdf p9 ===== makes the B32 thread mask
                // one dword wide, so EXEC_HI remains available to the program.
                StoreS(126, newExec);
            ...
            switch (instruction.Opcode)
            {
                ...
                default:
                {
                    ...
                    switch (instruction.Opcode)
                    {
                        ...
                        case "SMulHiI32":
                        {
                            // D.i32 = (S0.i32 * S1.i32) >> 32, signed. Sign-extend
                            // both operands to 64 bits first, then take the high
                            // half arithmetically. SCC is not written (same as
                            // s_mul_i32 / s_mul_hi_u32).
                            var product = _module.AddInstruction(
                                SpirvOp.IMul,
                                _longType,
                                _module.AddInstruction(
                        ...
                        case "SAbsdiffI32":
                        {
                            // D.i32 = abs(S0.i32 - S1.i32); SCC = (D != 0).
                            // The subtraction is done in 32 bits exactly as the
                            // hardware does, so INT_MIN wraps identically.
                            ...
                            result = Bitcast(
                                _uintType,
                                Ext(5, _intType, difference));  // GLSL SAbs
                            Store(_scc, IsNotZero(result));
                            break;
                        }
                        case "SAndB32":
            ...
            out string error)
        {
            ...
            {
                // SCC = (S0.u64[S1.u32[5:0]] == 0 or 1). The bit index is taken
                // modulo 64 for the 64-bit forms, not modulo 32.
                ...
                var isSet = IsNotZero64(
                    _module.AddInstruction(
                        SpirvOp.BitwiseAnd,
                        _ulongType,
                        ShiftRightLogical64(wide, index),
            ...
            out string error)
        {
            ...
            if (instruction.Opcode is "SLshlB64" or "SLshrB64" or "SAshrI64")
            {
                ...
                // The shift amount is S1[5:0]; the upper bits are ignored, and
                // masking also keeps the SPIR-V shift inside its defined range.
                var shift = _module.AddInstruction(
                    SpirvOp.UConvert,
                    _ulongType,
                    BitwiseAnd(GetRawSource(instruction, 1), UInt(63)));
                uint shiftedValue;
                if (instruction.Opcode == "SAshrI64")
                {
                    // Arithmetic: shift as a signed 64-bit value so the sign bit
                    // replicates, then reinterpret for the unsigned SGPR pair.
                    shiftedValue = _module.AddInstruction(
                        SpirvOp.Bitcast,
                        _ulongType,
                        _module.AddInstruction(
                            SpirvOp.ShiftRightArithmetic,
                            _longType,
                            _module.AddInstruction(SpirvOp.Bitcast, _longType, left),
                            shift));
                }
                else
                {
                    shiftedValue = instruction.Opcode == "SLshlB64"
        ...
        private uint ApplyDpp8Source(Gen5Dpp8Control control, uint value)
        {
            ...
            var shuffled = SubgroupShuffle(value, targetLane);
            ...
            var activeWord = _module.AddInstruction(
                ...
                UInt(1),
                UInt(0));
            var sourceActive = IsNotZero(SubgroupShuffle(activeWord, targetLane));
            return _module.AddInstruction(
                SpirvOp.Select,
                _uintType,
                sourceActive,
        ...
        private uint ApplyDppSource(Gen5DppControl control, uint value)
        {
            ...
            var shuffled = SubgroupShuffle(value, safeTarget);
            ...
            if (!control.FetchInactive)
            {
                var activeWord = _module.AddInstruction(
                    ...
                    UInt(1),
                    UInt(0));
                var shuffledActive = SubgroupShuffle(activeWord, safeTarget);
        ...
        private uint IsDppWriteEnabled(Gen5DppControl control)
        {
            ...
            // EXTRACTED: ===== 38.pdf p6 ===== defines
            // bank_id = (lane_id / 4) % 4.
            var bank = BitwiseAnd(
                ShiftRightLogical(lane, UInt(2)),
                UInt(3));
            ...
            out string error)
        {
            ...
            if (_emulateWave64)
            {
                // sdst = vsrc0[lane(src1)] with lane(src1) in 0..63.
                // OpGroupNonUniformBroadcast cannot express that: its Id is a
                // lane of the 32-lane host SUBGROUP, and SPIR-V leaves the
                // result undefined once Id reaches the subgroup size. Route
                // through the same workgroup-scratch rendezvous the wave64
                // bridge already uses for v_readfirstlane, which reaches all 64
                // guest lanes because the workgroup IS the wave in this mode.
                StoreS(destination, BroadcastWave64Lane(src0, laneSelect));
                return true;
            }

            if (_waveLaneCount == 64)
            {
                // wave64 without the bridge: the other 32 guest lanes are not
                // in this host subgroup at all and there is no rendezvous to
                // reach them through. A broadcast id of 32..63 is undefined in
                // SPIR-V and folding it to 0..31 would read a lane of the wrong
                // half. Refuse.
                ReportDiagnosticOnce(
                    "readlane-wave64-unmodelled",
                    $"[SPIRV][ERROR] program=0x{_state.Program.Address:X16} " +
                    "v_readlane_b32 in a wave64 program whose workgroup " +
                    $"({_localSizeX}x{_localSizeY}x{_localSizeZ}) is not one " +
                    "64-lane wave cannot reach the upper half of the wave: " +
                    "the wave64 bridge needs the workgroup to be the wave. " +
                    "Translation fails rather than broadcasting from a lane of " +
                    "the wrong half.");
                error = "v_readlane_b32 cannot be modelled in this wave64 program";
                return false;
            }

            // wave32. The lane select is masked into the modelled wave so the
            // OpGroupNonUniformBroadcast id cannot leave the host subgroup: an
            // out-of-range id is undefined in SPIR-V, and the guest's own
            // wave32 lane select is only meaningful modulo the wave anyway.
            var broadcast = _module.AddInstruction(
                SpirvOp.GroupNonUniformBroadcast,
                _uintType,
                UInt(3),  // Subgroup scope
                src0,
                BitwiseAnd(laneSelect, UInt(GuestWaveLaneMask)));
            StoreS(destination, broadcast);
            return true;
        }

        /// <summary>
        /// Broadcasts <paramref name="value"/> from the guest lane selected by
        /// <paramref name="laneSelect"/> to every lane of a 64-lane guest wave.
        /// </summary>
        /// <remarks>
        /// Only valid under <see cref="_emulateWave64"/>: it rendezvouses
        /// through the workgroup-scoped wave64 scratch dword and a workgroup
        /// <c>OpControlBarrier</c>, which only spans the guest wave when the
        /// workgroup IS the wave. Structurally identical to
        /// <see cref="BroadcastFirstWave64Active"/>, which is the same bridge
        /// with "first EXEC-active lane" instead of an explicit lane select.
        /// The scratch is pre-cleared so a select naming a lane that does not
        /// exist yields 0 rather than whatever the previous broadcast left.
        /// </remarks>
        private uint BroadcastWave64Lane(uint value, uint laneSelect)
        {
            var lane = GuestWaveLane();
            EmitConditional(
                _module.AddInstruction(
            ...
            var selected = BitwiseAnd(laneSelect, UInt(63));
            EmitConditional(
                _module.AddInstruction(
                    SpirvOp.IEqual,
                    _boolType,
            ...
            uint value)
        {
            ...
            if (clamp)
            {
                // EXTRACTED: ===== 38.pdf p9 ===== requires DX10_CLAMP to
                // flush NaN to +0 before saturating to [+0, 1].
                value = Bitcast(
                    _floatType,
                    EmitClampToUnitInterval(Bitcast(_uintType, value)));
            }
